...
<head>
    ...
    <?php echo Common::getScript("uploadify/jquery.uploadify.min.js,listimageup.js,common.js,choose.js"); ?>
            ...
            <td valign="top" class="content-rt-td" style="overflow:hidden">
                ...
                <div class="cfg-search-bar" >
                    <div class="cfg-header-search">
                        <input type="text" id="searchkey" placeholder="目的地名称" datadef="目的地名称" class="search-text">
                        <a href="javascript:;" title="搜索" class="search-btn" onclick="searchDest()">搜索</a>
                    </div>
                    <span class="cfg-search-tab display-mod fr">
                        <a href="javascript:void(0);" title="列表" class="item on" onClick="togMod(this,1)">列表</a>
                        <a href="javascript:void(0);" title="高级" class="item" onClick="togMod(this,2)">高级</a>
                    </span>
                </div>
...
<script>
    ...
    var typeid="<?php echo $model_info['id'];?>";
        ...
        function () {
            ...
            window.dest_store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    api: {
                        read: SITEURL+'spot/admin/destination/destination/action/read',  //读取数据的URL
                        update: SITEURL+'spot/admin/destination/destination/action/save',
                        destroy: SITEURL+'spot/admin/destination/destination/action/delete'
                    },
                    reader: 'json'
                },
                autoLoad: true,
            ...
            window.dest_treepanel = Ext.create('Ext.tree.Panel', {
                ...
                viewConfig: {
                    ...
                    listeners: {
                        boxready: function () {
                            var height = Ext.dom.Element.getViewportHeight();
                            this.up('treepanel').maxHeight = height - 40;
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        text: '目的地'+ST.Util.getGridHelp('spot_destination'),
                        dataIndex: 'kindname',
                        id: 'dest_name',
                        menuDisabled:true,
                        sortable:false,
                    ...
                    {
                        text: '显示'+ST.Util.getGridHelp('spot_destination_isopen'),
                        dataIndex: 'isopen',
                        width: '10%',
                        xtype: 'actioncolumn',
                        tdCls: 'dest-al-mid',
                    ...
                    {
                        text: '栏目首页显示'+ST.Util.getGridHelp('spot_destination_index_isopen'),
                        dataIndex: 'isnav',
                        width: '10%',
                        xtype: 'actioncolumn',
                        tdCls: 'dest-al-mid',
                    ...
                    {
                        text: '是否热门'+ST.Util.getGridHelp('spot_destination_hot'),
                        dataIndex: 'ishot',
                        width: '10%',
                        xtype: 'actioncolumn',
                        tdCls: 'dest-al-mid',
                    ...
                    {
                        text: '优化设置'+ST.Util.getGridHelp('spot_destination_seo'),
                        ...
                        renderer : function(value, metadata,record) {
                            ...
                            if(value==1)
                            {
                                return '<a href="javascropt:;" onclick="destProductSet(\'' + id + '\')"><span>已设置</span></a>'
                            }
                            else
                            {
                                return '<a href="javascropt:;" onclick="destProductSet(\'' + id +  '\')"><span style="color:red;">未设置</span></a>'
                            }
                        },
                        listeners: {
                            afterrender: function (obj, eopts) {
                    ...
                    {
                        text: '<span class="grid_column_text">PC端模板设置</span>'+ST.Util.getGridHelp('templates_name'),
                        ...
                        renderer : function(value, metadata,record) {
                            ...
                            return '<a href="javascript:;" title="PC端模板设置" onclick="setTemplate(\'' + id + '\',\'pc\')">'+value+'</a>';
                        },
                        listeners:{
                            afterrender:function(obj,eopts)
                            {
                    ...
                    {
                        text: '<span class="grid_column_text">移动端模板设置</span>'+ST.Util.getGridHelp('templates_name'),
                        dataIndex: 'wap_templet_name',
                        width: '12%',
                        align: 'center',
                        sortable:false,
                        menuDisabled:true,
                        renderer : function(value, metadata,record) {
                            ...
                            return '<a href="javascript:;" title="移动端模板设置" onclick="setTemplate(\'' + id + '\',\'web\')">'+value+'</a>';
                        },
                        listeners:{
                            afterrender:function(obj,eopts)
                            {
                        ...
                        tpl: new Ext.XTemplate(
                            '{[this.realName(values.id)]}',
                            {
                                realName: function (id) {
                                    if (id.indexOf('add') > 1)
                                        return '';
                                    var viewHtml=window.display_mode!=11?'&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="btn-link" title="预览" onclick="viewDestType(\''+id+'\')">预览</a>':'';
                                    return viewHtml;
                                }
                            }
                        ),
    ...
    function togStatus(obj, record, field) {
        ...
        Ext.Ajax.request({
            url: SITEURL+"spot/admin/destination/destination/action/update",
            method: "POST",
            datatype: "JSON",
            params: {id: id, field: field, val: newval, typeid: typeid},
    ...
    function updateField(ele,id,field,val,type)
    {
        ...
        Ext.Ajax.request({
            ...
            method: "POST",
            datatype: "JSON",
            params: {id: id, field: field, val: val, typeid: typeid},
            success: function (response, opts) {
                if (response.responseText == 'ok') {
    ...
    function destProductSet(dest_id) {
        ...
        var channel_name = "<?php echo $channel_name;?>";
        ST.Util.addTab(channel_name+record.data.kindname+'优化设置', 'spot/admin/destination/seo/menuid/<?php echo $_GET["menuid"];?>/id/'+id);
        //ST.Util.showBox('设置',SITEURL+'line/admin/destination/dialog_productinfo?id='+id+"&typeid="+display_mode,600,500,null,null,document,{loadCallback:setProductInfo,loadWindow:window});
    }
    //设置模板
    function setTemplate(dest_id,webtype) {
        var id = dest_id.substr(dest_id.indexOf('_') + 1);
        ST.Util.showBox('选择模板',SITEURL+'spot/admin/destination/set_template/id/'+id+'/webtype/'+webtype,600,'',null,null,document,{loadCallback:setTemplateCallback,loadWindow:window});
    }
    //设置模板回调
    function setTemplateCallback(result,bool)
    {
        ...
        $.ajax({
            data:result.data,
            dataType:'json',
            url:SITEURL+'spot/admin/destination/ajax_save_template',
            type:'post',
            success:function (data) {
                var record = dest_store.getNodeById(data.id);
                for(var i in data)
                {
                    var val=data[i];
                    record.set(i,val);
    ...
    Ext.EventManager.onWindowResize(function () {
        ...
        if (data_height > height - 40)
            window.dest_treepanel.height = (height - 40);
        else
    ...
    function searchDest() {
        ...
        if (!s_str)
            return;
        Ext.Ajax.request({
            url: SITEURL+'spot/admin/destination/destination/action/search',
