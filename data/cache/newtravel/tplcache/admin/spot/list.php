            ...
            <td valign="top" class="content-rt-td" style="overflow:hidden">
                ...
                <div class="cfg-search-bar" id="search_bar">
                    ...
                    <div class="cfg-header-search">
                        <input type="text" id="searchkey" placeholder="景点名称/景点编号" datadef="景点名称/景点编号/供应商" class="search-text">
                        <a href="javascript:;" class="search-btn" onclick="CHOOSE.searchKeyword()">搜索</a>
                    </div>
                    <span class="cfg-search-tab display-mod fr">
                        <a href="javascript:void(0);" title="基本信息" class="item on" onClick="CHOOSE.togMod(this,1)">景点</a>
                        <a href="javascript:void(0);" title="套餐" class="item" onClick="CHOOSE.togMod(this,2)">门票</a>
...
Ext.onReady(
    function () {
        ...
        $("#addbtn").click(function(){
            ST.Util.addTab('添加景点',SITEURL+'spot/admin/spot/add/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/product/itemid/4',0);
        });
        //产品store
            ...
            columns: [
              ...
              /*  {
               ...
               /* {
                    ...
                    border: 0,
                    sortable: false,
                    dataIndex: 'day_before',
                    cls: 'mod-2',
                    renderer: function (value, metadata, record, rowindex, colindex) {
                        var day_before = record.get('day_before');
                        var hour_before = record.get('hour_before');
                        var minute_before = record.get('minute_before');
                ...
                },
                {
                    ...
                    renderer: function (value, metadata, record) {
                        ...
                        if (id.indexOf('suit') != -1) {
                            return "<a href='javascript:;' class='btn-link' title='编辑' onclick=\"modSuit('"+ticketid+"','"+spotid+"')\">编辑</a>&nbsp;&nbsp;<a href='javascript:;' title='删除' class='btn-link' onclick=\"delSuit('" + id + "')\">删除</a>";
                        }
                        else
                        {
                            var spotid = id;
                            var spotkind_url='spot/admin/spot/tickettype/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/kind/itemid/6/spotid/'+spotid;
                            var addticket_url = 'spot/admin/spot/ticket_op/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/product/itemid/4/action/add/spotid/'+spotid;
                            return '<a href="javascript:;" class="btn-link" onclick="ST.Util.addTab(\''+spotname+'-门票类型\',\''+spotkind_url+'\')">门票类型<a>&nbsp;&nbsp;<a href="javascript:;" class="btn-link" onclick="ST.Util.addTab(\'添加门票\',\''+addticket_url+'\')">添加门票</a>';
                        }
                        // return getExpandableImage(value, metadata,record);
                    },
                    listeners: {
...
{
    var url = SITEURL+'spot/admin/spot/ticket/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/product/itemid/4/spotid/'+id;
...
function goModify(id)
{
    ...
    var url = SITEURL+'spot/admin/spot/edit/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/product/itemid/4/id/'+id;
...
function modSuit(ticketid,spotid)
{
    ...
    var title = record.get('title');
    var url = SITEURL+'spot/admin/spot/ticket_op/<?php if(isset($_GET['menuid'])) { ?>menuid/<?php echo $_GET['menuid'];?>/<?php } ?>
parentkey/product/itemid/4/action/edit/spotid/'+spotid+'/ticketid/'+ticketid;
...
function supplierCheckBox(id)
{
    ...
    ST.Util.showBox("审核",SITEURL+'spot/admin/spot/dialog_supplier_check?suitid='+suitid,500,'',null,null,document,{loadWindow: window, loadCallback: setSupplierCheck});
}
//执行审核
function setSupplierCheck(result,bool)
{
    Ext.Ajax.request({
        url   :  SITEURL+"spot/admin/spot/dialog_supplier_check",
        method  :  "get",
        datatype  :  "JSON",
