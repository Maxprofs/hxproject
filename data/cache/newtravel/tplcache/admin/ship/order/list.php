...
<head>
    <meta charset="utf-8">
    <title>租车订单管理-笛卡CMS<?php echo $coreVersion;?></title>
...
<body style="overflow:hidden" bottom_margin=thQzDt >
    ...
    function () {
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL+'ship/admin/order/index/action/read/',  //读取数据的URL
                    update: SITEURL+'ship/admin/order/index/action/save/',
                    destroy: SITEURL+'ship/admin/order/index/action/delete/'
                },
                reader: {
                    type: 'json',   //获取数据的格式
                    root: 'lists',
...
$(function(){
    ...
    $(".btn_report").click(function(){
        var url=SITEURL+"ship/admin/order/dataview/typeid/"+typeid;
        ST.Util.showBox(channelname+'订单数据报表查看',url,860,510,function(){});
    })
    //导出excel
    $(".btn_excel").click(function(){
        var url=SITEURL+"ship/admin/order/excel/typeid/"+typeid;
        ST.Util.showBox(channelname+'订单生成excel',url,560,380,function(){});
    })
    //获取当前产品订单常规信息
    $.getJSON(SITEURL+'ship/admin/order/ajax_sell_info/typeid/'+typeid,function(data){
...
function updateField(ele, id, field, value, type) {
    ...
    var view_el = window.product_grid.getView().getEl();
    Ext.Ajax.request({
        url: SITEURL+"ship/admin/order/index/action/update/",
        method: "POST",
        datatype: "JSON",
...
{
    var record = window.product_store.getById(id.toString());
    var url=SITEURL+"ship/admin/order/view/id/"+id+"/typeid/"+typeid+"<?php if(isset($_GET['menuid'])) { ?>/menuid/<?php echo $_GET['menuid'];?><?php } ?>
";
    ST.Util.addTab('邮轮订单：'+record.get('ordersn'),url,1);
}
</script>
</body>
</html>
