<!doctype html>
<html>
<head head_top=z8MzDt >
    <meta charset="utf-8">
    <title>图片管理-笛卡CMS<?php echo $coreVersion;?></title>
        ...
        <td valign="top" class="content-rt-td">
            <div class="cfg-header-bar clearfix">
                ...
                <div class="f-0 mt-6 fr">
                    <a href="javascript:;" class="btn btn-primary radius mr-10" id="image_param">缩略图配置</a>
                    ...
                    <a href="javascript:;" class="btn btn-primary radius mr-10" id="img_server" data="<?php echo $config['url'];?>">图片服务器</a>
                   <!-- <a href="javascript:;" class="btn btn-primary radius mr-10" id="img_config">图库配置<?php echo Common::get_help_icon('image_url');?></a>-->
...
<script>
    ...
    $('#upload_image').click(function () {
        ...
        function uploadInfo(res,bool){
            if(image_init_param.pid==parseInt(res.id)){
                var keyword = $("#keyword").val();
                var search_type = $('#search_type').val();
                window.image_init_param.url = SITEURL + 'image/group_manage';
                //保持筛选条件搜索
                window.image_init_param.url_param={action:'find',page:1,search_type:search_type,keyword:keyword};
    ...
    $('#create_group').click(function () {
        ...
        function add_group(result, bool) {
            ...
            result.data.level=parseInt('<?php echo $level;?>')+1;
            $.ajax({
                url: SITEURL + 'image/group_manage',
                async: false,
                type: 'POST',
    ...
    //缩略图配置
    $('#image_param').click(function () {
        var url = SITEURL + "image/img_url_config/menuid/<?php echo $_GET['menuid'];?>";
        ST.Util.addTab('缩略图配置', url);
    });
    //图片加速
    $('#img_promote').click(function () {
        var url = SITEURL + "image/promote/menuid/<?php echo $_GET['menuid'];?>";
    ...
    $("#recycle").click(function () {
        var url = SITEURL + "image/recycle/menuid/<?php echo $_GET['menuid'];?>";
    ...
    function load_data() {
        ...
        $.post(image_init_param.url, image_init_param.url_param, function (data) {
            ...
            if (image_init_param.url_param.search_type==0) {
                ...
                window.image_init_param.url_param.search_type=2;
                load_data();
            }
        }, 'json');
    }
