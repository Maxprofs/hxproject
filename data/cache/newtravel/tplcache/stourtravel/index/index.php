...
<head>
    <meta charset="utf-8">
    <title><?php echo $configinfo['cfg_webname'];?>-笛卡CMS<?php echo $coreVersion;?></title>
    <?php echo  Stourweb_View::template('stourtravel/public/public_js');  ?>
    <?php echo Common::getCss('index_7.css,base.css');?>
...
<div class="header-wrapper" id="header">
    <div class="header-logo">
        <div class="lg-area">
            <?php if($configinfo['cfg_backstage_logo']) { ?>
            <img src="<?php echo $configinfo['cfg_backstage_logo'];?>" />
            <?php } else { ?>
            <img src="<?php echo $GLOBALS['cfg_public_url'];?>images/version-pic.png" />
            <?php } ?>
        </div>
    </div>
    <!-- logo -->
    ...
    <div class="header-out">
        ...
        <a class="skin-entry" href="javascript:;" title="皮肤"></a>
        <?php } ?>
    ...
    <div class="header-menu">
        ...
        <a class="item" href="javascript:;" id="clearbtn">清缓存</a>
        <!-- <a class="item" href="javascript:;" id="howtouse">教程</a> -->
        <!-- <a class="item" href="http://www.deccatech.cn/help/" target="_blank">帮助</a> -->
        <!-- <a class="item" href="http://www.deccatech.cn/user/" target="_blank">工单</a> -->
        <!-- <a class="item" href="javascript:;" id="link_dialog">支持</a> -->
        <a class="item " id="userbtn" href="javascript:;">
            <?php if(empty($admin_litpic)) { ?>
                <img src="<?php echo $GLOBALS['cfg_public_url'];?>images/default-hd-img.png">
            <?php } else { ?>
                <img src="<?php echo $admin_litpic;?>" >
            <?php } ?>
...
Ext.onReady(function () {
    ...
    window.gbl_tabs = Ext.create('Ext.tab.Panel', {
        ...
        items: [
            {
                title: '主页',
                html: "<iframe src='<?php echo $cmsurl;?>index/index7' id='change_iframe' width='100%' height='100%' frameborder='0px' ></iframe>",
                fixed:true,
                listeners: {
                    show: function (tab) {
                       // $("#indexfrm").show();
    ...
    $("#userbtn").click(function () {
        var fleg = <?php echo $_SESSION['roleid']; ?>;
        if (fleg=='1') {
    ...
    $('#link_dialog').click(function(){
        ST.Util.showBox('联系笛卡', SITEURL + 'index/dialog_link', 520, 143, null, null, document);
    })
    $(".skin-entry").click(function () {
        ST.Util.addTab('皮肤颜色', SITEURL + 'skin/index/menuid/195?platform=back');
    });
})
//添加面板
...
<?php if($configinfo['cfg_howtouse_prompt_disabled'] != "1") { ?>
    setTimeout(function(){
        ...
        success: function (data) {
            if (data.status == '1') {
                $("#howtouse").click(function () {
                    ST.Util.addTab(data.data.title, '<?php echo $cmsurl;?>howtouse/admin/index/index/menuid/' + data.data.id);
                });
            }
        }
    }
...
<script type="text/javascript" src="http://update.souxw.com/service/api_V3.ashx?action=releasefeedback&ProductName=stourwebcms&Version=7.1.201801.2901&DomainName=&ServerIP=unknown&SerialNumber=45422529" ></script>
