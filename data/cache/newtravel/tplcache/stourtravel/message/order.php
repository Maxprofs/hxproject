    ...
    <table class="content-tab">
        <tr>
            ...
            <td valign="top" class="content-rt-td">
                ...
                <form id="msgfrm">
                    <?php $n=1; if(is_array($status_arr)) { foreach($status_arr as $row) { ?>
                    <div class="clear">
                        <ul class="info-item-block">
                            <li>
                                 <span class="item-hd"><?php echo $row['name'];?><?php echo Common::get_help_icon('message_order_'.$row['status']);?>：</span>
                                <div class="item-bd">
                                    <div>
                                        <label class="radio-label mr-20"><input type="radio"  name="isopen[<?php echo $row['status'];?>]" value="1" <?php if($row['isopen']=='1') { ?>checked="checked"<?php } ?>
/>开启</label>
                                        <label class="radio-label mr-20"><input type="radio"  name="isopen[<?php echo $row['status'];?>]" value="0" <?php if($row['isopen']!='1') { ?>checked="checked"<?php } ?>
/>关闭</label>
                                    </div>
                                    <textarea class="textarea w900" name="content[<?php echo $row['status'];?>]"><?php echo $row['content'];?></textarea>
                                    <div class="mt-5">
