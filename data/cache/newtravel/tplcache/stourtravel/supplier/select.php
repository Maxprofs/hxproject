...
<body >
    ...
    <div id="divids">
        <?php if($setids!='undefined') { ?>
            <input type="hidden" id="<?php echo $limittype;?>ids" name="<?php echo $limittype;?>ids" value="<?php echo $setids;?>">
        <?php } else { ?>
            <input type="hidden" id="<?php echo $limittype;?>ids" name="<?php echo $limittype;?>ids" value="">
        <?php } ?>
    </div>
    ...
    <script>
            ...
            $('#tree').treeview({
                ...
                onNodeChecked:function (e,n) {
                    ...
                    if (deststr.indexOf(n.text)==-1){
                        ...
                        if ($("#<?php echo $limittype;?>ids").val()!=='') {
                            $("#<?php echo $limittype;?>ids").val($("#<?php echo $limittype;?>ids").val()+','+id)
                        }else{
                            $("#<?php echo $limittype;?>ids").val(id);
                        }
                        if ($('#action',parent.document).val()=='edit') {
                                var tbl="<?php echo $limittype;?>";
                                if (tbl=='dest') {
                                    tbl='destinations'
                                }else{
                                    tbl='startplace'
                    ...
                    $.each(obj, function(i, v) {
                        ...
                        if (objval==nval) {
                            ...
                            if ($('#action',parent.document).val()=='add') {
                                var ids = $("#<?php echo $limittype;?>ids").val()
                                ids=ids.split(',');
                                var idx=ids.indexOf($(v).attr('id'));
                                ids.splice(idx,1);
                                ids=ids.join(',')
                                $("#<?php echo $limittype;?>ids").val(ids)
                            }else{
                                var tbl="<?php echo $limittype;?>";
                                if (tbl=='dest') {
                                    tbl='destinations'
        ...
        function setdest() {
                ...
                $("#<?php echo $limittype;?>_li",window.parent.document).html(htm);
                parent.d.remove()
        }
    </script>
</body>
