...
<head>
    <meta charset="utf-8">
    <title><?php echo __('用户注册');?>--<?php echo $webname;?></title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <?php echo Common::css('style.css,extend.css');?>
    <?php echo Common::css('base.css,header.css,account.css,mobilebone.css');?>
    ...
    <?php echo Common::js('Zepto.js,account.js,mobilebone.js');?>
    <script>
...
<div class="page out" id="reg">
    <div class="header_top bar-nav">
        <a class="back-link-icon" href="<?php echo $cmsurl;?>member/login" data-ajax=false></a>
        <h1 class="page-title-bar"><?php echo __('注册');?></h1>
    </div>
    <!-- 头部 -->
    <div class="account-container">
        <div class="account-content-box">
            <div class="account-tab-bar">
                <span class="item on" data-type="form-mobile"><?php echo __('手机号码注册');?></span>
                <span class="item" data-type="form-email"><?php echo __('电子邮箱注册');?></span>
            </div>
            <div class="account-tab-wrapper">
                <form id="form-mobile">
                    <ul class="account-group-list">
                        <li class="item-bar">
                            <input class="account-text form-clear-val" id="phone" type="text" name="user" placeholder="<?php echo __('请输入手机号码');?>">
                            <i class="clear-icon hide"></i>
                        </li>
                        <li class="item-bar">
                            <input class="account-text form-clear-val" type="text" id="mobile_code" name="code" placeholder="<?php echo __('图形验证码');?>">
                            <img  class="captcha yzm_pic cursor pic-yzm" src="" height="30"/>
                        </li>
                        <?php if($isopen) { ?>
                        <li class="item-bar">
                            <input class="account-text form-clear-val" type="text" name="msg" placeholder="<?php echo __('请输入短信验证码');?>">
                            <span class="dyn-code cursor" id="resend" do-send="true"><?php echo __('获取动态验证码');?></span>
                        </li>
                        <?php } ?>
                        <li class="item-bar">
                            <input class="account-text form-clear-val" type="password" name="pwd" placeholder="<?php echo __('设置登录密码(至少6位)');?>">
                            <i class="clear-icon hide"></i>
                        </li>
                    </ul>
                ...
                <form id="form-email">
                    <ul class="account-group-list">
                        <li class="item-bar">
                            <input class="account-text form-clear-val" id="email" type="text" name="user" placeholder="<?php echo __('请输入电子邮箱');?>">
                            <i class="clear-icon hide"></i>
                        </li>
                        <li class="item-bar">
                            <input class="account-text form-clear-val" type="text" id="email_code" name="code" placeholder="<?php echo __('图形验证码');?>">
                            <img  class="captcha yzm_pic cursor pic-yzm" src="" height="30"/>
                        </li>
                        <?php if($is_emailcode_open==1) { ?>
                        <li class="item-bar">
                            <input class="account-text form-clear-val" type="text" name="msg" placeholder="<?php echo __('请输入邮箱验证码');?>">
                            <span class="dyn-code" id="email_resend" do-send="true"><?php echo __('获取动态验证码');?></span>
                        </li>
                        <?php } ?>
                        <li class="item-bar">
                            <input class="account-text form-clear-val" type="password" name="pwd" placeholder="<?php echo __('设置登录密码(至少6位)');?>">
            ...
            <div class="register-agreement-bar">
                <span class="check-label checked" id="checkLabel"><i class="check-icon"></i><?php echo __('同意我们的');?></span><a href="#registerAgreement">《<?php echo $GLOBALS['cfg_member_agreement_title'];?>》</a>
            </div>
            <?php } ?>
            <div class="error_txt hide" id="error_txt"></div>
            <div class="login-entry-bar">
                <a href="javascript:;" class="login-bar-btn disabled" id="registerBarBtn"><?php echo __('注册');?></a>
            </div>
        </div>
    </div>
</div>
...
<div class="page out" id="registerAgreement">
    <header>
        <div class="header_top">
            ...
            <h1 class="page-title-bar"><?php echo $GLOBALS['cfg_member_agreement_title'];?></h1>
        </div>
    </header>
    <!-- 公用顶部 -->
    <div class="page-content">
        <div class="bk-document-page">
            <div class="bk-content-wrap">
                <?php echo Common::content_image_width($GLOBALS['cfg_member_agreement'],540,0);?>
            </div>
        </div>
    </div>
</div>
    ...
    $(document).ready(function () {
        ...
        $('#form-mobile').validate({
            ...
            messages: {
                user: {
                    ...
                    remote:'<?php echo __("error_user_exists");?>'
                },
                msg: '<?php echo __("account_sms_code_required");?>',
                code:{
        ...
        $('#form-email').validate({
            ...
            messages: {
                ...
                code:{
                    required: '<?php echo __("account_code_required");?>',
                    remote:'<?php echo __("account_code_error");?>'
                },
                pwd: {
                    required: '<?php echo __("account_pwd_required");?>',
                    minlength: '<?php echo __("account_pwd_min_length");?>'
            ...
            if(node.valid()){
                ...
                $.post(SITEURL+'member/register/ajax_reg',data,function(rs){
                    if(parseInt(rs.status)<1){
                        ...
                        layer.open({
                            content: '<?php echo __("account_register_success");?>',
                            skin: 'msg',
                            time: 2,
                            end:function(){
