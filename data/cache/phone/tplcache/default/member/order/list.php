    ...
    <div class="order-content">
        <div class="tab-bar">
            <ul class="tab-list clearfix">
                <li <?php if($type==-1) { ?>class="on"<?php } ?>
 data="-1">全部</li>
                <li <?php if($type==0) { ?>class="on"<?php } ?>
 data="0">待付款</li>
                <li <?php if($type==1) { ?>class="on"<?php } ?>
 data="1">待消费</li>
                <li <?php if($type==2) { ?>class="on"<?php } ?>
 data="2">待点评</li>
                <li <?php if($type==3) { ?>class="on"<?php } ?>
 data="3">退款</li>
            </ul>
        </div>
...
<script type="text/html" id="tpl_list_item">
    ...
    <li>
        ...
        <div class="item-jg">
            ...
            {{else if value.paytype=="全款支付"}}
            <span class="num fl">总额：<em><?php echo Currency_Tool::symbol();?>{{value.totalprice}}</em></span>
            <span class="num fr">全额支付：<em><?php echo Currency_Tool::symbol();?>{{if value.price}}{{value.price}}{{else}}0{{/if}}</em></span>
            {{else}}
            <span class="num fl">总额：<em><?php echo Currency_Tool::symbol();?>{{value.totalprice}}</em></span>
            <span class="num fr">定金支付：<em><?php echo Currency_Tool::symbol();?>{{value.price}}</em></span>
            {{/if}}
        </div>
