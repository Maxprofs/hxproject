...
<head>
    ...
    <?php echo Common::js('jquery.min.js,lib-flexible.js,swiper.min.js');?>
    <?php echo Common::css_plugin('line.css','line');?>
</head>
<body>
<?php echo Request::factory("pub/header_new/typeid/$typeid/islistpage/1/definetitle/".urlencode($header_title))->execute()->body(); ?>
<!-- 公用顶部 -->
...
<?php echo Request::factory("line/searchnav")->execute()->body(); ?>
...
<script>
    ...
    function on_selected_search(params)
    {
        ...
        url+=destpy+'-'+dayid+'-'+priceid+'-'+sorttype+'-0-'+startcityid+'-'+attrid+'-'+page;
    ...
    $(document).ready(function(){
        ...
        function get_line_list(page)
        {
            ...
            params['page']=page;
            var url=SITEURL+'line/ajax_line_more/';
            url+=params['destpy']+'-'+params['dayid']+'-'+params['priceid']+'-'+params['sorttype']+'-0-'+params['startcityid']+'-'+params['attrid']+'-'+params['page'];
                ...
                success:function(data) {
                    is_getting=false;
                     ...
                     for(var i in data.list) {
                         ...
                         html += '<div class="info">';
                         if(row.cfg_icon_rule==1)
                         {
                             html += '<h4 class="bt">' + row['title']+'</h4><p class="attr">';
                             if (row['iconlist'] && row['iconlist'].length > 0)
                             {
                                 for(var j in row['iconlist'])
                                 {
                                     html += '<em class="bor1 no-style">'+row['iconlist'][j]['kind']+'</em>';
                                 }
                             }
                             html += '</p>';
                         }
                         else
                         {
                             ...
                             if (row['iconlist'] && row['iconlist'].length > 0)
                             {
                                 for(var j in row['iconlist'])
                                 {
                                     icon += '<img class="icon" src="'+row['iconlist'][j]['litpic']+'"/>';
                                 }
                             }
                             html += '<h4 class="bt">' + row['title'] + icon+'</h4>';
                         }
