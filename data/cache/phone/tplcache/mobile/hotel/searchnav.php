...
<div id="filter-page" class="filter-page first-floor">
    ...
    <div class="filter-item">
        ...
        <div class="bd">
            <!--天数-->
            <ul id="sub_rankid">
                <li data-id="0" selfield="rankid" class="active">全部<i class="on"></i></li>
                <?php $hotel_tag = new Taglib_Hotel();if (method_exists($hotel_tag, 'rank_list')) {$data = $hotel_tag->rank_list(array('action'=>'rank_list',));}?>
                <?php $n=1; if(is_array($data)) { foreach($data as $r) { ?>
                <li data-id="<?php echo $r['id'];?>" selfield="rankid"><?php echo $r['title'];?><i class="on"></i></li>
                <?php $n++;}unset($n); } ?>
            ...
            <ul id="sub_priceid">
                ...
                <?php $hotel_tag = new Taglib_Hotel();if (method_exists($hotel_tag, 'price_list')) {$data = $hotel_tag->price_list(array('action'=>'price_list',));}?>
                <?php $n=1; if(is_array($data)) { foreach($data as $r) { ?>
...
<script>
    $(document).ready(function(){
        ...
        function get_next_dests(pinyin,isparent)
        {
            ...
            $.ajax({
                url:SITEURL+'hotel/ajax_get_next_dests',
                type:'POST',
                data:{
                    destpy:pinyin,
                    typeid:typeid,
