    ...
    <div class="filter-item">
        <div class="hd">
            <ul>
                ...
                <?php $n++;}unset($n); } ?>
                <li data-sub="priceid">租车价格</li>
            </ul>
        </div>
        <div class="bd">
            <?php $car_tag = new Taglib_Car();if (method_exists($car_tag, 'kind_list')) {$data = $car_tag->kind_list(array('action'=>'kind_list',));}?>
            <ul id="sub_kindid">
                <li data-id="0" selfield="kindid" class="active">全部<i class="on"></i></li>
                <?php $n=1; if(is_array($data)) { foreach($data as $r) { ?>
                   <li data-id="<?php echo $r['id'];?>" selfield="kindid"><?php echo $r['title'];?><i class="on"></i></li>
                <?php $n++;}unset($n); } ?>
            ...
            <ul id="sub_priceid">
                ...
                <?php $car_tag = new Taglib_Car();if (method_exists($car_tag, 'price_list')) {$data = $car_tag->price_list(array('action'=>'price_list',));}?>
                <?php $n=1; if(is_array($data)) { foreach($data as $r) { ?>
            ...
            <?php $n=1; if(is_array($p_attrlist)) { foreach($p_attrlist as $pattr) { ?>
                 <ul id="sub_attrid_<?php echo $pattr['id'];?>" class="sub_attrid">
                    ...
                    <?php require_once ("E:/wamp64/www/phone/taglib/attr.php");$attr_tag = new Taglib_Attr();if (method_exists($attr_tag, 'query')) {$attrlist = $attr_tag->query(array('action'=>'query','flag'=>'childitem','typeid'=>$typeid,'groupid'=>$pattr['id'],'return'=>'attrlist',));}?>
...
<script>
    $(document).ready(function(){
        ...
        function get_next_dests(pinyin,isparent)
        {
            ...
            $.ajax({
                url:SITEURL+'car/ajax_get_next_dests',
                type:'POST',
                data:{
                    destpy:pinyin,
                    typeid:typeid,
