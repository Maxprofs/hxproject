<?php if($GLOBALS['cfg_write_tourer']==1) { ?> <div class="booking-info-block"> <div class="bib-hd-bar"> <span class="col-title">旅客信息</span> </div> <div class="bib-bd-wrap"> <?php require_once ("E:/wamp64/www/taglib/member.php");$member_tag = new Taglib_Member();if (method_exists($member_tag, 'linkman')) {$tourerlist = $member_tag->linkman(array('action'=>'linkman','memberid'=>$userInfo['mid'],'return'=>'tourerlist',));}?> <?php if(!empty($userInfo) && !empty($tourerlist['0']['linkman'])) { ?> <div class="bib-select-linkman"> <div class="bib-select-bar">选择常用旅客</div> <div class="bib-select-bd"> <div class="bib-select-box"> <?php $n=1; if(is_array($tourerlist)) { foreach($tourerlist as $row) { ?> <span class="check-item choose-linkman" data-linkman="<?php echo $row['linkman'];?>" data-cardtype="<?php echo $row['cardtype'];?>"
                               data-idcard="<?php echo $row['idcard'];?>" data-sex="<?php echo $row['sex'];?>" data-mobile="<?php echo $row['mobile'];?>"><i class="check-icon"></i><?php echo $row['linkman'];?></span> <?php $n++;}unset($n); } ?> </div> <?php if(count($tourerlist)>5) { ?> <a class="bib-check-more down" href="javascript:;">更多</a> <?php } ?> </div> </div> <?php } ?> <div class="visitor-msg" id="tourer_list"> </div> </div> </div> <script>
   $(function(){
       //展开更多
