<!doctype html> <html> <head> <meta charset="utf-8"> <?php echo  Stourweb_View::template("pub/varname");  ?> <?php echo Common::css('base.css,user_new.css',false);?> <?php echo Common::css_plugin('update-add.css','line');?> <?php echo Common::load_skin();?> <?php echo Common::js('jquery.min.js,base.js');?> <style>
        .submit-btn {
    background: #fb4734;
    color: #fff;
...
$minuscoupon=$supplier_total_price-$info['iscoupon']['cmoney'];
 ?> </p> <p style="font-size: 8px;">(结算金额:<?php echo Currency_Tool::symbol();?><?php echo $minuscoupon;?>元)</p> <?php } ?> </div> </div> <div class="btn clearfix"> <?php if($info['distributor']!=$mid) { ?> <!-- 门市或游客自身订单 --> <!--待确认--> <?php if($info['status']==0) { ?> <a id="cancel-order-Click" class="cancel-btn" href="javascript:void(0)">取消订单</a> <?php } ?> <!--待付款--> <?php if($info['status']==1) { ?> <a id="cancel-order-Click" class="cancel-btn" href="javascript:void(0)">取消订单</a> <a class="pay-btn" href="javascript:void(0)">立即付款</a> <?php } ?> <!--待消费--> <?php if($info['status']==2) { ?> <a id="apply-refund-Click" class="refund-btn" href="javascript:void(0)">申请退款</a> <a class="confirm-btn" href="javascript:void(0)">确认消费</a> <?php } ?> <!--已完成--> <?php if($info['status']==5 && $info['ispinlun']=='0') { ?> <a class="comment-btn pl-btn" href="javascript:void(0)">立即点评</a> <?php } ?> <!--待审核--> <?php if($info['status']==6) { ?> <a id="cancel-refund-Click" class="cancel-refund-btn" href="javascript:void(0)">取消退款</a> <?php } ?> <!-- 门市或游客自身订单 --> <?php } else { ?> <!-- 下属游客订单 --> <!--待付款--> <?php if($info['status']==0 && $info['dconfirm']==0) { ?> <a id="submitclick" class="submit-btn" href="javascript:void(0)">提交订单</a> <a id="modifyprice" class="comment-btn" href="javascript:void(0)">修改价格</a> <input type="text" id="modify" class="modifyinput" placeholder="不能低于<?php echo Currency_Tool::symbol();?><?php echo $supplier_total_price;?>元" oninput = "value=value.replace(/[^\d]/g,'')"> <?php } ?> <!-- 下属游客订单结束 --> <?php } ?> </div> </div> </div> <div class="info-item"> <div class="order-speed-box"> <?php if($info['status']<6 && $info['status']!=4 && $info['status']!=3) { ?> <div class="order-speed-step"> <ul class="clearfix"> <li class="step-first cur"> <em></em> <strong></strong> <span><?php echo __('提订单');?></span> </li> <li class="step-second <?php if($info['status']>0) { ?>cur<?php } else if($info['status']==0) { ?>active<?php } ?>
