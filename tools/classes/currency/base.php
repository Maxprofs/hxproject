...
abstract class Currency_Base
{
     ...
     */
     public function get_exchange($currencyCode,$money,$ispositive=true,$precise=false)
     {
         ...
         {
             //old:$tempExchange = round(substr(round($exchange,$precise+1),0,-1),$precise);
             $tempExchange=floor($exchange*pow(10,$precise))*pow(0.1,$precise);
         }
         $tempExchange=Number_format($tempExchange, $precise, '.','');
         return $tempExchange;
     }
    /**
