 ...
 */
class St_Functions {
	public function st_getcityname($id) {
		$sql = "SELECT cityname from `sline_startplace` where id=" . $id . " and isopen=1 and pid!=0";
	 ...
	 * 设置全局出发地
	 */
	static function st_setStartCity() {
		...
		$baiduAK = '7e7g13BUILnuicmdpDnsW1kfRGxfBd7X';
		//初始化
		$curl = curl_init();
		//设置抓取的url
		$data = json_decode(file_get_contents('http://api.map.baidu.com/location/ip?ip=' . $ip . '&ak=' . $baiduAK));
		foreach ($data as $key => $value) {
			if ($key == 'address') {
				$cityname = explode("|", $value);
			}
		}
		$sql = "SELECT id FROM `sline_startplace` WHERE isopen=1 and cityname='" . $cityname[2] . "'";
		$cityid = DB::query(1, $sql)->execute()->current()['id'];
		if ($cityid == '') {
			return array('name' => '成都', 'id' => 2);
		}
		return array('name' => $cityname[2], 'id' => $cityid);
	...
	*/
	...
	static function st_platform() {
		//平台判断
		$version = St_Client::is_mobile() ? 'mobile_5.0' : 'pc_5.0';
		return $version;
	}
	...
	static function head_bottom() {
		...
		case 'mobile_5.0':
			...
			$web_url = self::get_main_host();
			$mobile_url = DB::select('value')->from('sysconfig')->where('varname', '=', 'cfg_m_main_url')->execute()->get('value');
	...
	static function third_bind($post) {
		...
		//对三方头像地址http和https头像地址匹配替换为//
		$data['litpic'] = preg_replace("/^(http:\/\/|https:\/\/)/", "//", $third['litpic'], 1);
	...
	*/
	...
	public static function img($src, $width = '', $height = '', $watermark) {
		...
		if (preg_match('/\.[gif]/i', $src)) {
			$full_file = BASEPATH . $src;
			if (file_exists($full_file)) {
				$fp = fopen($full_file, 'rb');
				$filecontent = fread($fp, filesize($full_file));
				fclose($fp);
				//判断是否是动态效果的gif
				if (strpos($filecontent, chr(0x21) . chr(0xff) . chr(0x0b) . 'NETSCAPE2.0')) {
					return $GLOBALS['cfg_m_img_url'] . $src;
				}
			}
		}
		if (!preg_match('~_\d+x\d+\.(jpg|jpeg|png|gif)~', $src)) {
			...
			if (preg_match('~^https?://|//~', $src)) {
				...
				*/
				...
				// $src = $GLOBALS['cfg_m_main_url'] . $src;
			}
			if ($width != '' || $height != '') {
				$width = (int) $width;
				$height = (int) $height;
				...
				case 'qiniu':
					...
					break;
				case 'default':
					if ($GLOBALS['cfg_img_param'] === '1') {
						$src = preg_replace('/(\.(?:jpg|jpeg|png|gif))$/is', "$1?" . ($watermark ? "mark=1" : '') . ($watermark ? "&w=" . $width : "w=" . $width) . ($width ? "&h=" . $height : "h=" . $height), $src);
					} else {
	...
	public static function get_satisfy($typeid, $articleid, $satisfyscore, $param = array()) {
		...
		if ($satisfyscore) {
			...
			$number = $number <= 0 || empty($number) ? 1 : $number;
...
*/
	...
	/**
	 * @function 获取订单通知管理员的账号
	 * @param $orderinfo 订单号
	 * @param $type 通知类型 email phone
	 */
	public static function get_administrator_notice_account($orderinfo, $type = 'phone') {

		$typeid = $orderinfo['typeid'];
		if (!$typeid) {
			return false;
		}
		$notice_arr = DB::select(DB::expr("$type as value"))->from('admin')
			->where("find_in_set($typeid,notice_products)")
			->execute()->as_array();
		$account = array();
		foreach ($notice_arr as $notice) {
			if ($notice['value']) {
				$account[] = $notice['value'];
			}
		}
		if ($account) {
	...
	/**
	 * @function 返回手机端的根访问路径，如果手机端域名与PC端一致则返回带/phone路径的url,否则，返回同PC端相同域名， 例如 www.lvyou.com/phone
	 * @return string
	 */
	public static function get_mobile_url() {
		$mobile_weburl = DB::select('value')->from('sysconfig')->where('varname', '=', 'cfg_m_main_url')->execute()->get('value');
		$pc_weburl = DB::select('weburl')->from('weblist')->where('webid', '=', '0')->execute()->get('weburl');

		$mobile_weburl = empty($mobile_weburl) ? $pc_weburl : $mobile_weburl;
		$protocol = St_Client::get_http_protocol();

		$mobile_weburl = strpos($mobile_weburl, 'http') === false ? $protocol . $mobile_weburl : $mobile_weburl;
		$pc_weburl = strpos($pc_weburl, 'http') === false ? $protocol . $pc_weburl : $pc_weburl;

		if ($mobile_weburl != $pc_weburl) {
			return $mobile_weburl;
		} else {
			return trim($mobile_weburl, '/\\') . '/phone';
		}
	}
	 ...
	 * @return 图片路径
	 */
	public static function local_upload_picture($filedata) {
