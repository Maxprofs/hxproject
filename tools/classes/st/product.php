...
class St_Product
{
    ...
    public static function add_order($arr, $productModel, $params)
    {
        ...
        if (is_array($arr))
        {
            ...
            if (empty($arr['memberid']))
            {
                ...
                if(!$member&&Model_Sysconfig::get_configs(0,'cfg_login_order',1)==1)
                {
                    Request::$current->redirect('error/tips?msg=order_addOrderFailureMustLogin');
                    exit;
                }
                $arr['memberid'] = $member?$member['mid'] : self::auto_reg($arr['linktel']);
            }
            ...
            try
            {
                ...
                //线路户外二次确认流程调整,不走此逻辑
                if(!in_array($productModel,array('Model_Line','Model_Outdoor','Model_Spot')))
                {
    ...
    /*
     * 生成订单编号
     * 新的生成规则:产品类型(3位)+日期时间戳(12位)+毫秒截取(3位)+随机数(2位) = 20位订单号
     *
     * */
    public static function get_ordersn($kind)
    {
        ...
        while ($max_cycle > 0)
        {
            $order_sn = str_pad($kind, 3, 0, STR_PAD_LEFT) .date('ymdHis') .substr(microtime(), 2, 3) .sprintf('%02d', rand(0, 99));
            if (!DB::select('ordersn')->from('member_order')->where('ordersn', '=', $order_sn)->execute()->current())
            {
                break;
            }
            $max_cycle--;
    ...
    public static function calculate_jifenbook($jifenid, $typeid, $ordersn)
    {
        ...
        if ($jifenbook_info['rewardway'] == 1)
        {
            ...
            $jifenbook = floor($order_info['payprice'] * $jifenbook_info['value'] / 100);
        }
        else
        {
     ...
     * @return bool|string
     */
    public static function get_product_model($type_id)
    {
        $table_name = DB::select('maintable')->from('model')->where('id','=',$type_id)->execute()->get('maintable');
        if(empty($table_name))
            return '';
        //是否包含下划线
        if(strpos($table_name,'_')!== false)
        {
            ...
            $product_model =  'Model_'.implode('_',$ar);
        }
        else
        {
            $product_model = 'Model_'.ucfirst($table_name);
        }
