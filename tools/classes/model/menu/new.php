...
class Model_Menu_New extends ORM
{
        ...
        if (!$data || !is_array($data))
        {
            ...
            foreach ($result as $v)
            {
                ...
                else
                {
                    ...
                    $v['url'] = ltrim($v['url'], '/');
                    //3级菜单
                    if ($v['level'] == 3&&$v['typeid'])
                    {
                        $nav_info = self::get_nav_title($v['typeid']);
                        $typename = !empty($nav_info['shortname']) ? $nav_info['shortname'] : $nav_info['title'];
    ...
    public static function ordey_by_nav($menu)
    {
        ...
        $cache_menu = $cache->get('back_menu_cache_'.$user_id,array());
        if(empty($cache_menu))
        {
            foreach($menu as $k =>$v)
            {
            ...
            $cache->set('back_menu_cache_'.$user_id,$new_list);
            return $new_list;
        }
        else
        {
     ...
     * @param $menu
     */
    public static function get_true_nav_title($menu)
    {

        if($menu['typeid']&&$menu['pid']==1)
        {
            $rs  = DB::select('shortname')
                ->from('nav')
                ->where('typeid','=',$menu['typeid'])
                ->execute()
                ->current();
            $menu['title'] = !empty($rs['shortname']) ? $rs['shortname'] : $rs['title'];
        }
        return $menu;
