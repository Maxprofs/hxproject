...
class Model_Tongyong extends ORM
{
    ...
    public static function gen_seotitle($param)
    {
        ...
        if (!empty($param['channel_name']) && count($main_items) < 1)
        {
    ...
    /**
     * @function 通用产品tagword获取
     * @param $typeid , 模型id
     * @param $pinyin , 目的地拼音
     * @return string
     */
    public static function get_list_tag_word($typeid, $pinyin)
    {
        if (!$typeid || $pinyin == '' || $pinyin == 'all')
        {
            return '';
        }
        ...
        if ($dest_id)
        {
            ...
            if ($module)
            {
                $tagword = DB::select('tagword')
                    ->from($module . '_kindlist')
