...
class Model_Member_Order_Compute {
	...
	public static function add($ordersn) {
		...
		$data = array(
			...
			'suppliertotalprice' => $info['suppliertotalprice'],//供应商结算总价
		);
	...
	public static function update($ordersn) {
		if ($ordersn) {
            ...
            $supplier_totalprice=$info['suppliertotalprice']+$sub_order_info['total_supplice_sub_price'];
			if (empty($info['supplierlist'])) {
				//平台自营，平台分佣为所有订单价
				$platform_commission = $total_price;
			} else {
			...
			$data = array(
                ...
                'suppliertotalprice'=>$supplier_totalprice,
			);
	...
	public static function caculate_platform_commission($order) {
		...
		$total_price = $order['suppliertotalprice'];
		//按比例返佣
		if ($product_commission_type == 1) {

			$product_commission_cash = round($product_commission_ratio * $total_price / 100, 2);
		}
		//按现金返佣
	...
	public static function get_income_info($order_id) {
		...
		if ($info) {
			//供应商收益 = 订单结算总额-平台佣金-附加总额
			$out['supplier_income'] = $info['suppliertotalprice']- $info['privileg_price'] - $info['platform_commission'] - $info['additional_total_price'];
            
			if ($out['supplier_income'] < 0) {
				$out['supplier_income'] = 0;
			}
			//平台收益 = 平台佣金-优惠金额+附加产品金额
			$out['platform_income'] = $info['platform_commission']  + $info['additional_total_price'];
            ...
            $out['privileg_price']=$info['privileg_price'];
		}
		return $out;
	...
	private static function get_sub_info($pid) {
		...
		if ($sub_orders) {
			foreach ($sub_orders as $order) {
                ...
                $sub_supplier_totalprice+=Model_Member_Order::order_supplier_total_price($order['ordersn']);
			}
		}
		return array(
            ...
            'total_supplice_sub_price'=>$sub_supplier_totalprice,
		);
	}
