...
Class Model_Member_Order extends ORM {
    ...
    */
	...
	/**
	 * @function 通过ordersn获取供应商结算总金额
	 * @param $ordersn
	 * @return mixed
	 */
	public static function order_supplier_total_price($ordersn) {
		// sline_car_suit_price,sline_line_suit_price,sline_ship_line_suit_price,sline_hotel_room_price,sline_spot_ticket_price,sline_visa(签证都还没有设置结算和利润，所以需要先改表才能求结算价）,定制单需要再研究
		// $typeidArr = array(
		// 	'1' => 'line',
		// 	'2' => 'hotel',
		// 	'3' => 'car',
		...
		$usedate = strtotime($order['usedate']); //使用时间
		$productautoid = $order['productautoid']; //产品ID
		$suitid = $order['suitid']; //套餐ID
		$typeid = $order['typeid']; //产品类型
		$orderid= $order['id'];
		$w = "orderid={$orderid}";
		$sql = "select * from {$h}member_order_extend where $w";
		$r = DB::query(Database::SELECT, $sql)->execute()->current();
		switch ($typeid) {
		case '1':
			$adult = array('basicprice' => '', 'num' => $order['dingnum']); //成年人结算价、数量
			$child = array('basicprice' => '', 'num' => $order['childnum']); //小孩结算价、数量
			$old = array('basicprice' => '', 'num' => $order['oldnum']); //老人结算价、数量
			$roombalance = array('roombalance' => $order['roombalance'], 'num' => $order['roombalancenum']); //单房差结算价、数量
			$roombalance_paytype = $order['roombalance_paytype']; //单房差支付方式1,预定;2,到店;
			$adult['basicprice'] = $r['basicprice'];
			$child['basicprice'] = $r['childbasicprice'];
			$old['basicprice'] = $r['oldbasicprice'];
			$total = $adult['basicprice'] * $adult['num'] + $child['basicprice'] * $child['num'] + $old['basicprice'] * $old['num'];
			if ($roombalance_paytype == 1 && $roombalance['num'] && $roombalance['roombalance']) {
				$total += $roombalance['roombalance'] * intval($roombalance['num']);
			}
			break;
		case '2':
			break;
		...
		// if(St_Functions::is_normal_app_install('coupon'))
		//  {
		//      $iscoupon = Model_Coupon::order_view($ordersn);
	 ...
	 * @return array
	 */
	public static function order_list($typeid, $mid, $ordertype, $currentpage, $pagesize = '10', $linktel = '', $did) {
