            ...
            if ($app['isBuy'] === 1)
            {
                foreach ($app_installed_result as $app_installed_item)
                {
                    if (strtolower($app['number']) == strtolower($app_installed_item['number']))
                    {
                        ...
                        $partinfo_list = Model_Upgrade3::get_system_part_list(array($app["id"]));
                        if (is_array($partinfo_list[$app["id"]]))
                        {
                            $app['version'] = $partinfo_list[$app["id"]]["cVersion"];
                        }
            ...
            } else
            {
                ...
                for ($index = 0; $index < count($templet_installed_result); $index++)
                {
                    ...
                    if (strtolower($templet['update_ID']) == strtolower($templet_installed_item['number']))
                    {
                        ...
                        $partinfo_list = Model_Upgrade3::get_system_part_list(array($templet["id"]));
                        if (is_array($partinfo_list[$templet["id"]]))
                        {
                            $templet['version'] = $partinfo_list[$templet["id"]]["cVersion"];
                        }
