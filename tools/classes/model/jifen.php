...
class Model_Jifen extends ORM
{
   ...
   // public static $field_section_names = array('0'=>'其他策略','1'=>'产品预订','2'=>'产品评论','3'=>'产品发布');
    ...
    /**
     * @function 获取指定积分策略是否开启
     * @param        $section
     * @param string $label
     * @param int    $issystem
     * @return mixed
     */
    public static function check_policy_is_open($section = '', $label = '', $issystem = 1)
    {
        $query = DB::select(DB::expr('COUNT(`id`) AS `num`'))
                   ->from('jifen')
                   ->where('isopen', '=', 1)
                   ->and_where('issystem', '=', $issystem);
        if ($section)
        {
            $query->and_where('section', '=', $section);
        }
        if ($label)
        {
            $query->and_where('label', '=', $label);
        }
