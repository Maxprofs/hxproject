...
class Model_Model extends ORM
{
    ...
    {
        $sql = "SELECT * FROM sline_model ";
        $sql .= "WHERE issystem=0 and maintable='model_archive' and isopen=1";
    ...
    public static function get_search_model()
    {
        ...
        $sql.= "WHERE a.isopen=1 AND a.webid=0 AND a.linktype=1 and b.isopen=1 ";
    ...
    public static function get_wap_search_model()
    {
        ...
        $sql.= "WHERE c.m_isopen=1 AND a.webid=0 AND a.linktype and b.isopen=1 ";
        $sql.= "AND a.typeid not in(7,9,10,12,14,105) ";
    ...
    /**
     * 支付前检测订单是否在最大可预订时间范围内
     */
    public static function check_usedate($orderinfo)
    {
        $typeid = $orderinfo['typeid'];
        $productaid = $orderinfo['productaid'];
        $suitid = $orderinfo['suitid'];
        $usedate_time = strtotime($orderinfo['usedate']) + 24*3600 -1;
        $array = array(5,1,114);
        ...
        switch ($typeid) 
        {
            // 景点门票订单检测
            case 5 :
                $ticket_sql = "SELECT day_before,hour_before,minute_before FROM sline_spot_ticket WHERE id = ".$suitid;
                $ticketinfo = DB::query(Database::SELECT, $ticket_sql)->execute()->as_array();
                if ($ticketinfo[0]['hour_before'] != 0 || $ticketinfo[0]['minute_before'] != 0) 
                {
                    $lpreset_time = $ticketinfo[0]['day_before'] * 3600 * 24 + (24 - $ticketinfo[0]['hour_before']) * 3600 + $ticketinfo[0]['minute_before'] * 60;
                } 
                else
                {
                    $lpreset_time = $ticketinfo[0]['day_before'] * 3600 * 24;
                }
                if ($lpreset_time == 0 && time() > $usedate_time) {
                    return false;
                }
                $last_preset_time = $usedate_time - $lpreset_time;
                // 当前支付时间大于了最后的限定时间
                if (time() >= $last_preset_time) {
                    return false;
                }
                return true;
                break;
    ...
    public static function createModel($arr)
    {
        ...
        if ($typeid)
        {
            ...
            self::createRightModule($typeid,$modulename);
     ...
     * @param $typeid
     */
    public function createRightModule($typeid,$modulename)
    {
        $arr = array(
        ...
        $arr = array(
            array('modulename' => "相关{$modulename}", 'body' => "/v5/views/default/tongyong/block/{$typeid}/destrelated.php"),
            array('modulename' => "热门{$modulename}", 'body' => "/v5/views/default/tongyong/block/{$typeid}/hot.php"),
            array('modulename' => "最新{$modulename}", 'body' => "/v5/views/default/tongyong/block/{$typeid}/newest.php"),
            array('modulename' => "猜你喜欢", 'body' => "/v5/views/default/tongyong/block/{$typeid}/tagwordrelated.php")
        );

        foreach ($arr as $row)
        {
            $aid = Common::getLastAid('sline_module_list', 0);
            $sql = "insert into sline_module_list(aid,webid,modulename,body,issystem,type,version)";
            $sql .= "values({$aid},0,'{$row['modulename']}','{$row['body']}',1,{$typeid},5)";
            DB::query(2, $sql)->execute();
        }
    }
    ...
    public function deleteRightModule($typeid)
    {
        ...
        $sql = "delete from sline_module_list where webid=0 and type={$typeid}";
        DB::query(2, $sql)->execute();
    }
    ...
    public function createTyFor5($typeid, $pinyin, $modulename)
    {
        ...
        $tongyong_block = array(
            array(
                "dest_path" => BASEPATH . '/v5/views/default/tongyong/block/' . $typeid . '/destrelated.php',
                "file" => APPPATH . 'data/init/tongyongblock/destrelated.txt'
            ),
            array(
                "dest_path" => BASEPATH . '/v5/views/default/tongyong/block/' . $typeid . '/hot.php',
                "file" => APPPATH . 'data/init/tongyongblock/hot.txt'
            ),
            array(
                "dest_path" => BASEPATH . '/v5/views/default/tongyong/block/' . $typeid . '/newest.php',
                "file" => APPPATH . 'data/init/tongyongblock/newest.txt'
            ),
            array(
                "dest_path" => BASEPATH . '/v5/views/default/tongyong/block/' . $typeid . '/tagwordrelated.php',
                "file" => APPPATH . 'data/init/tongyongblock/tagwordrelated.txt'
            ),
        );
        ...
        //创建右侧模块视图文件
        foreach ($tongyong_block as $tongyong_block_item)
        {
            if (file_exists($tongyong_block_item['file']))
            {
                $content = file_get_contents($tongyong_block_item['file']);
                $content = str_replace('#pinyin#', $pinyin, $content);
                $content = str_replace('#typeid#', $typeid, $content);
                $content = str_replace('#modulename#', $modulename, $content);

                $dir = dirname($tongyong_block_item['dest_path']);
                if (!file_exists($dir))
                {
                    mkdir($dir, 0777, true);
                }
                file_put_contents($tongyong_block_item['dest_path'], $content);
            }
    ...
    public function deleteTyFor5($typeid, $pinyin, $modulename)
    {
        ...
        $dest_ControllerPath = BASEPATH . '/v5/classes/controller/' . $pinyin . '.php';
        $dest_RoutePath = BASEPATH . '/v5/tyroute.php';
        $dest_blockPath = BASEPATH . '/v5/views/default/tongyong/block/' . $typeid;
     ...
     * @return mixed
     */
    public static function get_all_pdt_models($isopen = null)
    {
        $query = DB::select('id', 'modulename', 'pinyin', 'correct')
                   ->from('model')
                   ->where('is_orderable', '=', 1);
        if (! is_null($isopen))
        {
            $query->and_where('isopen', '=', $isopen);
        }
