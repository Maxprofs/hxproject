...
class NoticeCommon
{
    ...
    const MEMBER_QUICK_LOGIN_CODE_MSGTAG = "login_msgcode";
    const MEMBER_FINDPWD_CODE_MSGTAG = "reg_findpwd";
    //第三方登录账号在会员中心第一次绑定手机或邮箱并生成密码
    const MEMBER_THIRD_BIND_NEW_MSGTAG = "bind_third_new";
    ...
    const PRODUCT_ORDER_REVIEWING_MSGTAG = "order_msg5";
    const PRODUCT_ORDER_REFUND_MSGTAG = "order_msg6";//退款
    ...
    public static function create_product_order_msgtag_summary($product_order_msgtag, $model_pinyin, $product_id = "")
    {
        ...
        if(self::PRODUCT_ORDER_REVIEWING_MSGTAG == $product_order_msgtag)
        {
            unset($msgtypelist["供应商"]);
            unset($msgtypelist["会员"]);
        }

        if (self::PRODUCT_ORDER_UNPROCESSING_MSGTAG == $product_order_msgtag)
            $result['订单未处理'] = $msgtypelist;
        if (self::PRODUCT_ORDER_REVIEWING_MSGTAG == $product_order_msgtag)
            $result['订单退款审核中'] = $msgtypelist;
        if (self::PRODUCT_ORDER_REFUND_MSGTAG == $product_order_msgtag)
            $result['订单退款成功'] = $msgtypelist;
    ...
    public static function create_member_msgtag_summary()
    {
        ...
        $result['第三方绑定'] = self::MEMBER_THIRD_BIND_NEW_MSGTAG;
    ...
    public static function summary_price_number($orderinfo)
    {
        ...
        $payAllPrice = Model_Member_Order::cur_order_total_pay_price($orderinfo['ordersn']);
        $result['payPrice'] = sprintf("%.2f", $payAllPrice);
        return $result;
    }
    ...
    public static function generate_product_order_msg_content($msgtag, $msgtype, array $order_info)
    {
        ...
        foreach ($product_order_msgtag_summary_result as $product_order_msgtag_summary_item => $product_order_msgtag_list)
        {
            foreach ($product_order_msgtag_list as $product_order_msgtag_name => $product_order_msgtag_value)
            {
                ...
                {
                    $recipient = (NoticeCommon::EMAIL_MSGTYPE == $msgtype ? St_Functions::get_administrator_notice_account($order_info,'email')  : St_Functions::get_administrator_notice_account($order_info,'phone'));
                }
                else
                {
                ...
                if (count($config_data) > 0)
                {
                    ...
                    if (!empty($config_data[0]["msg"]))
                    {
                        ...
                        $content = str_ireplace('{#PAYPRICE#}', $order_price_num_summary['payPrice'], $content);
