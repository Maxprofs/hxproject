...
class EmailService
{
    ...
    public static function send_email($maillto, $title, $content)
    {
        ...
        $cfg_mail_address = empty($smtp_config['cfg_mail_address']) ? $cfg_mail_user : $smtp_config['cfg_mail_address'];
        ...
        $smtpmailaddress = html_entity_decode($cfg_mail_address); //SMTP服务器的用户邮件地址
        $smtpuser = html_entity_decode($cfg_mail_user); //SMTP服务器的用户帐号
        $smtppass = html_entity_decode($cfg_mail_pass); //SMTP服务器的用户密码
        ...
        try
        {
            ...
            $mail->From = $smtpmailaddress; //邮件发送者email地址
            $sender = explode("@", $smtpmailaddress);
            $cfg_mail_fromname = Model_Sysconfig::get_sys_conf('value', 'cfg_mail_fromname');
            $mail->FromName = $cfg_mail_fromname ? $cfg_mail_fromname : $sender[0]; //发件人名称
   ...
   * */
    ...
    public static function get_email_config()
    {
        $sql = "select varname,value from sline_sysconfig where
         ...
         varname='cfg_mail_address' or
         varname='cfg_mail_user' or
         varname='cfg_mail_pass' or
         varname='cfg_mail_fromname')
         and webid=0";
        ...
        {
            $result[$config_data_item["varname"]] = $config_data_item['value'];
        }
