...
class Taglib_Car
{
    ...
    public static function query($params)
    {
        ...
        switch ($flag)
        {
            ...
            case 'theme':
                ...
                break;
            case 'arrvisa':
                $list = self::get_visa($row, $offset, $visatype);
                return $list;
                break;
    ...
    private static function get_line_bymdd($offset, $row, $destid)
    {
        ...
        $sql = "SELECT " . self::$basefield . " FROM `sline_car` AS a LEFT JOIN `sline_kindorderlist` b ON (a.id=b.aid and b.typeid=3 and b.classid=$destid) ";
     ...
     * @desc 读取签证类型
     */
    public static function kin($param)
    {
        $default = array(
            'row' => '5',
            'offset' => 0,
    ...
    //获取签证产品
    public static function get_visa($row,$offset,$visatype)
    {

        $sql = 'SELECT * FROM `sline_visa`  ';
        $sql .= "WHERE  ishidden=0 and webid=0 and visatype={$visatype} " ;
