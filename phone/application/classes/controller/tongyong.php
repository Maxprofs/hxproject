...
class Controller_Tongyong extends Stourweb_Controller
{
    ...
    public function action_show()
    {
        ...
        //销售数量
        $info['sellnum'] = Model_Member_Order::get_sell_num($info['id'], $this->typeid) + $info['bookcount'];
        //产品编号
    ...
    public function action_create()
    {
        ...
        if (ST_Product::add_order($arr, 'Model_Model_Archive', $arr)) {
            ...
            $use_address=intval($_POST['use_address']) ? intval($_POST['use_address']) : 0;
            if($user&&$use_address==1)
            {
                $data = array(
                    'memberid' => $user['mid'],
                    'province' => Common::remove_xss($_POST['province']),
                    'city' => Common::remove_xss($_POST['city']),
