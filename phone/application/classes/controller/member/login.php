...
class Controller_Member_Login extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        {
            $this->referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : Common::get_main_host().$this->cmsurl;
        }
        ...
        foreach ($url_array as $k=>$item)
        {
            if (stripos($this->referer, $item))//&& !stripos($this->referer, '/member/')
            {
                Cookie::set('referer', Common::get_main_host().$this->cmsurl);
            }
        }
        //取消登陆跳转
        if (isset($_GET['cancelurl']) && !empty($_GET['cancelurl']))
        {
           $url = $_GET['cancelurl'];
        }
        else
        {
            $url = Cookie::get('referer', Common::get_main_host().$this->cmsurl);
        }
        ...
        if($envelope_order_id)
        {
            ...
            $url = rtrim($baseurl,'//') .'/'.'envelope/view/params/'.base64_encode($envelope_order_id);
            Cookie::set('referer', $url);
        }
        ...
        $this->assign('url',$url);
//        $this->assign('backurl',$this->referer);
    ...
    public function action_ajax_check()
    {
        ...
        {
            $message =Model_Member_Login::login_init($member);
            $message['url'] = Cookie::get('referer', $this->cmsurl);
            $message['msg'] = __("success_login");
            $message['status'] = 1;
            #api{{
            $ucsynlogin = '';
...
HEAD;
            if(Model_Fenxiao::is_installed())
            {
                ...
                $member['head'] .= <<<FENXIAO
                <a style="position: absolute;top:10px;right:0px;font-size: 1rem;color:white;background: #FF6600;padding: 2px;padding-left:4px;padding-right:4px;border-top-left-radius: 8px;border-bottom-left-radius: 8px" href="//{$GLOBALS['main_host']}/plugins/fx_phone/">{$fx_name}</a>
FENXIAO;
            }
            if($fx && Model_Fenxiao::is_installed()){
                ...
                $member['fx_member']=1;
                $member['fx_btn']=<<<FXBTN
<a style="background:#ffc11a;color:#fff" id="share_btn" href="javascript:;">我要分销</a>
<script>
  (function()
  {
...
NAV;
        ...
        echo 'is_login(' . json_encode(array('islogin' => $bool, 'info' => $member)) . ');ST.Login.update_storage_login_data("'.($bool==1?$serectkey:'').'",'.time().');';
    ...
    public function action_ajax_out()
    {
        ...
        $this->request->redirect($this->cmsurl . 'member?login_out=1');
    ...
    public function action_ajax_login_out()
    {
        ...
        $url=$this->cmsurl . 'member?login_out=1';
...
//        $member = Common::session('member');
        ...
        {
            $member=Model_Member::get_member_info($cookie_member);
            $storeType=2;
        }
        if (!empty($member))
        {
...
NAV;
        ...
        echo 'is_login(' . json_encode(array('islogin' => $bool, 'info' => $member,'storeType'=>$storeType)) . ');';
    }
    ...
    public function action_ajax_send_code()
    {
        ...
        {
            Common::session('login_num', 1);
            $message = array('msg' => __("account_mobile_error"), 'status' => 0);
            exit(json_encode($message));
        }
        //检测用户是否存在
    ...
    //短信快速登录检查
    public function action_ajax_sms_code_login()
    {
        //验证码检查
        $code = Common::remove_xss(Arr::get($_POST, 'code'));
        if($code)
        ...
        $phone = Common::remove_xss(Arr::get($_POST, 'mobile'));
        ...
        if($return['status']==1)
        {
            ...
            $data['url'] = Cookie::get('referer', $this->cmsurl.'member');
