...
class Controller_Member_Index extends Stourweb_Controller
{
    ...
    private function caculate_num($mid)
    {
        ...
        //待点评
        $out['needcomment'] = DB::select('id')->from('member_order')->where('status','=',5)->and_where('memberid','=',$mid)->and_where('ispinlun','=',0)->and_where('typeid', '!=', 14)->execute()->count();

        //待退款
        $out['needrefunds'] = DB::select('id')->from('member_order')->where('status', '=',6)->and_where('memberid', '=', $mid)->and_where('ispinlun', '=', 0)->and_where('typeid', '!=', 14)->execute()->count();
