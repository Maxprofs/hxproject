...
class Controller_Member_Register extends Stourweb_Controller
{
    ...
    public function action_ajax_reg()
    {
        ...
        $addtime = time();
        list($insertId, $rows) = DB::insert('member', array('nickname', $userType, 'jointime', 'logintime', 'pwd', 'regtype'))->values(array($nickname, $user, $addtime, $addtime, md5($pwd), $regtype))->execute();
        if ($rows > 0)
        {
