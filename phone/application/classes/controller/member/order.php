...
class Controller_Member_Order extends Stourweb_Controller
{
    ...
    public function before()
    {
        ...
        $this->member = Model_Member_Login::check_login_info();
        ...
        if(empty($this->member)&&in_array($this->request->action(),$unlogin))
        {

        }
        else if (empty($this->member) && empty($order_query_token))
        {
    ...
    private function get_data_initialization($data)
    {
        foreach ($data as &$v)
        {
            ...
            //订单详情
            $v['url'] = Common::get_web_url(0) . "/member/order/show?id={$v['id']}";
            //支付url
            $v['payurl'] = Common::get_main_host() . "/payment/?ordersn={$v['ordersn']}";
            //评论url
            $v['commenturl'] = Common::get_web_url(0) . "/member/comment/index?id={$v['id']}";
            //取消订单
            ...
            $v['statusname'] = Model_Member_Order::get_status_name($v['status'],$v['typeid']);
            //分割订单产品名称
            ...
            $v['ticket_type_name'] = $ticket_type_name ? $ticket_type_name : '';
            //全额支付
            $v['price'] = $info['pay_price'];//$v['price'] * $v['dingnum'] + $v['childprice'] * $v['childnum'] + $v['oldprice'] * $v['oldnum'];
            $v['totalprice'] = $info['totalprice'];
