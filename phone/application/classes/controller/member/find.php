...
class Controller_Member_Find extends Stourweb_Controller
{
    ...
    public function action_ajax_reset_pwd()
    {
        $_POST = Common::remove_xss($_POST);
        $param=$_POST['query'];
        ...
        if (!empty($param) || file_exists($cache))
        {
            ...
            if (time() < $data['expired'])
            {
                ...
                if ($mid)
                {
                    ...
                    //删除验证缓存
                    unlink($cache);
                    $message = array('msg' => __("account_find_success"), 'status' => 1);
                    exit(json_encode($message));
                }
                else
                {
            ...
            {
                $message = array('msg' => __("account_find_pwd_expired"), 'status' => 0);
                exit(json_encode($message));
            }
        }
        else
    ...
    public function action_email()
    {
        ...
        $content .= "</body></html>";
        $email_params=explode('@',$member['user']);
        $this->assign('email', $member['user']);
        $this->assign('login_email_url', St_Functions::get_http_prefix() .'mail.'.$email_params[1]);
    ...
    public function action_ajax_send_code()
    {
        ...
        if (isset($_POST['code']))
        {
            ...
            $validataion->rule('user', 'not_empty');
            $validataion->rule('user', 'phone');
            if ($validataion->check())
            {
                $user = Arr::get($_POST, 'user');
                $yzm_code = Arr::get($_POST, 'code');
                if (Captcha::valid($yzm_code))
                {
                    //Session::instance()->delete('captcha_response');
                    //检查用户是否存在
                    $member = Model_Member::member_find($user);
                    if ($member)
                    {
                        ...
                        Common::session('msg_code',null);
                        $status = json_decode(St_SMSService::send_member_msg($user,NoticeCommon::MEMBER_FINDPWD_CODE_MSGTAG,"","",$sms_code));
                        if($status->Success)
                        {
                            Common::session('msg_code', $sms_code);
                            $data['msg']="我们向您的手机<span>".preg_replace('/(\d{3})\d{4}(\d{4})/','$1***$2',$member['mobile'])."</span>发送了短信验证码，请输入您接收到的验证码,进行下一步验证！";
                        }
                        else
                        {
                            $data['msg']=__("account_sms_code_send_error");
                        }
    ...
    public function action_ajax_check_sms_code()
    {
        ...
        $phone = Common::remove_xss(Arr::get($_POST, 'user'));
        //检查用户是否存在
        ...
        if (isset($_POST['msg']))
        {
            if (empty($_POST['msg']) || Common::session('msg_code') != $_POST['msg'])
            {
                Common::session('msg_code', null);
                ...
                //$type = 'mobile';
                $this->put_cache($data);
                $message = array('url' => $this->cmsurl."member/find/reset/".md5($phone), 'status' => 1);
                exit(json_encode($message));
            }
        }
    }
