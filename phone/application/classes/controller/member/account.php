...
class Controller_Member_Account extends Stourweb_Controller
{
    ...
    public function action_ajax_account_save()
    {
        ...
        if ($flag) {
            $jifen = Model_Jifen::reward_jifen('sys_member_upload_head',$this->_member['mid']);
            if(!empty($jifen))
            {
                St_Product::add_jifen_log($this->_member['mid'],"上传头像送积分{$jifen}",$jifen,2);
            }
            $status = 1;
        }
    ...
    public function action_ajax_phone_save()
    {
        ...
        if (!$mobile_member) {
            ...
            if ($flag) {
                ...
                if(!empty($data['pwd']))
                {
                    St_SMSService::send_member_msg($mobile,NoticeCommon::MEMBER_THIRD_BIND_NEW_MSGTAG,$member_info['nickname'],$mobile,"");
                }
                $is_bundle = true;
            }
        }
        ...
        {
            $account_info = $this->get_bind_account_info($data['mobile']);
            $cur_member = Model_Member::get_member_byid($this->_member['mid']);
            ...
            if($account_info['type']==2)
            {
                ...
                {
                   DB::update('member_third')->set(array('mid'=>$account_info['member']['mid'],'last_mid'=>$this->_member['mid']))->where('mid','=',$this->_member['mid'])->execute();
                }
                Model_Member::login($account_info['member']['mobile'],$account_info['member']['pwd'],true);
    ...
    //检测手机是否注册, 只有第三方登录的，且未绑定过的用户，才能绑定已有账户
    ...
    public function action_ajax_get_account_type()
    {
        $account = $_POST['account'];
        $account_info = $this->get_bind_account_info($account);
        echo $account_info['type'];
    }
        ...
        if (Upload::valid($file)) {
            if (Upload::size($file, "2M")) {
                if (Upload::type($file, array('jpg', 'png', 'gif','jpeg'))) {
                    ...
                    $arr['success'] = 'false';
                    $arr['msg'] = '类型'.$path_info['extension'].'不支持';
                }
            }
            else {
                $arr['success'] = 'false';
    ...
    private function get_bind_account_info($account)
    {
        ...
        else if($result['mid']==$this->_member['mid'])
        {
            $info['type'] = 1;
        }
        else
