...
class Controller_Pub extends Stourweb_Controller
{
    ...
    public function action_header()
    {
        ...
        $not_use_tpl = $this->params['not_use_tpl'];
    ...
    public function action_header_new()
    {
        ...
        $isplpage = $this->params['isplpage'] ? $this->params['isplpage'] : 0;
        $isbookpage = $this->params['isbookpage'] ? $this->params['isbookpage'] : 0;
        $islistpage = $this->params['islistpage'] ? $this->params['islistpage'] : 0;
        $isorder = $this->params['isorder'] ? $this->params['isorder'] : 0;
        $islinkman = $this->params['islinkman'] ? $this->params['islinkman'] : 0;
        $ispaypage = $this->params['ispaypage'] ? $this->params['ispaypage'] : 0;
        $iscommontitle = $this->params['iscommontitle'] ? $this->params['iscommontitle'] : 0;
        ...
        if ($isshowpage == 1 || $isplpage == 1 || $isbookpage == 1 || $islistpage == 1)
        {
            $showlogo = 0;
        }
        if ($isplpage == 1)
        ...
        $this->assign('backurl', $backUrl);
        $this->assign('channelname', $channelname);
        $this->assign('isordershow', $isordershow);

        $default_tpl = Common::remove_xss($this->params["default_tpl"]);
        if(empty($default_tpl))
        {
            $this->display('pub/header_new', 'header');
        }
        else
        {
            $this->display("pub/{$default_tpl}", 'header');
        }
    ...
    public function action_footer()
    {
        ...
        {
            $this->display("pub/{$default_tpl}", 'footer');
        }
    }
    ...
    public function action_ajax_article_comment_save()
    {
        ...
        //游记是否开启审核
        if ($GLOBALS['cfg_notes_pinlun_audit_open']==='0' && $typeid == 101)
        {
            $arr['isshow'] = 1;
            $need_confirm=0;
        }
        ...
        if($arr['isshow']==1)
        {
            Model_Comment::on_verified($flag[0],$typeid,$arr['memberid']);
        }
        echo json_encode(array('status' => $flag,'need_confirm'=>$need_confirm));
        exit;
    ...
    * */
     ...
     * 检查用户名是否不存在
     */
    public function action_ajax_check_username_available()
    {
        $input = Arr::get($_POST,'username');
    ...
    public function action_thirdlogin()
    {
        ...
        $refer = urlencode(urldecode($refer));
