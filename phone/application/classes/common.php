...
class Common
{
    ...
    /**
     * @function 返回指定plugin的资源链接
     * @param string 指定的plugin应用
     * @return string 资源链接
     */
    public static function plugin_res_path($app){
        $plugin_res_url = $GLOBALS['cfg_plugin_' . $app . '_public_url'];
        $http_type = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https')) ? 'https://' : 'http://';
        $full_host = $http_type . $GLOBALS['main_host'];
        $file = $full_host.'/'.ltrim($plugin_res_url, '/\\');
        return $file;
    }
   ...
   */
    ...
    public static function session($k, $v = '')
    {
        ...
        if (empty($v))
        {
            ...
            //这里针对手机端会员作一次查询判断,会员是否存在,防止后台删除了会员,但当前会员是登陆状态,session没有过期,
            //导致还可下单或者操作的问题,这个使用地方很多,暂时只在这里判断,防止漏改.
            if($k == 'member' && is_array($session))
            {
                $row = DB::select()->from('member')->where('mid','=',$session['mid'])->execute()->current();
                if(!$row)
                {
...
EOT;
    ...
    * */
    ...
    public static function get_web_url($webid)
    {
        ...
        $prefix = self::get_main_prefix();
        $url = isset($GLOBALS['cfg_phone_cmspath']{1}) ? St_Functions::get_http_prefix() . $prefix . $domain . $GLOBALS['cfg_phone_cmspath'].$suffix : $GLOBALS['cfg_basehost'] . $suffix;
        return $url;
    }
...
//跳转301
    ...
    static function payment_from($data)
    {
        ...
        $url = isset($GLOBALS['miniProgram_application'])?url::site('miniprogram/payment',true):self::get_main_host() . '/payment/';
     ...
     * 预处理
     */
    public static function before_header($controller = null, $action = null,$envelope_order_id=0)
    {
        if (!isset($GLOBALS['miniProgram_application']))
        {
            self::wx_client_login($controller, $action,$envelope_order_id);
        }
    }
     ...
     * 微信客户端登陆
     */
    public static function wx_client_login($controller = null, $action = null,$envelope_order_id)
    {
        $file = BASEPATH . '/plugins/login_wx_client/index.php';
        $current_url = self::get_current_url();
        $fast_login = !is_null($_SERVER['HTTP_REFERER']);
        $wx_client_login =   self::session('wx_client_login');
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') === false || !file_exists($file) || ($fast_login&&$wx_client_login))
        {
            return;
        }
        if (in_array($controller, array('login', 'register', 'pub', 'error', 'dz', 'thirdpay'))&&!in_array($action,array(400,500)))
        {
            return;
        }
        self::session('wx_client_login',1);
        self::session('envelope_order_id',$envelope_order_id);
    ...
    public static function head_404()
    {
        Request::current()->redirect('error/404');
        exit;
    }
}
