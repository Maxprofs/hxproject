<!doctype html>
<html>
<head head_script=yIACXC >
    <meta charset="utf-8">
    <title>{$info['title']}预订-{$webname}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    {Common::css('style-new.css,mobilebone.css,reset-style.css')}
    ...
    <div class="page-content">
                ...
                <form action="{$cmsurl}{$pinyin}/create" id="orderfrm" method="post">
                    ...
                    <div class="booking-info-block clearfix">
                        ...
                        <div class="name-block">
                            <strong class="bt no-style">产品名称</strong>
                            ...
                            <ul>
                                ...
                                <li>
                                    <a class="all" href="#choose_date" id="selectUseDate">
                                        <strong class="item-hd no-style">使用日期</strong>
                                ...
                                <li>
                                    <strong class="item-hd no-style">预订数量</strong>
                        ...
                        <div class="foo-box hide" id="suit_list">
                            <div class="tc-container">
                                <div class="tc-tit-bar"><strong class="bt no-style">选择套餐</strong><i class="close-icon" id="closeSuit"></i></div>
                                <div class="tc-wrapper">
                                    <ul>
                                        {st:tongyong action="suit" productid="$info['id']" row="100" return="suits"}
                                        {loop $suits $v}
                                        {php}if($v['maxnumber']==0){continue;}{/php}
                                        <li data-suitid = {$v['id']} {if $v['id']==$suit['id']}class="active"{/if}suit="{id:{$v['id']},suitename:'{$v['suitname']}',day:'{$v['day']}',price:{$v['price']},maxnumber:{$v['maxnumber']},'jifentprice':'{$v['jifentprice']}',paytype:'{$v['paytype']}',dingjin:'{$v['dingjin']}'}"><em
                                            class="item no-style">{$v['suitname']}</em><i class="radio-btn"></i></li>
                                        {/loop}
                                    </ul>
                                </div>
                            </div>
                    ...
                    <i class="hide" id="extParam">
                        <!--是否需要收货地址-->
                        <input type="hidden" name="needaddress" id="needAddress" value="1">
                    </i>
        ...
        <div class="bom-fixed-content">
            <div class="bom-fixed-block">
                    <span class="total">
                        <em class="jg no-style">应付总额：{Currency_Tool::symbol()}<i class="payPrice">0</i></em>
                    </span>
            ...
            <div class="fee-container">
                <div class="fee-row">
                    <p class="ze">
                        ...
                        <em class="fr no-style">{Currency_Tool::symbol()}<i class="payPrice">0</i></em>
                    </p>
                    <p class="sm"><i class="payType"></i> {Currency_Tool::symbol()}<i class="payPrice">0</i></p>
                </div>
                <ul class="mx-list">
                    <li id="suit_info" style="display: none">
                        <strong class="no-style"><span class="suitName">{$suit['suitname']}</span>（<i class="bookNumber">{$info['bookNumber']}</i>）</strong>
                        <em class="no-style">{Currency_Tool::symbol()}<i class="totalPrice">0</i> </em>
                    </li>
                </ul>
            </div>
        </div>
...
<script>
    ...
    $(function(){
        !function init() {
            ...
            var getsuitid = "{$_GET['suitid']}";
            if (!getsuitid || $("#suit_list li[data-suitid="+getsuitid+"]").length <1) {
                // 默认选中第一个
                $("#suit_list li:first").click();
            }
            else 
            {
                // 选取上个页面的值
                $("#suit_list li[data-suitid="+getsuitid+"]").click();
            }
        }();
        //预订数量
        ...
        //选择框
        $(".check-box").not("#addAddress").on("click", function () {
...
//                return false;
        ...
        var old_address_name,old_address_tel;
        $("input[name=linkman],input[name=linktel]").focus(function(){
            old_address_name=$("input[name=linkman]").val();
            old_address_tel=$("input[name=linktel]").val();
        });
        $("input[name=linkman],input[name=linktel]").blur(function(){
            if(!$("input[name=receiver]").val()||old_address_name==$("input[name=receiver]").val())
            {
                $("input[name=receiver]").val($("input[name=linkman]").val());
            }
            if(!$("input[name=phone]").val()||old_address_tel==$("input[name=phone]").val())
            {
                $("input[name=phone]").val($("input[name=linktel]").val());
            }
        });
        $("#orderfrm").validate({
            rules: {
