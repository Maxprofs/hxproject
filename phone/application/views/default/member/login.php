...
<div class="header_top bar-nav">
    ...
    <h1 class="page-title-bar">{__('登录')}</h1>
</div>
<!-- 头部 -->
...
<div class="account-container">
    ...
    <div class="account-content-box st_login">
        ...
        <div class="account-tab-bar">
            <span class="item{if $login_type!=2} on{/if}" data-type="form-submit">{__('账号密码登录')}</span>
            {if $isopen==1}
            <span class="item{if $login_type==2} on{/if}" data-type="sms-form-submit">{__('动态密码登录')}</span>
            {/if}
        </div>
        <div class="account-tab-wrapper">
            <form id="form-submit">
                <ul class="account-group-list">
                    <li class="item-bar">
                        <input class="account-text form-clear-val" id="type" type="text" name="user" placeholder="{__('手机号')}/{__('邮箱')}">
                        <i class="clear-icon hide"></i>
                    </li>
                    <li class="item-bar">
                        <input class="account-text form-clear-val" id="loginPassWord" type="password" name="pwd" placeholder="{__('登录密码')}">
                        <i class="clear-icon hide"></i>
                    </li>
                    {if !$one}
                    <li class="item-bar">
                        <input class="account-text form-clear-val" type="text" id="user_code" name="code" placeholder="{__('图形验证码')}">
            ...
            <form id="sms-form-submit">
                <ul class="account-group-list">
                    <li class="item-bar" id="account_phone_content">
                        <input class="account-text form-clear-val mobile" id="loginPhone" type="text" name="mobile" placeholder="{__('请输入手机号码')}">
                        <i class="clear-icon hide"></i>
                    </li>
                    {if !$one}
                        <li class="item-bar">
                            <input class="account-text form-clear-val" id="loginCheck" type="text" name="code" placeholder="{__('图形验证码')}">
                            <img class="pic-yzm captcha cursor" src="" />
                        </li>
                    {/if}
                    <li class="item-bar">
                        <input class="account-text form-clear-val" id="loginDynamic" type="text" name="sms_code" placeholder="{__('请输入短信验证码')}">
                        <span class="dyn-code">{__('获取动态验证码')}</span>
                    </li>
                </ul>
            </form>
            {/if}
        ...
        <div class="login-entry-bar">
            <a href="javascript:;" class="login-bar-btn" id="submit_btn">{__('登录')}</a>
        </div>

        <div class="account-jump-bar">
            <a href="{$cmsurl}member/register" class="link">{__('注册新用户')}</a>
            <a href="{$cmsurl}member/find" class="link">{__('找回密码')}</a>
        </div>
        <input type="hidden" id="send_waiting" value="{if $send_waiting}{$send_waiting}{else}0{/if}">
    </div>
...
//            $('.dyn-code').addClass('disabled');
        ...
        $('#form-submit').validate({
            ...
            messages:{
                user: {
                    required: '{__("account_user_required")}',
                },
                mobile: {
        ...
        $('#sms-form-submit').validate({
            ...
            messages:{
                ...
                sms_code:{
                    required: '{__("account_sms_code_required")}',
                },
            },
            errorElement: "p",
            errorPlacement: function(error, element) {
            ...
            ST.Login.login_sendCode_m(data,function (re) {
                if (parseInt(re.status) < 1) {
                    if(re.msg){
                        ...
                        layer.open({
                            content: '{__("account_sms_code_send_error")}',
                            skin: 'msg',
                            time: 3
                        });
                    }
                    ...
                    if(re.type&&re.msg)
                    {
                        ...
                        layer.open({
                            content: '{__("account_sms_code_send_success")}',
                            skin: 'msg',
                            time: 3
                        });
                    }
