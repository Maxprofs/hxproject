...
<div class="header_top">
    {if $showlogo==1}
    <div class="st_logo"><a href="{$GLOBALS['cfg_m_main_url']}"><img src="{Common::img($GLOBALS['cfg_m_logo'])}" height="30" /></a></div>
    {else}
        ...
        if($obj['islogin']==1){
            ...
            if($obj['info']['fx_member']){
                if(window.location.href.indexOf('/show_')!=-1)
                    $('.bom_fixed a').eq(0).after($obj['info']['fx_btn'].replace('[fx_url]',fx_url));
            }
        }
    }
