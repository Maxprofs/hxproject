...
class HttpDown
{
    ...
    function PrivateOpenHost()
    {
      //echo $this->m_host;
        ...
        $errno = "";
        $errstr = "";
        $this->m_fp = @fsockopen('ssl://'.$this->m_host, 443, $errno, $errstr,10);
    ...
    function FillUrl($surl)
    {
        ...
        if($surl[0]=="/")
        {
            $okurl = $this->m_scheme."://".$this->HomeUrl.$surl;
        }
        else if($surl[0]==".")
        {
            if(strlen($surl)<=1)
            ...
            {
                $okurl = $this->m_scheme."://".$this->BaseUrlPath."/".substr($surl,2,strlen($surl)-2);
            }
            else
            {
                $urls = explode("/",$surl);
        ...
        else
        {
            if(strlen($surl)<7)
            {
                $okurl = $this->m_scheme."://".$this->BaseUrlPath."/".$surl;
            }
        ...
        $okurl = preg_replace("/^(".$this->m_scheme.":\/\/)/i","",$okurl);
        $okurl = preg_replace("/\/{1,}/", "/", $okurl);
        return $this->m_scheme."://".$okurl;
    }
}//End Class