...
class Taglib_Tongyong
{
    ...
    public static function suit($params)
    {
        ...
        foreach ($suit as &$r)
        {
            ...
            $suitid = $r['id'];
            $sql = "select MIN(price) as minprice from sline_model_suit_price WHERE suitid='{$suitid}' and productid='$productid' and day>={$time} and number!=0";
    ...
    public static function query($params)
    {
        ...
        switch ($flag)
        {
            ...
            case 'order':
                ...
                break;
            case 'mdd':
                $list = self::get_list_bymdd($modelInfo, $offset, $row, $destid);
                break;
            case 'tagrelative':
        ...
        foreach ($list as &$v)
        {
            ...
            $v['satisfyscore'] =St_Functions::get_satisfy($typeid,$v['id'],$v['satisfyscore'],array('suffix'=>''));
        }
        return $list;
    }
    ...
    public static function get_list_new($modelInfo, $offset, $row)
    {
        ...
        $sql = "SELECT a.* FROM sline_model_archive as a " .
            ...
            $w .
            " ORDER BY a.addtime desc,ifnull(c.displayorder,9999) asc " .
            " limit {$offset},{$row}";
    ...
    public static function get_list_bymdd($modelInfo, $offset, $row, $destid)
    {
        $typeid = $modelInfo['id'];
        $addtable = $modelInfo['addtable'];
        $addtable = 'sline_' . $addtable;
        ...
        $w = " where a.ishidden=0 and a.typeid={$typeid} and find_in_set({$destid},a.kindlist) ";
        $sql = "SELECT a.* FROM sline_model_archive as a " .
            " LEFT JOIN {$addtable} b on (a.id=b.productid) " .
            " LEFT JOIN sline_kindorderlist c on (a.aid=c.aid and c.typeid={$typeid} AND c.classid={$destid})" .
            $w .
            " ORDER BY ifnull(c.displayorder,9999) asc,a.addtime desc " .
            " limit {$offset},{$row}";
        $list = DB::query(Database::SELECT, $sql)->execute()->as_array();
