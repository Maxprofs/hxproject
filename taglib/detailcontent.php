...
class Taglib_Detailcontent
{
    ...
    public static function get_outdoor_content($params)
    {
        $default = array(
            'typeid' => '',
            'productinfo' => 0,
        ...
        $arr = DB::select('columnname', 'chinesename', 'isrealfield')->from('outdoor_content')->where('isopen', '=', 1)->order_by('displayorder', 'asc')->execute()->as_array();
        ...
        foreach ($arr as $v)
        {
            ...
            {
                $lineContent = DB::query(1, "SELECT * FROM sline_outdoor_jieshao where product_id ={$productid} and `day`=1 and title!='' and title is not null")->execute()->current();
                if (!$lineContent)
                {
                    continue;
                }
            }
            if($v['columnname'] == 'content' && $productinfo['isstyle'] == 0)
            {
            ...
            $a['columnname'] = $v['columnname'];
            $a['chinesename'] = $v['chinesename'];
            if (empty($content) && $v['columnname'] != 'content')
            {
                continue;
            }
            // $content = Product::replace_strong_to_span($content);
            $a['content'] = $content;//$pc == 0 ? Product::strip_style($content) : $content; //针对PC/手机版选择是否去样式.
