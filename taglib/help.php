...
class Taglib_Help {
   ...
   */
    ...
    public static function newest($params)
    {
        $default = array(
            'row' => 6,
            'offset' => 0,
        ...
        $sql = "SELECT aid,title,body,addtime,modtime FROM `sline_help` ";
        $sql .= "WHERE webid=0 {$where} ";
        $sql .= "ORDER BY modtime desc,addtime DESC,displayorder ASC LIMIT {$offset},{$row}";
        $arr = DB::query(1, $sql)->execute()->as_array();
        foreach ($arr as &$r)
        {
            $r['url'] = Common::get_web_url(0) . '/help/show_' . $r['aid'] . '.html';
    ...
    public static function hot($params)
    {
        ...
        $sql = "SELECT aid,title,body,addtime,modtime FROM `sline_help` ";
        $sql .= "WHERE webid=0 {$where} ";
        $sql .= "ORDER BY displayorder ASC,modtime desc,addtime DESC LIMIT {$offset},{$row}";
        $arr = DB::query(1, $sql)->execute()->as_array();
        foreach ($arr as &$r)
        {
            $r['url'] = Common::get_web_url(0) . '/help/show_' . $r['aid'] . '.html';
