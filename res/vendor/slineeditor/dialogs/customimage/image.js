...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            // 判断七牛云的配置是否有url
            var imageUrl = editor.options.imageUrl;
            if (editor.options.qiniu !== '1') {
                imageUrl = editor.options.qiniu;
            }
            var _this = this,
                ...
                imageMaxSize = editor.options.imageMaxSize,
                imageCompressBorder = editor.options.imageCompressBorder;
            if (!WebUploader.Uploader.support()) {
                $('#filePickerReady').after($('<div>').html(lang.errorNotSupport)).hide();
                return;
            } else if (!editor.options.imageUrl) {
            ...
            uploader = _this.uploader = WebUploader.create({
                ...
                server: actionUrl,
                fileVal: editor.options.imageFieldName,
                duplicate: true,
                fileSingleSizeLimit: imageMaxSize,    // 默认 2 M
                compress: editor.options.imageCompressEnable ? {
        ...
        getQueueCount: function () {
            ...
            for (i = 0; file = files[i++];) {
                ...
                if (status == 'queued' || status == 'uploading' || status == 'progress'|| status == 'inited') readyFile++;
            }
            return readyFile;
        },
        destroy: function () {
