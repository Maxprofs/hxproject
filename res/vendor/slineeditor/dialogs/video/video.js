...
(function(){
    ...
    function convert_url(s){
        return s.replace(/http:\/\/www\.tudou\.com\/programs\/view\/([\w\-]+)\/?/i,"http://www.tudou.com/v/$1")
            ...
            //腾讯视频匹配
            .replace(/v\.qq\.com\/x\/cover\/(\w+)\/(\w+)\.html/i, "static.video.qq.com/TPout.swf?vid=$2")
            .replace(/v\.qq\.com\/x\/page\/(\w+)\.html/i, "static.video.qq.com/TPout.swf?vid=$1")
    ...
    function createPreviewVideo(url){
        ...
        //优酷播放地址匹配
        var youku_video=convert_url(url);
        var youkuPlay = /player\.youku\.com/ig.test(youku_video);

        //腾讯播放地址匹配
        var tx_video=convert_url(url);
        var txPlay = /static\.video\.qq\.com/ig.test(tx_video);
        if(youkuPlay){
            //优酷 flash地址匹配
            var url_arr=youku_video.split("?");//多余参数去除
            url=url_arr[0];
            ...
            //腾讯 flash地址匹配
            url=tx_video.replace(/static\.video\.qq\.com\/TPout\.swf\?vid=(\w+)/, "imgcache.qq.com/tencentvideo_v1/playerv3/TPout.swf?max_age=86400&v=20161117&vid=$1&auto=0");
            $G("preview").innerHTML = '<embed type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
                ' src="' + url + '"' +
                ' width="' + 420  + '"' +
                ' height="' + 280  + '"' +
            ...
            if (matches){
                ...
                if(endWith(url,[".mp4"])){
                    $G("preview").innerHTML = '<canvas id="stour_tmp_canvas" style="display: none;"></canvas><video id="stour_preview"' +
                        ' src="'+ url + '"' +
                        ' width="' + 420  + '"' +
                        ' height="' + 280  + '"' +
                        ' controls preload="metadata" style="object-fit:fill" x5-video-player-type="h5" webkit-playsinline playsinline ></video>';
                }else{
    ...
    function insertUpload(){
        var videoObjs=[],
            uploadDir = window.location.origin+editor.options.videoPath,
    ...
    function initUpload(){
        var settings = {
            ...
            //选择文件完成回调
            file_dialog_complete_handler:function(numFilesSelected, numFilesQueued,numFilesInQueue,file) {
            ...
            upload_start_handler:function (file) {
                ...
                $.ajax({
                    type:'POST',
                    url:'../../php/checkConfig.php',
                    data:{CheckType:'file',CheckSize:file.size},
                    dataType:'json',
                    async: false,
            ...
            upload_success_handler:function (file, serverData) {
                ...
                if(info.state=="SUCCESS"){
                    ...
                    isModifyUploadVideo = true;
                    var full_path=window.location.origin+editor.options.videoPath+info.url;
                    createPreviewVideo(full_path);
                    $G("videoUrl").value=full_path;
            ...
            upload_complete_handler:function () {
                var $video = document.getElementById("stour_preview");
                var canvas = document.getElementById("stour_tmp_canvas");
                $video.addEventListener('loadeddata',function(){
                    var w  = $G('upload_width').value || 420;
                    var h  = $G('upload_height').value || 280;
                    canvas.width=w;
                    canvas.height=h;
                    var ctx = canvas.getContext('2d');
                    // var w = $video.width;//视频DOM宽
                    // var h = $video.height;//视频DOM高
                    ctx.drawImage($video, 0, 0, w, h);
                    var _base64 = canvas.toDataURL('images/jpeg',1);
                    // $G("videoPoster").value = _base64;
                    // poster=_base64;
                    // $video.setAttribute("poster", _base64);
                    //save base64_poster2image
                    $.ajax({
                        type:'POST',
                        url:'../../php/Base64ToImgSave.php',
                        data:{imgBase64:_base64},
                        dataType:'json',
                        async: false,
                        success:function(data){
                            if(data.status){
                                poster=window.location.origin+editor.options.videoPath+data.url;
