// JavaScript Document
/*
   笛卡验证支持必填和正则两种验证方式.  dom属性填写方式,   data-required:必填   data-regrex:正则   data-msg:正则验证错误时的提示信息
*/
        ...
        require:
            {
                blur:function(ele,e,action)
                {
                    ...
                    if(!val)
                    {
                        ...
                        var width=$(ele).width();
                        $.st_hintvalidate("必填",x+width+15,y,1000);
                    }
                    else
        ...
        regrex:
            {
                keyup:function(ele,e,action)
                {
                    ...
                    if(regrex=='number')
                    {
                        ...
                        if(Number(val_max)>0)
                        {
                            val>val_max ? val = val_max : val= val;
                        }
                        $(ele).val(val);
                        ...
                        {
                            $.st_hintvalidate(msg,x+width+15,y,1000);
                            return false;
                        }
                    }
                    regrex=eval(regrex);
