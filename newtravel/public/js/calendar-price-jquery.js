...
(function ($) {
  ...
  fn._prevMonth = function () {
    ...
    //月份处理回调
     ...
     {
         this.data = calendarCallback(this.month);
  ...
  fn.renderDataToTalbe = function () {
    ...
    this.calendar.find('.valid-hook').each(function () {
      ...
      if(typeof onRenderFinish=='function')
      {
          this.data = onRenderFinish();
      }
  ...
  fn.dayComplate = function () {
    ...
    if (arr && arr instanceof Array) {
      for (var i = 0; i < arr.length; i++) {
        var val = arr[i];
        html += '<p class="item"  ><span class="attr">'+ val.name +'</span><span class="num">{'+ val.key +'}</span></p>'
      }
    }
    return html;
  };
  ...
  fn.handleClickEvent = function () {
    ...
    this.calendar.off('click','.valid-hook');
