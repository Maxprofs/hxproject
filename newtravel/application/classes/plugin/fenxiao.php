...
class Plugin_Fenxiao extends Plugin_Core_Base{
    ...
    private function pay_commission($order)
    {
        ...
        foreach($fenxiaoArr as $k=>$v) {
            if ($v) {
                ...
                if ($fenxiaoModel->loaded())
                {
                    ...
                    else if($fenxiao_data['type'.$number]==2)
                    {

                        $payAmount = $compute_info['platform_commission']*$fenxiao_data['pm_fxratio'.$number];
                    }
                    else
                    {
                        ...
                        else if($order['typeid']==104)
                        {
                            $dingnum = DB::query(Database::SELECT,"select sum(dingnum) as num from sline_member_order_child where pid={$order['id']}")->execute()->get('num');
        ...
        if($cur_fenxiao_model->status==1 && $cur_fenxiao_model->isfrozen!=1)
        {
            ...
            $payAmount=0;
            if($fenxiao_data['type']==1)
            {
                $rate = $fenxiao_data['fxratio'];
                $payAmount = floatval($consumeAmount * $rate);
            }
            else if($fenxiao_data['type']==2)
            {
                $rate = $fenxiao_data['pm_fxratio'];
                $payAmount = floatval($compute_info['platform_commission'] * $rate);

            }else
            {
                if($order['typeid']==1)
                {
                    $payAmount = floatval($fenxiao_data['cash'])*intval($order['dingnum'])+floatval($fenxiao_data['cash_old'])*intval($order['oldnum'])+floatval($fenxiao_data['cash_child'])*intval($order['childnum']);
                }
                else if($order['typeid']==104)
                {
                    $dingnum = DB::query(Database::SELECT,"select sum(dingnum) as num from sline_member_order_child where pid={$order['id']}")->execute()->get('num');
                    $payAmount = floatval($fenxiao_data['cash'])*intval($dingnum);
                }
                else
                {
                    $payAmount = floatval($fenxiao_data['cash'])*intval($order['dingnum']);
                }
            }
    ...
    //反佣
    private function pay_commission_old($order)
    {
        ...
        foreach($fenxiaoArr as $k=>$v) {
            if ($v) {
                ...
                if ($fenxiaoModel->loaded())
                {
                    ...
                    {
                        $payAmount = floatval($compute_info['platform_commission'] * $commission_info['pm_rate_arr'][$k]);
                    }
                    else
                    {
                        ...
                        {
                            $dingnum = DB::query(Database::SELECT,"select sum(dingnum) as num from sline_member_order_child where pid={$order['id']}")->execute()->get('num');
                            $payAmount = floatval($cash_info['adult'])*intval($dingnum);
                        }
                        else
                        {
        ...
        if($cur_fenxiao_model->status==1 && $cur_fenxiao_model->isfrozen!=1)
        {
            ...
            {
                $rate = array_pop($commission_info['pm_rate_arr']);
                $payAmount = floatval($compute_info['platform_commission'] *$rate);
            }
            else
            {
