...
class Common
{
    ...
    * */
    ...
    {
        global $dsql;
        $result = array('totalNumber' => 0, 'totalPrice' => 0, 'payPrice' => 0, 'numberDescript' => '', 'priceDescript' => '');
        ...
        switch ($modelId)
        {
            ...
            default:
                $lastOffer = array(
                    ...
                    'basicprice' => $data['basicprice'],
                    'profit' => $data['profit'],
                    'price' => floatval($data['basicprice']) + floatval($data['profit']),
                    'starttime' => $data['starttime'],
                    'endtime' => $data['endtime'],
                );
        }
        $lastOffer['number'] = $data['number'];
        $lastOffer['price'] = (string)$lastOffer['price'];
            ...
            if (isset($result['data']) && $result['data'])
            {
                ...
                foreach ($page as $k => $item)
                {
                    foreach ($pid as $value)
                    {
                        ...
                        if ($item['pid'] == $value)
                        {
                            array_push($level_3, array('id' => "c{$item['id']}", 'pid' => "c{$item['pid']}", 'name' => $item['kindname'], 'page_name' => $item['pagename']));
                        }
                    }
                }
