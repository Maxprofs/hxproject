...
class Controller_Finance extends Stourweb_Controller
{
    ...
    public function action_cash_config()
	{
        ...
        if(!$action)
        {
            ...
            $this->display('stourtravel/finance/cash_config');
        }
    }

    public function action_ajax_save_cash_config()
	{
        $params = array('webid'=>0);
        if(isset($_POST['cash_min']))
        {
            $params['cash_min'] = intval($_POST['cash_min']);
        }
        if(isset($_POST['cash_min_num']))
        {
            $params['cash_min_num'] = intval($_POST['cash_min_num']);
        }
        if(isset($_POST['cash_max']))
        {
            $params['cash_max'] = intval($_POST['cash_max']);
        }
        if(isset($_POST['cash_max_num']))
        {
            $params['cash_max_num'] = intval($_POST['cash_max_num']);
        }
