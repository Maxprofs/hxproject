...
class Controller_Kefu extends Stourweb_Controller{
        ...
        if($action=='read')
        {
            ...
            if($node=='root')//属性组根
            {
                ...
                $list[]=array(
                    'leaf'=>true,
                    'id'=>'0add',
                    'qqname'=>'<button class="btn btn-primary radius size-S" onclick="addSub(0)">添加</button>',
                    'displayorder'=>'add'
                );
            }
            else //子级
            {
                ...
                $list[]=array(
                    'leaf'=>true,
                    'id'=>$node.'add',
                    'qqname'=>'<button class="btn btn-primary radius size-S" onclick="addSub(\''.$node.'\')">添加</button>',
                    'displayorder'=>'add'
                );
            }
   ...
   //保存系统配置
    ...
    public function action_viewed()
    {
        ...
        if ($data)
        {
            DB::update('freekefu')->set(array('status'=>1))->where('id', 'in', DB::expr("({$data})"))->execute();
        }
        echo 'ok';
    }
