...
class Controller_Icon extends Stourweb_Controller{
  ...
  * */
    ...
    /**
     * @function 新版的弹窗。可以配置图标的显示类型
     * @throws Kohana_Exception
     */
    public function action_dialog_seticon_new()
    {
        $id=$_GET['id'];
        $typeid=$_GET['typeid'];
        $iconStr=$_GET['iconlist'];
        ...
        $this->assign('selector',$selector);
        $this->assign('id',$id);
        $config = Model_Sysconfig::get_configs(0,array('cfg_icon_rule'));
        if($config['cfg_icon_rule'])
        {
            $this->display('stourtravel/icon/dialog_seticon_new');
        }
        else
        {
            $this->display('stourtravel/icon/dialog_seticon');
     ...
     * @function 配置显示规则
     */
      public function action_cfg_rule()
      {
          $config = Model_Sysconfig::get_configs(0,array('cfg_icon_rule'));
          $this->assign('config',$config);
          $this->display('stourtravel/icon/cfg_rule');
      }
}