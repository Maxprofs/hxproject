...
class Controller_Question extends Stourweb_Controller{
    ...
    {
        $id = $this->params['id'];
        DB::update('question')->set(array('viewstatus'=>1))->where('id','=',$id)->execute();
        ...
        $info['addtime'] = Common::myDate('Y-m-d H:i:s',$info['addtime']);
    ...
    public function action_viewed()
    {
        ...
        if ($data)
        {
            DB::update('question')->set(array('viewstatus'=>1))->where('id', 'in', DB::expr("({$data})"))->execute();
        }
        echo 'ok';
    }
}