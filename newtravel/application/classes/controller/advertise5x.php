...
class Controller_Advertise5x extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        switch ($action)
        {
            ...
            case 'read':
                ...
                if ($prefix)
                {
                    list($prefix, $remark) = explode(',', $prefix);
                }
                ...
                if (!is_null($remark))
                {
                    $where .= $remark ? " and a.remark='{$remark}'" : ' and a.remark not like "%install%" ';
                }
    ...
    //页面对应模板
    public function action_ajax_page_module()
    {
        $data = $_POST['webid'] > 0 ? Model_Templet::get_templet_list_by_page('sub_site', $_POST['page'], $_POST['webid']) : Model_Templet::get_templet_list_by_page($_POST['ismobile'] ? 'wap' : 'pc', $_POST['page']);
        if (count($data) > 1)
        {
