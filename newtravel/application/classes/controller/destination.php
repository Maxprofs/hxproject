            ...
            if ($typeid == 0)
            {
                ...
                if ($field == 'iswebsite' && $val > 0)
                {
                    ...
                    if (file_exists($fileHtaccess))
                    {
                        ...
                        $hostname = preg_replace('/^https?\:\/\/' . $weburl['webprefix'] . '/is', '', $weburl['weburl']);
                        $prefix = $weburl['webprefix'];
                        $mobile_access='';
                        $mobile_site=DB::select()->from('sysconfig')->where('varname','=','cfg_m_main_url')->execute()->current();
                        if($mobile_site && $mobile_site['value'] &&($mobile_site=parse_url($mobile_site['value']))){
                            if($mobile_site['host']!=$prefix.$hostname){
                                $mobile_access="RewriteCond %{HTTP_HOST} !^{$mobile_site['host']}\$\r\n";
                            }
                        }
                        $subhtaccess = "RewriteCond %{HTTP_HOST} ^(.*)?{$hostname}\$\r\nRewriteCond %{HTTP_HOST} !^({$prefix}){$hostname}\$\r\n{$mobile_access}RewriteRule ^(lines|hotels|cars|spots|raiders|photos)(/)?\$ \\$1/%1 [L]";
    ...
    */
    ...
    //
     /*
      获取下级目的地和已设置的目的地名称,用于产品的目的地设置
     */
    public function action_ajax_getDestsetList_search()
    {
        $keyword = Arr::get($_POST, 'keyword');
        $typeid = $_POST['typeid'];
        $w = !empty($typeid) ? " and find_in_set({$typeid},opentypeids)" : '';
        if ($keyword) {
            $sql = "select id,pid,kindname,pinyin from sline_destinations where kindname like '%{$keyword}%' and isopen=1 {$w}  order by pinyin asc";
        } else {
            $sql = "select id,kindname,pinyin from sline_destinations where pid=0 and isopen=1 {$w} order by pinyin asc";
        }
        $destlist = DB::query(Database::SELECT, $sql)->execute()->as_array();
        $pid_arr = array();
        foreach ($destlist as $key => $row) {
            ...
            $r = DB::query(1, $sql)->execute()->as_array();
            $destlist[$key]['childnum'] = $r[0]['num'];
            $tem_arr = Model_Destinations::getParentsStr($row['pid']);
            $pid_arr = array_merge($pid_arr,$tem_arr);
        }
        $pid_info  = Model_Destinations::getKindlistArr(implode(',',$pid_arr));
        if($pid_info)
        {
            foreach ($pid_info as $k =>$v) 
            {
                ...
                $nv['pid'] = $v['pid'];
                $nv['pinyin'] = $v['pinyin'];
                $nv['childnum'] = $v['childnum'] ? $v['childnum'] : 0;
                $nv['kindname'] = $v['kindname'];

                $new_arr[] = $nv;
            }
        }
        $dest_parents = Model_Destinations::getParents($pid);
        echo json_encode(array('nextlist' => $destlist,'parentlist'=> $new_arr));
    }
