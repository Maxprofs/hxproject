...
class Controller_User extends Stourweb_Controller{
    ...
    public function action_list()
    {
        ...
        else if($action=='read')
        {
            ...
            foreach ($list as &$l)
            {
                if($l['notice_products'])
                {
                    $notice_title = '';
                    $notice_products = explode(',',$l['notice_products']);
                    foreach ($notice_products as $type)
                    {
                        if($type)
                        {
                            $notice_title .= DB::select('shortname')->from('nav')
                                ->where('typeid','=',$type)->execute()->get('shortname').',';
                        }

                    }
                    $l['notice_title'] = rtrim($notice_title,',');
    ...
    public function action_ajax_save()
    {
        ...
        $model->notice_products = implode(',',Arr::get($_POST,'products'));
        $model->notice_other = implode(',',$_POST['notice_other']);
        $model->save();
        if($model->saved())
        {
            $model->reload();
            $_arr=$model->as_array();
            echo json_encode(array('status'=>true,'msg'=>'保存成功','adminid'=>$_arr['id']));
        }
        else
        {
    ...
    public function action_dialog_edit()
    {
        ...
        if(!empty($id))
        {
            ...
            {
                $info=$model->as_array();
                $info['notice_products'] = explode(',',$info['notice_products']);
                $info['notice_other_arr'] = explode(',',$info['notice_other']);
                $this->assign('info',$info);
            }
        }
        $roles=ORM::factory('role')->get_all();
        $this->assign('roles',$roles);
        $this->assign('products',$this->_get_notice_product());
        $this->display('stourtravel/user/edit');
     ...
     * @function 获取可通知的产品
     */
    private function _get_notice_product()
    {
        $sql="select a.id,b.shortname from sline_model as a LEFT JOIN sline_nav as b ON a.id=b.typeid WHERE a.is_orderable=1 GROUP by a.id";
