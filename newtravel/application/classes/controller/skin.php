...
class Controller_Skin extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        //后台皮肤
        $cfg_skin_back_id = DB::select('value')->from('sysconfig')->where('varname','=','cfg_skin_back_id')->execute()->get('value');
        $skin_back_list = DB::select()->from('skin_back')->execute()->as_array();
        $this->assign('cfg_skin_back_id',$cfg_skin_back_id);
        $this->assign('skin_back_list',$skin_back_list);
    ...
    public function action_ajax_save()
    {
        ...
        $skin_back_id = Arr::get($_POST,'cfg_skin_back_id');

        //保存skin_id到sysconfig表
        $data = array();
        $data['cfg_skin_id'] = $skin_id;
        $data['cfg_skin_back_id'] = $skin_back_id;
        $data['webid'] = 0;
