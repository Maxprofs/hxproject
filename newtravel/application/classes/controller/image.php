...
class Controller_Image extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        $group = DB::select()->from('image_group')->where('group_id', '=', $group_id)
            ->and_where('belong_type','IS', NULL)->execute()->current();
    ...
    public function action_image_manage()
    {
        ...
        switch ($action)
        {
            ...
            case 'find':
                ...
                {
                    $image_obj->where('group_id', '=', $group_id);
                }
                if (!empty($limit_ext)&&!empty($limit_ext_arr))
                {
                    $image_obj->and_where('ext', 'in', $limit_ext_arr);
                }
            ...
            case 'rename_url':
                ...
                $sql = "update sline_image set image_name='{$name}' where url='{$url}'";
                $rows = DB::query(Database::UPDATE, $sql)->execute();
                break;
        }
    ...
    public function action_image_move()
    {
        ...
        $rows = DB::select()->from('image_group')->where('pid', '=', $group_pid)
            ->and_where('belong_type', 'IS', NULL)
            ->execute()->as_array();
    ...
    public function action_group_manage()
    {
        ...
        switch ($action)
        {
            ...
            case 'find':
                ...
                $image_group_obj->and_where('belong_type','IS', NULL);
                //关键词搜索
    ...
    {
        $rows = DB::select()->from('image_group')->where('belong_type','IS', NULL)
            ->order_by('group_id', 'asc')->execute()->as_array();
        $items = array();
        foreach ($rows as $v)
        {
     ...
     * 缩略图地址规则配置
     */
    public function action_img_url_config()
    {
        $data = DB::select()->from('sysconfig')->where('varname', '=', 'cfg_img_param')->execute()->current();
        $config_info = array('cfg_img_param' => $data['value']);
        if (isset($this->params['set']))
        {
            $cfg_img_param=Arr::get($_POST, 'cfg_img_param');
            //更新设置项
            DB::update('sysconfig')->set(array('value' =>$cfg_img_param))->where('varname', '=', 'cfg_img_param')->execute();
            Model_Config::clear_cache();
            ...
            {
                $exist=array(
                    'delimiterLeft' => '#image_new_param start',
                    'delimiterRight' => '#image_new_param end',
                    'rules' => '{PHP_EOL}#image_new_param start{PHP_EOL}RewriteCond %{QUERY_STRING} mark=1&w=(.*)&h=(.*){PHP_EOL}RewriteRule ^(uploads/.*?)\.(jpg|gif|png|jpeg) /images/index?file=$1_1_%1x%2.$2&w=%1&h=%2&type=$2 [L,NC]{PHP_EOL}{PHP_EOL}RewriteCond %{QUERY_STRING} w=(.*)&h=(.*){PHP_EOL}RewriteRule ^(uploads/.*?)\.(jpg|gif|png|jpeg) /images/index?file=$1_%1x%2.$2&w=%1&h=%2&type=$2 [L,NC]{PHP_EOL}#image_new_param end{PHP_EOL}',
                );
                if(!preg_match("~" . $exist['delimiterLeft'] . '.*' . $exist['delimiterRight'] . '~is', $content))
                {
                    $config=array(
                        'delimiterLeft' => '#mobile end',
                        'delimiterRight' => '# Protect hidden files from being viewed',
                        'rules' => '{PHP_EOL}#image_new_param start{PHP_EOL}RewriteCond %{QUERY_STRING} mark=1&w=(.*)&h=(.*){PHP_EOL}RewriteRule ^(uploads/.*?)\.(jpg|gif|png|jpeg) /images/index?file=$1_1_%1x%2.$2&w=%1&h=%2&type=$2 [L,NC]{PHP_EOL}{PHP_EOL}RewriteCond %{QUERY_STRING} w=(.*)&h=(.*){PHP_EOL}RewriteRule ^(uploads/.*?)\.(jpg|gif|png|jpeg) /images/index?file=$1_%1x%2.$2&w=%1&h=%2&type=$2 [L,NC]{PHP_EOL}#image_new_param end{PHP_EOL}',
                    );
                    $match=preg_match("~" . $config['delimiterLeft'] . '(.*)' . $config['delimiterRight'] . '~is', $content,$old_content);

                    $replace = $config['delimiterLeft'];
                    $replace .= str_replace(array('{PHP_EOL}','$1','$2','$3','$4'), array("\r\n",'\$1','\$2','\$3','\$4'), $config['rules']);
                    $replace .= str_replace(array('$1','$2','$3','$4'), array('\$1','\$2','\$3','\$4'), $old_content[1]);//追加匹配内容
                    $replace .= $config['delimiterRight'];
                    if ($match)
                    {
                        $content = preg_replace("~" . $config['delimiterLeft'] . '.*' . $config['delimiterRight'] . '~is', $replace, $content);
        ...
        else
        {
            ...
            $this->display('stourtravel/image/img_url_config');
        }
