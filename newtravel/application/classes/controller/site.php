...
class Controller_Site extends Stourweb_Controller{
    ...
    * */
    ...
    public function action_index()
    {
        ...
        $dest_menu_id = DB::query(Database::SELECT, "select id from sline_menu_new where level=2 and (directory is null or directory='') and controller='destination' and method='destination'")
                          ->execute()
                          ->get('id');
        $templet_menu_id = DB::query(Database::SELECT, "select id from sline_menu_new where level=2 and (`directory` is null or `directory`='') and controller='site' and method='index'")
                             ->execute()
                             ->get('id');
        $webname_menu_id = DB::query(Database::SELECT, "select id from sline_menu_new where level=2 and (directory is null or directory='') and controller='config' and method='base'")
                             ->execute()
                             ->get('id');
