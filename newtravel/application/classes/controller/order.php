...
class Controller_Order extends Stourweb_Controller
{
    ...
    public function action_ajax_save()
    {
        ...
        if ($model->saved())
        {
            ...
            $order = $model->as_array();
            //更新支付信息表
            $updata_pay_data=Model_Member_Order_Compute::update($order['ordersn']);
    ...
    public function action_genexcel()
    {
        ...
        foreach ($arr as $row)
        {
            ...
            if($model['maintable']=='model_archive')
            {
                $address_info = DB::select()->from('member_address')->where('id','=',$row['receiver_address_id'])->execute()->current();
                $address_str = !empty($address_info)?$address_info['province'].$address_info['city'].$address_info['address'].'('.$address_info['receiver'].'收) '.$address_info['phone']:'';
                $table .= "<td>".$address_str."</td>";
            }
    ...
    public function action_viewed()
    {
        ...
        if ($data)
        {
            DB::update('member_order')->set(array('viewstatus'=>1))->where('id', 'in', DB::expr("({$data})"))->execute();
        }
        echo 'ok';
    }
