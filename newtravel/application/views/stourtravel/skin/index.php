                    ...
                    <div class="w-set-con">
                        <div class="cfg-header-bar">
                            <div class="cfg-header-tab">
                                <span class="item {if $_GET['platform'] != 'back'}on{/if}" id="column_front" onclick="Product.switchTabs(this,'front')">PC前台皮肤</span>
                                <span class="item {if $_GET['platform'] == 'back'}on{/if}" id="column_back" onclick="Product.switchTabs(this,'back')">后台皮肤</span>
                            </div>
                            <a href="javascript:;" class="fr btn btn-primary radius mt-6 mr-10" onclick="window.location.reload()">刷新</a>
                        </div>
                        <div class="clear">
                            <div class="clear clearfix product-add-div {if $_GET['platform'] != 'back'}content-show{else}content-hide{/if}" id="content_front">
                            ...
                            <div class="clear clearfix product-add-div {if $_GET['platform'] == 'back'}content-show{else}content-hide{/if}" id="content_back">
                                <ul class="info-item-block">
                                    <li>
                                        <span class="item-hd">皮肤方案：</span>
                                        <div class="item-bd">
                                            {loop $skin_back_list $v}
                                            <label class="skinitem radio-label mr-20" data-skin_back_id="{$v['id']}">
                                                <input type="radio" id="skin_back_id_{$v['id']}"  name="cfg_skin_back_id" data-issystem="{$v['is_system']}" value="{$v['id']}" {if $cfg_skin_back_id==$v['id']}checked{/if}>
                                                <span for="skin_back_id_{$v['id']}">{$v['title']}<span style=" display: inline-block; width: 50px; height: 20px; vertical-align: middle; margin-left: 5px; margin-top: -3px; background-color: {$v['color']}"></span></span>
                                            </label>
                                            {/loop}
                                        </div>
                                    </li>
