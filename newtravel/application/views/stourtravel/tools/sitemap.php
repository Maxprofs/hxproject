        ...
        <tr>
            ...
            <td valign="top" class="content-rt-td" style="overflow:hidden">
                ...
                <div class="clear pd-20">
                    <h5 class="c-primary">XML格式sitemap{Common::get_help_icon('sitemap')}</h5>
                    <div class="mt-10">
                        <span class="item-text ml-20">上次生成日期:<span class="gentxt">{$xmlinfo['time']}</span></span>
                        <span class="item-text ml-20">生成数量:<span class="gentxt">{$xmlinfo['number']}</span></span>
                    </div>
                    <div class="mt-5">
                        <a class="btn btn-primary radius" onclick="genXmlMap()">生成XML</a>
                        <a class="btn btn-primary radius ml-5" id="xml_sitemap_btn" target="_blank" href="javascript:;" data-ok="{$isxml}" data-url="{$GLOBALS['cfg_basehost']}/Sitemap.xml">预览地图</a>
                        <a class="btn btn-primary radius ml-5" id="xmlbtn" href="javascript:;" data-value="Sitemap.xml">复制链接地址</a>
                    </div>
                    <div class="item-section mt-10 c-999"></div>
                </div>
                <div class="line"></div>
                <div class="clear pd-20">
                    <h5 class="c-primary">HTML格式sitemap</h5>
                    <div class="mt-10">
                        <span class="item-text ml-20">上次生成日期:<span class="gentxt">{$htmlinfo['time']}</span></span>
                        <span class="item-text ml-20">生成数量:<span class="gentxt">{$htmlinfo['number']}</span></span>
                    </div>
                    <div class="mt-5">
                        <a class="btn btn-primary radius" onclick="genHtmlMap()">生成HTML</a>
                        <a class="btn btn-primary radius ml-5" target="_blank" id="html_sitemap_btn"  href="javascript:;" data-ok="{$ishtml}" data-url="{$GLOBALS['cfg_basehost']}/Sitemap.html">预览地图</a>
