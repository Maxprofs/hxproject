...
<head>
    <meta charset="utf-8">
    <title clear_background=5Ya1fl >移动到组-笛卡CMS{$coreVersion}</title>
    {template 'stourtravel/public/public_min_js'}
    {php echo Common::getCss('base.css,picmanage.css,base_new.css'); }
    <script type="text/javascript" src="{$publicPath}js/plupload-2.3.6/js/plupload.full.min.js"></script>
    <script type="text/javascript" src="{$publicPath}js/plupload-2.3.6/js/i18n/zh_CN.js"></script>
    <script>
...
<div class="pic-manage-container" id="picManageContainer">
    <div class="manage-tab-wrap">
        ...
        <div class="manage-tab-box">
            ...
            <div class="manage-tab-bd" id="local_content" style="display: none;">
                <div class="manage-tab-bar">
                    ...
                    <span class="col-tit">上传图片大小每张不超过5M</span>
                </div>
                <div class="manage-tab-block">
                    <div class="manage-file-wrap">
            ...
            <div class="manage-tab-bd" id="net_content" style="display: none;">
                <div class="manage-tab-bar">
                    <span class="col-tit">输入链接地址，如 http://www.deccatech.cn/Public/Home/images/top_logo.png</span>
                </div>
                <div class="manage-tab-block">
        ...
        <div class="manage-btn-block">
            <a class="btn btn-primary size-L mr-10 mt-2" id="submit" href="#">确定</a>
            <a class="btn btn-default size-L mt-2" id="cancel" href="#">取消</a>
        </div>
    </div>
</div>
<!--插入图片-->
...
<script>
    ...
    function getImages(p){
        ...
        $.post(SITEURL + 'image/image_manage', {action: 'find', pid:pid,page:p,keyword:search_keys,search_type:search_type,limit_ext:limit_ext}, function (data) {
             var html='';
             for(var i in data){
        ...
        init: {
            ...
            FilesAdded: function (up, files) {
                plupload.each(files, function (file) {
                    var tpl = '<li class="pic-item" id="{id}" name="{name}"><div class="pic-img"><div class="progress-wrap"><span class="progress-bar" style=" width: 0%"></span><em class="txt">上传中 45%</em></div><img src="{imgsrc}" /></div><input class="input-text pic-name" style=" border: 0" onblur="changeName(\'{id}\',this.value)" value="{name}"/></li>';
            ...
            Error: function(up, err) {
                if(err.code==-600){
                    ST.Util.showMsg('每张上传图片不超过5M',5,1000);
                }
            }
        }
    });
    ...
    function changeName(id, name) {
        ...
        if (name.length > 0) {
            $.post(SITEURL + 'image/image_manage', {'action': 'rename_url', 'name': name, 'url': url}, function (data) {
            }, 'json');
        }
    }
