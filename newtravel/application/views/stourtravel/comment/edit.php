                    ...
                    <div id="product_grid_panel" class="manage-nr">
                        ...
                        <div class="product-add-div" >
                            <ul class="info-item-block">
                                    ...
                                    <div class="item-bd">
                                        <div class="select-box w100">
                                            <select class="select" name="level">
                                                ...
                                                <option value="5" {if $info['level']==5 || empty($info['id'])}selected="selected"{/if}>5星</option>
                                            </select>
                                        </div>
                                    </div>
                                </li>
                                ...
                                <li>
                                    <span class="item-hd">评论图片{Common::get_help_icon('comment_piclist')}：</span>
                                    <div class="item-bd">
                                ...
                                <li class="list_dl">
                                    <span class="item-hd">评论日期：</span>
                                    <div class="item-bd">
                                        {if empty($info['memberid'])}
                                        <input type="text" class="input-text w150" readonly="readonly" name="addtime" value="{if $info['addtime']}{date('Y-m-d',$info['addtime'])}{else}{date('Y-m-d')}{/if}" onclick=" calendar.show({ id: this,maxDay:'{date('Y-m-d')}'})" />
                                        {else}
                                        <label class="radio-label">{date('Y-m-d H:i:s',$info['addtime'])}</label>
                                        {/if}
                                    </div>
                                </li>
                                <li class="list_dl">
                                    <span class="item-hd"><i class="c-red va-m mr-5">*</i>会员昵称{Common::get_help_icon('comment_nickname')}：</span>
                                    <div class="item-bd">
                                ...
                                <li class="list_dl">
                                    ...
                                    <div class="item-bd">
                                        <input type="text" class="input-text w100"  name="vr_jifencomment" id="vr_jifencomment" value="{$info['vr_jifencomment']}" > 分
                                    </div>
                                </li>
                                {else}
                                <li class="list_dl">
                                    <span class="item-hd">评论日期：</span>
                                    <div class="item-bd"><label class="radio-label">{date('Y-m-d H:i:s',$info['addtime'])}</label></div>
                                </li>
                                <li class="list_dl">
                                    <span class="item-hd">会员昵称{Common::get_help_icon('comment_nickname')}：</span>
                                    <div class="item-bd"><label class="radio-label">{$info['member']['nickname']}</label></div>
                                </li>
                                {/if}
                                {if empty($info['memberid'])}
                                <li class="list_dl">
                                    <span class="item-hd">评论来源：</span>
                                    <div class="item-bd"><label class="radio-label">后台添加</label></div>
                                </li>
                                {else}
                                <li class="list_dl">
                                    <span class="item-hd">评论来源：</span>
                                    <div class="item-bd"><label class="radio-label">用户评论</label></div>
                                </li>
                                {/if}
                                <li class="list_dl">
                                    <span class="item-hd">前台显示：</span>
                                    <div class="item-bd">
                                        <label class="radio-label"><input type="radio" name="isshow" {if $info['isshow']==1}checked="checked"{/if} value="1"/>显示</label>
                                        <label class="radio-label ml-20"><input type="radio" {if empty($info['isshow'])}checked="checked"{/if} name="isshow" value="0"/>隐藏</label>
                                    </div>
                                </li>
                           </ul>
                        </div>
...
<script language="JavaScript">
    ...
    $(function(){
        ...
        $('#pic_btn').click(function(){
           if($('.pic-sel img').length>=10)
           {
               ST.Util.showMsg('图片数量不能超过10张','5',2000);
               return;
           }
