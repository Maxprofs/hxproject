						     ...
						     foreach($list as $k=>$v)
						      {
                               ...
                               <tr>
                                <td align="center" class="text-c">
                                    <input class="input-text w80 text-c" name="displayorder[{$v['id']}]" value="{$v['displayorder']}"  size="6">
                                </td>
                                <td align="center" class="text-c">
                                    <input class="input-text w200 kindname-item" name="kindname[{$v['id']}]" value="{$v['kindname']}" size="20">
                                </td>
                                <td align="center" valign="center" class="text-c">
                                    <span class="va-t" id="litpic_{$v['id']}">
                                        {if !empty($v['litpic'])}
                                        <img src="{$v['litpic']}" width="20" height="20" class="mr-5" />
                                        {/if}
                                        {if !empty($v['litpic'])}
                                        <a href="javascript:;" style=" display: inline-block" class="image_delbtn btn-link va-m" data-id="{$v['id']}">删除</a>
                                        {/if}
                                    </span>
                                    <a href="javascript:;" style=" display: inline-block" class="image_upbtn btn-link va-m" data-id="{$v['id']}">上传</a>
                                </td>
...
<script>
  ...
  function setIcon(id,img)
  {
      $.ajax({
          ...
          success:function(data){
              if(data.status){
                  $("#litpic_"+id).html("<img src='"+img+"' width='20' height='20' class='mr-5' /><a href='javascript:;' class='image_delbtn btn-link va-m' data-id='"+id+"'>删除</a>");
              }
          }
      })
  }
  ...
  function addRow()
  {
      ...
      html+='<td align="center" valign="center" class="text-c"><span id="litpic_'+id+'"></span><a href="javascript:;" class="image_upbtn btn-link"  data-id="'+id+'">上传</a></td>';
