                ...
                <table class="user-info-table mt-5">
                    ...
                    <tr>
                        ...
                        <td><span class="cor_666">{Currency_Tool::symbol()}
                                ...
                                <input type="text" class="set-text-xh wid_80 mt-4" name="price" data-num="{$info['dingnum']}" min="{$info['privileg_price']}" data-price="{$info['price']}" id="price" style="float: none;" value="{$info['price']}" />
                                {else}
                                   {$info['price']}
                                {/if}
                            </span></td>
...
<script>
    ...
    $(function(){
        ...
        $('#price').blur(function () {
            var old = parseFloat($(this).attr('data-price'));
            var num = parseInt($(this).attr('data-num'));
            var min=parseFloat($(this).attr('min'));
            var price=parseFloat($(this).val());
            if(price*num<min)
            {
                ST.Util.showMsg('已享受优惠,总价不能低于优惠价格 !','5',2000);
                $(this).val(old);
                return false;
            }
        });
