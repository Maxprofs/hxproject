<!doctype html>
<html>
<head float_border=5MfLuj >
    <meta charset="utf-8">
    <title>广告管理-笛卡CMS{$coreVersion}</title>
        ...
        <td valign="top" class="content-rt-td" style="overflow:hidden">
            ...
            <div class="cfg-header-bar clearfix">
                    <span class="select-box w150 mt-5 ml-10 fl">
                        <select name="ismobile" id="ismobile" class="select" onchange="goSearch({'ismobile':this.value})">
                            <option value="0">电脑版</option>
                            <option value="1">移动版</option>
                        </select>
                    </span>
                    <span class="select-box w150 mt-5 ml-10 fl">
                        <select name="weblist" id="weblist" class="select" onchange="goSearch({'webid':this.value})">
                        </select>
                    </span>
                    <span class="select-box w150 mt-5 ml-10 fl">
                    ...
                    <span class="select-box w150 mt-5 ml-10 fl hide">
                        <select name="adtype" id="adtype" class="select" onchange="load_module(this.value)">

                        </select>
                    </span>
                    <span class="select-box w150 mt-5 ml-10 fl hide">
                            <select name="module" id="module" class="select" onchange="goSearch('adtype',this.value)">
...
<script>
        ...
        if (val != 0) {
            ...
            for (var i in pageConfig['page']) {
                ...
                if (val == item['pid']) {
                    ...
                    if(val==8){
                        pageHtml += '<option value="custom_index">自定义广告</option>';
                    }
                }
            }
            ...
            $('#module').html('<option value="" selected></option>').parent().addClass('hide');
        }
        $('#adtype').change();
    });
    ...
    //进行搜索
    function goSearch(param,val) {
        if(param['ismobile'] || param['webid'] ||  param['keyword']){
            $('#mould').find('option[value="0"]').attr('selected',true);
            $('#mould').change();
            window.product_store.getProxy().extraParams =param;
        }else{
            window.product_store.getProxy().extraParams[param] =val;
        }
    ...
    function load_module(page){
        ...
        $('#adtype').parent().removeClass('hide');
        $.post(SITEURL + 'advertise5x/ajax_page_module/',{page:page,'ismobile':$('#ismobile').val(),'webid':$('#weblist').val()},function(result){
            var pageHtml='';
            var item=result.data;
            var length=item.length;
            for(var i=0;i<length;i++){
                pageHtml+='<option value ="'+page+','+item[i]['advertise_name']+'">'+item[i]['title']+'</option>';
            }
            $('#module').html(pageHtml).parent().removeClass('hide');
            $('#module').change();
        },'json')
    }
</script>
