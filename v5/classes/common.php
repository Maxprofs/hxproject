...
class Common
{
    ...
    public static function get_editor($fname, $fvalue, $nwidth = "700", $nheight = "350", $etype = "Sline", $ptype = '', $gtype = "print", $jsEditor = false)
    {
        ...
        $config['qiniu'] = 1;
        if (St_Functions::is_normal_app_install('image_qiniu') && Model_Qiniu::get_qiniu_config()) 
        {
            $config['qiniu'] = '/qiniu/index/ueditor_qiniu_upload';
        }
...
';
    ...
    */
     ...
     * @return string
     */
    public static function css_plugin_distributor($filelist, $pinyin, $mincss = true, $default = true)
    {
        $filearr = explode(',', $filelist);
        $filelist = array();
        $out = '';
        ...
        foreach ($filearr as $file)
        {
            if ($default == true)
            {
                $tfile = DOCROOT . ltrim($plugin_res_url, '/\\') . "default/pc/css/" . $file;
                $file = ltrim($plugin_res_url, '/\\') . "default/pc/css/{$file}";
            }
            else
            {
                $tfile = DOCROOT . $file;
        ...
        //加载独立模块的js
    public static function js_plugin_distributor($filelist, $pinyin, $minjs = true, $default = true)
    {
        $filearr = explode(',', $filelist);
        $jsArr = array();
        $out = $v = '';
        ...
        foreach ($filearr as $file)
        {
            if ($default == true)
            {
                $tfile = DOCROOT . ltrim($plugin_res_url, '/\\') . "default/pc/js/" . $file;
                $file = ltrim($plugin_res_url, '/\\') . 'default/pc/js/' . $file;
            }
            else
            {
                $tfile = DOCROOT . $file;
    ...
    public static function check_is_sub_web($destid, $url)
    {
        ...
        if ($iswebsite == 1)
        {
            if($pdestinfo['id']!=$webid)
			{
			        $baseurl = Common::get_web_url($webid);
                    self::head301($url);
			}
		...
		$pdestinfo = DB::select('id','pid', 'iswebsite','weburl')->from('destinations')->where('id', '=', $destid)->execute()->current();
        $pid = $pdestinfo['pid'];
        $iswebsite = $pdestinfo['iswebsite'];
		$webid = $GLOBALS['sys_webid'];
        ...
        if ($iswebsite == 1)
        {
            if($pdestinfo['id']!=$webid)
			{
			        $weburl = $pdestinfo['weburl'];
			        $model_info=explode('/',$url);
			        if($model_info&&$model_info[0])
                    {
                        self::head301($weburl. '/' . $model_info[0]);
                    }
                    self::head301($weburl);
			}
