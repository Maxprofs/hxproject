...
class Controller_Member_Register extends Stourweb_Controller {
	...
	public function action_index() {
		...
		if ($this->_data['mid']!='') {
			$this->assign('did',$this->_data['mid']);
		}
	...
	public function action_ajax_doreg() {
		if (Arr::get($_POST, 'did') == 'nothing') {
			echo json_encode(array('status' => 0, 'msg' => '请选择服务网点'));
			exit;
		}
		...
		if ($m->saved()) {
			...
			// 绑定服务网点
			Model_Distributor::distributor_bind($m->mid, Arr::get($_POST, 'did'), Arr::get($_POST, 'sendnum'));
			// 更新短信发送记录
			if ($regType == 'phone') {
				$time = Common::session('smstime' . $mobile);
				$sql = "update sline_sms_send_log set mid=$m->mid,smstype='游客注册成功' where sendtime=$time";
