...
class Controller_Member_Index extends Stourweb_Controller
{
    ...
    public function action_linkman()
    {
        ...
        $out = Model_Member_Linkman::get_linkman($this->mid, $page, $pageSize);

        $pager = Pagination::factory(
            array(
                'current_page' => array('source' => 'query_string', 'key' => 'p'),
                'view' => 'default/pagination/new_page',
                'total_items' => $out['total'],
                'items_per_page' => $pageSize,
                'first_page_in_url' => false,
            )
     ...
     * @function 修改常用联系人
     */
    public function action_edit_linkman()
    {
        $id = $this->request->param('query');
        if(is_numeric($id))
        {
            $info = Model_Member_Linkman::detail($id);
            if($info['memberid']!=$this->mid)
            {
    ...
    public function action_add_linkman()
    {
    ...
    public function action_ajax_check_linkman_card()
    {
        ...
        $where = "idcard = '$idcard' and cardtype='$cardtype' and memberid={$this->mid}";
        if($id)
        {
            $where .= " and id<>$id";
        }
    ...
    {
        $linkman = Common::remove_xss($_POST['linkman']);
        $mobile = Common::remove_xss($_POST['mobile']);
        $cardtype = Common::remove_xss($_POST['cardtype']);
        $idcard = Common::remove_xss($_POST['idcard']);
        $sex = Common::remove_xss($_POST['sex']);
        $id = Common::remove_xss($_POST['id']);
        $check = DB::select('id')->from('member_linkman')
            ->where('idcard','=',$idcard)
            ->and_where('cardtype','=',$cardtype)
        ...
        if($check&&$check!=$id||($info['memberid']!=$this->mid&&$id))
        {
           $status = 0 ;
        }
        else
        {
            ...
            if($id)
            {
                DB::update('member_linkman')->set($data)->where('id','=',$id)->execute();
            }
            else
            {
                $data['memberid'] = $this->mid;
                DB::insert('member_linkman',array_keys($data))->values(array_values($data))->execute();
            }
            $status = 1;
        }
    ...
    public function action_ajax_do_del_linkman()
    {
        ...
        if($id)
        {
            ...
            DB::delete('member_linkman')->where('id','=',$id)->execute();
    ...
    public function action_ajax_userinfo_save()
    {
        ...
        if ($m->loaded())
        {
            ...
            if ($m->saved())
            {
                if($old_litpic!=$litpic && !empty($litpic) && $litpic!=Common::member_nopic())
                {
    ...
    * */
    ...
    public function action_invoice()
    {
        ...
        $out = Model_Member_Invoice::search_result($this->mid,null,null,$page,$pageSize);
    ...
    {
        $this->display('member/invoice_edit');
    }
    ...
    {
        $id = $_GET['id'];
        $info = DB::select()->from('member_invoice')->where('id','=',$id)->and_where('memberid','=',$this->mid)->execute()->current();
        $this->assign('info',$info);
        $this->display('member/invoice_edit');
    }
    ...
    public function action_ajax_invoice_save()
    {
        ...
        $invoice_model = ORM::factory('member_invoice',$_POST['id']);
        if($invoice_model->loaded() && $invoice_model->memberid!=$this->mid)
        {
        ...
        $invoice_model->memberid = $this->mid;
    ...
    {
        $id = $_POST['id'];
        $invoice_model = ORM::factory('member_invoice')->where('id','=',$id)->and_where('memberid','=',$this->mid)->find();
