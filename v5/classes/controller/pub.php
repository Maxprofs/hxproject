...
class Controller_Pub extends Stourweb_Controller {
	...
	*/
	...
	/*
		     * Deccatech网站站点选择
	*/
	public function action_startcity() {
		$templet = 'pub/user_start_city_station';
		$this->display($templet);
	}
...
AND (unix_timestamp() - addtime) > ({$timeout} * 60)";
	 ...
	 * @function 获取更多咨询
	 */
	public function action_ajax_get_more_ask() {
		$product_id = intval(Common::remove_xss(Arr::get($_GET, 'product_id')));
		$type_id = intval(Common::remove_xss(Arr::get($_GET, 'type_id')));
		$page = intval(Common::remove_xss(Arr::get($_GET, 'page')));
		$page = $page > 0 ? $page : 1;
		$page_size = 10;
		$lists = Model_Question::search_result($page, $page_size, 0, $type_id, $product_id);

		foreach ($lists['list'] as &$list) {
			$list['fmt_date'] = date('Y-m-d', $list['addtime']);
			$list['fmt_time'] = date('H:i', $list['addtime']);
			$list['nickname'] = empty($list['nickname']) ? '匿名' : $list['nickname'];
		}
		$lists['num'] = count($lists['list']);
		$lists['status'] = empty($lists['list']) ? 0 : 1;
