...
<div class="tabcon-list" id="comment_target">
    <div class="list-tit"><strong>{__('客户点评')}</strong></div>
    <div class="st-list-pl">
        <div class="st-pl-box" style="border-bottom: 1px solid #d8d8d8;">
            ...
            <span class="myd" data-myd="{$count['satisfyscore']}">{__('满意度')}<strong>{$count['satisfyscore']}</strong></span>
            <span class="txt">{__('回程归来发表点评即可获赠积分')}，<br>{__('获得的积分可在下次消费时抵用部分现金')}。</span>
            <a class="pl-btn" href="{$cmsurl}member">{__('我要评论')} </a>
        </div>
        {if $count['total']>0}
        <div class="st-comment-block">
            <div class="comment-nav">
                <span class="on" data-flag="all">{__('全部点评')}<em>({$count['total']})</em></span>
                <span data-flag="pic">{__('有图')}<em>({$count['picnum']})</em></span>
                <span data-flag="well">{__('好评')}<em>({$count['wellnum']})</em></span>
                <span data-flag="mid">{__('中评')}<em>({$count['midnum']})</em></span>
                <span data-flag="bad">{__('差评')}<em>({$count['badnum']})</em></span>
            </div>
                ...
                <dl>
                    ...
                    <dd>
                        <div class="zs">
                            ...
                            <span class="jf">{$c['jifencomment']}{__('分')}<i></i></span>
                        </div>
                        ...
                        {/if}
                        <div class="date">{Common::mydate('Y-m-d',$c['addtime'])}</div>
                    </dd>
                </dl>
                {/loop}
            </div>
    ...
    <dl>
        ...
        <dd>
            <div class="zs">
                ...
                <span class="jf">{{value.jifencomment}}{__('分')}<i></i></span>
            </div>
...
<script>
    $(function(){
        ...
        if(myd)
        {
            $(".cp-show-msg .sl .myd").html("{__('满意度')}："+myd);
        }
