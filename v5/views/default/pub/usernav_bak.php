...
<div class="global-list" {if empty($indexpage)}style="display: none;"{/if}>
        ...
        {loop $data $nav}
            <div class="gl-list-tabbox">
                <h3>
                    <strong><em><i style="float:left;background: url({Common::img($nav['litpic'])});width: 35px;height: 35px; margin-right: 5px;" class="usernavicon{$i++}"></i></em><a {if !empty($nav['url'])&&filter_var($nav['url'],FILTER_VALIDATE_URL)}href="{$nav['url']}" {else}href="javascript:;"{/if}target="_blank">{$nav['kindname']}</a></strong>
                    <p>
