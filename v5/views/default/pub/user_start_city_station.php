{Common::css('user_start_city_station.css',true)}
{Common::js("py.js")}
<?php
$arrInitParam = array(
    'controller' => 'line',
...
{st:citystation action="citystation" return="arrCityList"}
<div class="city_warp" id="city_warp">
    <div class="city_select">
        <dl class="start_city_station">
            <dt class="city_station"><i class="city_station_icon"></i><span class="city_station_btn"><?php 
                if ($_COOKIE['cityname']=='') { ?>
                   {$cityname}
                <?php }else{ echo $_COOKIE['cityname']; }
             ?></span><span>站</span><b class="city_station_arrow"></b><span class="city_station_id" style="visibility: hidden;">
                <?php 
                if ($_COOKIE['cityid']=='') { ?>
                   {$cityid}
                <?php }else{ echo $_COOKIE['cityname']; }
    ...
    <dd class="city_searchBox">
        <div class="hot_station">
            <h4>
                <?php  
                    foreach ($arrCityList as $key => $val) {
                        if (array_search('热门出发城市', $val)){ ?>
                            {$val['cityname']}
                    <?php 
                            $hotid=$val['id'];
                            break; 
            ...
            <p>
                <?php 
                    foreach ($arrCityList as $key => $city) {
                        if ($hotid==$city['pid']) { ?> 
                            <a id="{$city['id']}" href="###" onclick="changecity(this)">{$city['cityname']}</a>
                            <!-- <a href="{Model_Line::get_search_url($city['id'],'startcityid',$arrInitParam)}">{$city['cityname']}</a> -->
                        <?php
                            continue;
                        }
                    }
        ...
        <div class="station_search">
            <dir class="station_search_box">
                <p class="station_search_p">搜索城市（支持汉字、首字母查询）</p>
                <input class="station_search_input" type="text" oninput="detectInput(this.value)">
            </dir>
                <div class="station_search_result" style="display:block;">
    </div>
        ...
        <?php
            ...
            foreach ($arrCityList as $key => $value) {
                if ($value['pid']==0 and $value['cityname']!='热门出发城市') { 
                    if ($value['cityname']=='A') { ?>
                        <a href="###" onmouseover="displayul(this);" class="{$value['id']} on">{$value['cityname']}</a>
                     <?php } else { ?>
                        <a href="###" onmouseover="displayul(this);" class="{$value['id']}">{$value['cityname']}</a>
                     <?php }?>
    ...
    <div class="city_list" style="scroll-behavior: auto;">
    <?php 
    foreach ($arrCityIndex as $key => $value) { 
        if ($value['cityname']=='A') { ?>
            <ul class="station_search_list {$value['cityname']}" style="display: block;">
        <?php }else{ ?>
            <ul class="station_search_list {$value['cityname']}" style="display: none;">
        <?php }
        ?>
       
            <li>
                <span class="{$value['id']}">{$value['cityname']}</span>
            <?php 
                foreach ($arrCityList as $cityid => $city) {
                    if ($city['pid']==$value['id']) { ?>
                        <a id="{$city['id']}" href="###" onclick="changecity(this)">{$city['cityname']}</a>
                        <!-- <a href="{Model_Line::get_search_url($city['id'],'startcityid',$arrInitParam)}">{$city['cityname']}</a> -->
                <?php    }
                }
...
function createLink() {
            ...
            $.each(startCityName, function(index, val) {
                if (typeof(val) == 'string' && i<5) {
                    i++;
                    $('.station_search_result').append("<a style='font-weight:bold;font-size:12px;font-family:Tahoma,Simsun,sans-serif;display: inline-block;margin-right: 10px;line-height: 28px;color: #0065bb' href='{Model_Line::get_search_url('" + index + "','startcityid',$arrInitParam)}'>" + val + "</a>");
                }
            });
}
