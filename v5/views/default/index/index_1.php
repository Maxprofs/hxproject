  ...
  <div class="big">
  	<div class="wm-1200">
        ...
        {loop $data $row}
            ...
            {elseif !empty($row['typeid'])&&(Model_Model::all_model($row['typeid'],'maintable'))=='model_archive'&&(Model_Model::all_model($row['typeid'],'issystem'))==0}
                {if $row['isopen']==1}
                    <div class="st-slideTab">
                        {php}$pinyin=Model_Model::all_model($row['typeid'],'pinyin');{/php}
                        <div class="st-tabnav">
                            <h3 class="ty-bt">{$row['title']}</h3>
                            <a href="{$cmsurl}{$pinyin}" class="more">{__('更多')}</a>
                        </div>
                        <div class="st-tabcon">
                            <ul class="st-cp-list tuan-list">
                                ...
                                <li {if $ty_index==4}class="mr_0"{/if}>
                                    <div class="pic">
                                        <img src="{$defaultimg}" st-src="{Common::img($ty['litpic'],285,194)}" alt="{$ty['title']}" width="285" height="194" />
                                        <div class="buy">
                                            <a href="{$ty['url']}" target="_blank" title="{$ty['title']}">{__('立即预定')}</a>
                                        </div>
                                    </div>
                                    <div class="js">
                                        <a class="tit" href="{$ty['url']}" target="_blank" title="{$ty['title']}">{$ty['title']}</a>
                                        <p class="attr">
                                            {loop $ty['iconlist'] $ico}
                                            <img src="{$ico['litpic']}" />
                                            {/loop}
                                        </p>
                                        <p class="num">
                                            {if !empty($ty['sellprice'])}
                                            <del>{__('原价')}：<i class="currency_sy">{Currency_Tool::symbol()}</i>{$ty['sellprice']}</del>
                                            {/if}
                                            <span>
                                                {if !empty($ty['price'])}
                                                    <b><i class="currency_sy">{Currency_Tool::symbol()}</i>{$ty['price']}</b>{__('起')}
                                                {else}
                                                    {__('电询')}
                                                {/if}
                                            </span>
