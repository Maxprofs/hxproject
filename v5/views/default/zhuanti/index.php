  ...
  <div class="big" {if !empty($info['bgimage'])}style=" background:url({$info['bgimage']}) repeat"{else}style=" background:#dce9fa"{/if}>
  	<div class="wm-1200">
        ...
        {if count($linelist)>0}
        	...
        	<div class="cp-tit"><strong>{__('推荐线路')}</strong><a class="g_more" target="_blank" href="/lines/all-0-0-4-0-0-0-1">{__('查看更多')}</a></div>
          <div class="cp-conbox">
            ...
            <div class="cp-conlist">
            	<ul>
                    ...
                    <div class="pic">
                        {if $l['saveprice']>0}
                        <span class="sub"><em>{__('立减')}</em><strong>{$l['saveprice']}{__('元')}</strong></span>
                        {/if}
                        <img src="{Common::img($l['litpic'],280,190)}" alt="{$l['title']}" /></div>
                    <div class="bt">{$l['title']}</div>
                    <div class="cz">
                      <span class="jg"><b><i class="currency_sy">{Currency_Tool::symbol()}</i>{$l['price']}</b>{__('起')}</span>
                      <span class="now-gm">{__('立即购买')}</span>
                    </div>
                  </a>
                </li>
                    {/loop}
        ...
        {if count($hotellist)>0}
            <div class="st-theme-cpbox">
        	<div class="cp-tit"><strong>{__('推荐酒店')}</strong><a class="g_more" target="_blank" href="/hotels/all-0-0-4-0-0-1">{__('查看更多')}</a></div>
          <div class="cp-conbox">
            <div class="cp-conlist">
            	<ul>
                 {loop $hotellist $h}
              	<li>
                    ...
                    <div class="cz">
                      <span class="jg"><b><i class="currency_sy">{Currency_Tool::symbol()}</i>{$h['price']}</b>{__('起')}</span>
                      <span class="now-gm">{__('立即购买')}</span>
                    </div>
                  </a>
                </li>
        ...
        {/if}
          ...
          {st:car action="query" flag="theme" themeid="$info['id']" row="6" return="carlist"}
            {if count($carlist)>0}
            <div class="st-theme-cpbox">
                <div class="cp-tit"><strong>{__('推荐车辆')}</strong><a class="g_more" target="_blank" href="/cars/all-0-4-0-1">{__('查看更多')}</a></div>
                <div class="cp-conbox">
                        ...
                        <ul>
                            ...
                            <li>
                                <a class="show-link" href="{$c['url']}" target="_blank">
                                    ...
                                    <div class="cz">
                                        <span class="jg"><b><i class="currency_sy">{Currency_Tool::symbol()}</i>{$c['price']}</b>{__('起')}</span>
                                        <span class="now-gm">{__('立即购买')}</span>
                                    </div>
                                </a>
                            </li>
            ...
            {if count($spotlist)>0}
            <div class="st-theme-cpbox">
                <div class="cp-tit"><strong>{__('推荐景点')}</strong><a class="g_more"  target="_blank" href="/spots/all-0-4-0-1">{__('查看更多')}</a></div>
                <div class="cp-conbox">
                    <div class="cp-conlist">
                        <ul>
                            {loop $spotlist $s}
                            <li>
                                <a class="show-link" href="{$s['url']}" target="_blank">
                                    ...
                                    <div class="cz">
                                        <span class="jg"><b><i class="currency_sy">{Currency_Tool::symbol()}</i>{$s['price']}</b>{__('起')}</span>
                                        <span class="now-gm">{__('立即购买')}</span>
                                    </div>
                                </a>
                            </li>
        ...
        {if count($arclist)>0}
            <div class="st-theme-cpbox">
        	<div class="cp-tit"><strong>{__('推荐攻略')}</strong><a class="g_more"  target="_blank" href="/raiders/all">{__('查看更多')}</a></div>
          <div class="gl-conbox">
          	<ul>
                {loop $arclist $a}
            	    <li>
    ...
    $(function(){
        ...
        ST.Login.check_login(function (data) {
            if(data.status){
                ...
                $txt+= '<a class="dl" href="javascript:ST.Login.login_out();">{__("退出")}</a>';
