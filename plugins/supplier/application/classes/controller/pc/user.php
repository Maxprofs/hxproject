...
class Controller_Pc_User extends Stourweb_Controller
{
    ...
    public function action_modify_email()
    {
        ...
        $data = $this->_model->get_supplier_byid($st_supplier_id);
        $this->assign('data', $data);
        $this->display('user/modify_email');
    }
    /**
     * 修改密码
     */
    ...
    public function action_ajax_do_modify_phone_setp2()
    {
        ...
        if (Common::session('msg_code') != $smscode && Common::session('msg_account') != $phone)
        {
        ...
        $st_supplier_id = intval(Cookie::get('st_supplier_id'));
        $exists_supplier = Model_Supplier::supplier_find($phone, null, true);
        if ($exists_supplier && $exists_supplier['id'] != $st_supplier_id)
        {
            echo json_encode(array('status' => 0, 'msg' => '“手机号码”已经存在'));
            exit;
        }

        $updateArr = array('mobile' => $phone, 'account' => $phone);
     ...
     * 修改邮件 第二步
     */
    public function action_ajax_do_modify_email_setp2()
    {
        if(!$this->request->is_ajax())
        {
            echo json_encode(array('status' => 0,'msg' => '请求异常'));
        ...
        $email = Common::remove_xss(Arr::get($_POST,'newemail'));
        $smscode = Common::remove_xss(Arr::get($_POST,'smscode'));

        if (Common::session('msg_code') != $smscode && Common::session('msg_account') != $email)
        {
            echo json_encode(array('status' => 0,'msg' => '“邮件验证码”错误'));
            exit;
        }
        Common::session('msg_code', null);
        Common::session('msg_account', null);
        ...
        $st_supplier_id = intval(Cookie::get('st_supplier_id'));
        $exists_supplier = Model_Supplier::supplier_find($email, null, true);
        if ($exists_supplier && $exists_supplier['id'] != $st_supplier_id)
        {
            echo json_encode(array('status' => 0, 'msg' => '“邮件地址”已经存在'));
            exit;
        }

        $updateArr = array('email' => $email, 'account' => $email);
        $whereStr = "id='{$st_supplier_id}'";
