...
class Model_Member_Private_Order extends Model
{
    ...
    private function _get_data_format($data)
    {
        ...
        else
        {
            ...
            if($table['maintable'])
            {
                ...
                if(Model_Model::is_standard_product($data['typeid'])&&empty($data['litpic']))
                {
            ...
            if($data['typeid']==113)
            {
                $data['order_status']=DB::select('status_name')->from('store_order_status')
                    ->where('status','=',$data['status'])->and_where('is_show','=',1)
                    ->execute()->get('status_name');
            }
            else
            {
