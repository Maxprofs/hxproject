        ...
        function () {
            ...
            window.dest_store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    api: {
                        read: SITEURL+'notes/admin/destination/destination/action/read',  //读取数据的URL
                        update: SITEURL+'notes/admin/destination/destination/action/save',
                        destroy: SITEURL+'notes/admin/destination/destination/action/delete'
                    },
                    reader: 'json'
                },
                autoLoad: true,
                    ...
                    },
                    {
                        ...
                        renderer : function(value, metadata,record) {
                            ...
                            if(id.indexOf('add')==-1)
                            {
                                ...
                                return value+"&nbsp;&nbsp;<font color='orange' padding_bottom=oRNzDt >[id:"+id+"]</font>";
...
//
//                                    return '<a href="javascript:;" class="btn-link" title="优化设置" onclick="destProductSet(\'' + id + '\')">优化设置</a>'+viewHtml;
//                                }
//                            }
//                        ),
//                        listeners: {
...
//                    }
    ...
    function togStatus(obj, record, field) {
        ...
        Ext.Ajax.request({
            url: SITEURL+"notes/admin/destination/destination/action/update",
            method: "POST",
            datatype: "JSON",
    ...
    function destProductSet(dest_id) {
        ...
        ST.Util.addTab('优化设置-'+record.get('kindname'),SITEURL+'notes/admin/destination/dialog_productinfo/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}?id='+id+"&typeid="+display_mode,0);
    ...
    function searchDest() {
        ...
        if (!s_str)
            return;
        Ext.Ajax.request({
            url: SITEURL+'notes/admin/destination/destination/action/search',
