                    ...
                    <table class="table table-bg table-hover" id="day_tab">
                        ...
                        <tbody>
                            ...
                            <tr class="text-c" id="item_{$v['id']}">
                                <td><input type="text" name="displayorder[{$v['id']}]" class="input-text w80 center" value="{$v['displayorder']}" size="6"/></td>
                                <td><input type="text" name="chinesename[{$v['id']}]" class="input-text" value="{$v['chinesename']}" size="20"/></td>
                                <td>{$v['columnname']}</td>
                                <td><input type="checkbox" name="isopen[{$v['id']}]" value="1"
...
<script>
  ...
  var typeid="{$typeid}";
   ...
   $("#additem").click(function(){
       ...
       $.ajax({
           type: 'POST',
           url: SITEURL+"visa/admin/visa/ajax_content_add",
           dataType:'json',
           success: function(data)
           {
               ...
               if(data.status)
               {
                   ...
                   var html='<tr class="text-c" id="item_'+row['id']+'"><td><input type="text" name="displayorder['+row['id']+']" class="input-text w80" value="'+row['displayorder']+'" size="6"/></td>'
  ...
  function rowSave()
  {
      ...
      $.ajaxform({
          url   :  SITEURL+"visa/admin/visa/content/action/save",
          method  :  "POST",
          form  : "#day_fm",
          dataType  :  "html",
  ...
  {
      var url =  SITEURL+"visa/admin/visa/ajax_content_del";
