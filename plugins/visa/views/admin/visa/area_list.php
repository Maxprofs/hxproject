    ...
    function () {
        ...
        window.attr_store = Ext.create('Ext.data.TreeStore', {
            ...
            proxy: {
                ...
                api: {
                    read: SITEURL+'visa/admin/visa/visaarea/action/read/',  //读取数据的URL
                    update:  SITEURL+'visa/admin/visa/visaarea/action/save/',
                    destroy:  SITEURL+'visa/admin/visa/visaarea/action/delete/'
                },
                reader: 'json'
            },
            autoLoad: true,
            ...
            viewConfig: {
                ...
                listeners: {
                    ...
                    drop: function (node, data, overModel, dropPosition, eOpts) {
                        ...
                        Ext.Ajax.request({
                            url: SITEURL+'visa/admin/visa/visaarea/action/drag/',
                            params: params,
                            method: 'POST',
            ...
            columns: [
                ...
                },
                {
                    ...
                    renderer : function(value, metadata,record) {
                        ...
                        if(id.indexOf('add')==-1)
                        {
                            return "<span class='row-editable-sp'>"+ value+"</span>&nbsp;&nbsp;<font color='orange' margin_size=BQe65k >[id:"+id+"]</font>";
                ...
                },
                {
                    ...
                    renderer : function(value, metadata,record) {
                        ...
                        if(id.indexOf('add')!=-1)
                            return '';
                        if(pid!=0){
                            return '<a href="javascript:;" title="配置" class="btn-link" onclick="config('+id+',\''+kindname+'\')">配置</a>&nbsp;&nbsp;<a href="javascript:;" onclick="del('+id+')" class="btn-link" title="删除">删除</a>';
                        }else{
                            return '<a href="javascript:;" class="btn-link" title="删除" onclick="del('+id+')">删除</a>';
                        }
                    }
                }
            ],
...
function config(id,countryname){
    var url = SITEURL+'visa/admin/visa/config/id/'+id;
...
function addSub(pid) {
    ...
    Ext.Ajax.request({
        method: 'post',
        url: SITEURL+'visa/admin/visa/visaarea/action/addsub/',
        params: {pid: pid},
        success: function (response) {
...
function updateField(ele,id,field,value,type)
{
    ...
    Ext.Ajax.request({
        url   :  SITEURL+"visa/admin/visa/visaarea/action/update/",
        method  :  "POST",
        datatype  :  "JSON",
