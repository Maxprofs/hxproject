...
<head>
    ...
    {Common::css('base.css')}
    {Common::css_plugin('order.css','visa')}
...
<div class="order-show-item">
    <div class="order-pdt-info">
        ...
        <ul class="pdt-info-list">
            ...
            <li><span class="type"></span><span class="data">{Currency_Tool::symbol()}{$info['price']}×{$info['dingnum']}</span></li>
            {/if}
        </ul>
            ...
            <span class="btn-block">
             ...
             {if $info['status']==5 && $info['ispinlun']!=1}
              ...
              {/if}
                      ...
                      {if $info['status']==2}
               <a href="{$cmsurl}visa/member/order_refund?ordersn={$info['ordersn']}" data-ajax="false" class="btn btn-blue refund_btn">申请退款</a>
              {/if}
                 {if $info['status']==6}
    ...
    $(document).ready(function(){
        ...
        $('.refund_cancel_btn').click(function () {
            ...
            layer.open({
                ...
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(SITEURL+'visa/member/ajax_order_refund_back', {ordersn:ordersn}, function (result) {
                        parent.layer.open({
