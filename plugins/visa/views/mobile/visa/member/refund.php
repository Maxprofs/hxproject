...
<head>
    ...
    {Common::css('base.css')}
    {Common::css_plugin('refund.css','visa')}
...
<!-- 公用顶部 -->
<form id="frm" bottom_strong=71QzDt >
...
<script type="text/javascript">
    ...
    function refund_status(data) {
        $.post('{$cmsurl}visa/member/ajax_order_refund', data, function (result) {
            parent.layer.open({
                content: result.message,
                btn: ['{__("OK")}'],
                end:function(){
                    window.location = '{$cmsurl}visa/member/orderview?ordersn={$info['ordersn']}';
                }
            });
        }, 'json');
    }
