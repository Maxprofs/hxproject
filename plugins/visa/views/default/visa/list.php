...
<!doctype html>
<html>
<head table_border=BQJIAj >
<meta charset="utf-8">
    <title>{$searchtitle}-{$GLOBALS['cfg_webname']}</title>
    {if !empty($country['keyword'])}
    <meta name="keywords" content="{$country['keyword']}"/>
    {/if}
      ...
      <div class="st-main-page">
        ...
        <div class="list_headimg" style="background: url({$country['banner']}) center no-repeat;">
          <div class="gj_txt">
            <dl>
              <dt>
                <img src="{Common::img($country['nation_flag'],62,62)}" alt="{$country['name']}" />
        ...
        <div class="visa_list_box">
          ...
          <ul>
            {loop $list $v}
                <li>
                  <div class="pic"><a href="{$v['url']}" target="_blank"><img src="{Product::get_lazy_img()}" st-src="{Common::img($v['litpic'],200,136)}" /><em class="attr-ico01"><b>{$v['visatype']}</b></em></a></div>
                  <div class="txt_con">
                    <p class="p1">
