...
<table class="content-tab" margin_padding=tPOzDt >
    <tr>
    ...
    function () {
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL+'visa/admin/order/index/action/read/',  //读取数据的URL
                    update: SITEURL+'visa/admin/order/index/action/save/',
                    destroy: SITEURL+'visa/admin/order/index/action/delete/'
                },
                reader: {
                    type: 'json',   //获取数据的格式
                    root: 'lists',
...
$(function(){
    ...
    $(".btn_report").click(function(){
        var url=SITEURL+"visa/admin/order/dataview/typeid/"+typeid;
        ST.Util.showBox(channelname+'订单数据报表查看',url,860,510,function(){});

    })
    //导出excel
    $(".btn_excel").click(function(){
        var url=SITEURL+"visa/admin/order/excel/typeid/"+typeid;
        ST.Util.showBox(channelname+'订单生成excel',url,560,380,function(){});
    })

    //获取当前产品订单常规信息
    $.getJSON(SITEURL+'visa/admin/order/ajax_sell_info/typeid/'+typeid,function(data){
...
function updateField(ele, id, field, value, type) {
    ...
    Ext.Ajax.request({
        url: SITEURL+"visa/admin/order/index/action/update/",
        method: "POST",
        datatype: "JSON",
...
function view(id,typeid)
{
    ...
    var url=SITEURL+"visa/admin/order/view/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}id/"+id+"/typeid/"+typeid;
    ST.Util.addTab('签证订单：'+record.get('ordersn'),url,1);
}
