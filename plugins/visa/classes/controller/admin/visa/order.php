...
class Controller_Admin_Visa_Order extends Stourweb_Controller
{
    private $_typeid = 8;
    /*
     * 订单总控制器
    ...
    public function action_index()
    {
        ...
        if (empty($action))  //显示列表
        {
            ...
            $this->display('admin/visa/order/list');
        }
        else if ($action == 'read')    //读取列表
        {
            ...
            $status=DB::select()->from('visa_order_status')->where('is_show','=',1)->execute()->as_array();
        ...
        else if ($action == 'delete') //删除某个记录
        {
            ...
            if (is_numeric($id)) //
            {
                ...
                if($order['status'] < 3)
                {
                    ...
                    Model_Visa::storage($order['suitid'],$dingnum,strtotime($order['userdate']));
                }
    ...
    public function action_view()
    {
        ...
        //套餐信息
        //$suit = DB::select()->from('hotel_room')->where('id','=',$info['suitid'])->execute()->current();
        $status = DB::select()->from('visa_order_status')->where('is_show', '=', 1)->order_by('displayorder', 'asc')->execute()->as_array();
        foreach ($status as $v)
        {
        ...
        $this->display('admin/visa/order/view');
    }
    ...
    public function action_ajax_save()
    {
        ...
        if ($model->saved())
        {
           ...
           $status = Model_Member_Order::back_order_status_changed($oldstatus,$order,$product_model);
            //价格更新,更新计算价格
            if($old_price * 100 != $_POST['price'] * 100)
            {
                Model_Member_Order_Compute::update($model->ordersn);
            }
    ...
    public function action_dataview()
    {
        ...
        $this->assign('typeid', $this->_typeid);
        $this->display('admin/visa/order/data_view');
    }
    /*
     * 异步获取相关统计数据
     * */
    ...
    public function action_excel()
    {
        ...
        $this->display('admin/visa/order/excel');
    }
