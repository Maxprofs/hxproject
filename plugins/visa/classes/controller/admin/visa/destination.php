 ...
 * 线路目的地控制器
 */
class Controller_Admin_Visa_Destination extends Stourweb_Controller
{
    ...
    public function action_destination()
    {
        ...
        $product_dest_table = 'svisa_' . $model_info['pinyin'] . '_kindlist';
        $this->assign('model_info', $model_info);
        if (empty($action))
        {
            $position = $model_info['modulename'] . '目的地';
            $this->assign('position', $position);
            $this->display('admin/visa/destination/destination');
        }
        else if ($action == 'read')
        {
            ...
            $sql = "select a.id,a.kindname,if(find_in_set($this->_typeid,opentypeids),1,0) as isopen,a.pinyin,a.iswebsite,{$bfields} from svisa_destinations a left join $product_dest_table b on a.id=b.kindid where a.pid=$pid and a.isopen=1";
        ...
        else
        {
            ...
            else
            {
                if ($action == 'search')
                {
                    ...
                    $list = DB::query(Database::SELECT, "select id,pid from svisa_destinations where kindname like '%{$keyword}%'")->execute()->as_array();
                    $new_arr = array();
                    foreach ($list as $k => $v)
                    {
                        $temp_arr = array();
    ...
    */
    public function action_ajax_getNextDestList()
    {
        ...
        if ($keyword)
        {
            $sql = "select id,kindname,pinyin from svisa_destinations where kindname like '%{$keyword}%' order by pinyin asc";
        }
        else
        {
            $sql = "select id,kindname,pinyin from svisa_destinations where pid=$pid  order by pinyin asc";
        }
    ...
    */
    public function action_ajax_getDestsetList()
    {
        ...
        if ($keyword)
        {
            $sql = "select id,kindname,pinyin from svisa_destinations where kindname like '%{$keyword}%' and isopen=1  order by pinyin asc";
        }
        else
        {
            $sql = "select id,kindname,pinyin from svisa_destinations where pid=$pid and isopen=1  order by pinyin asc";
        }
        $destlist = DB::query(Database::SELECT, $sql)->execute()->as_array();
        foreach ($destlist as $key => $row)
        {
            $sql = "select count(*) as num from svisa_destinations where pid='{$row['id']}' and isopen=1";
    ...
    public function action_dialog_productinfo()
    {
        ...
        $this->display('admin/visa/destination/dialog_productinfo');
    }
}