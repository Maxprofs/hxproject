...
class Controller_Index extends Stourweb_Controller
{
    ...
    private function _seed_admin_msg($phone)
    {
        ...
        $admin_arr = DB::select('phone')->from('admin')->where(DB::expr("FIND_IN_SET('free_kefu',notice_other)"),'>',0)->execute()->as_array();

        if(empty($admin_arr) || count($admin_arr)==0)
        {
            return false;
        }
        $cfg_webname = functions::get_sys_para("cfg_webname");//网站名称
        $login_user=Model_Member_Login::check_login_info();
        $member_id = intval($login_user['mid']);
        if($member_id)
        {
        ...
        foreach($admin_arr as $admin)
        {
            ...
            {
                SMSService::send_msg($admin['phone'], '', $content);
            }
        }
        return true;
