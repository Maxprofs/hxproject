...
//        $keyword[] = $params['keyword'];
     ...
     */
    public static function get_search_model($seachkeys,$keyword,$typeid)
    {
        $out = array();
        $has_search = array();
        if(ISMOBILE)
     ...
     * @function 获取第一个热搜词
     */
    public static function get_hot_search_default()
    {
        return DB::select('keyword')->from('search_keyword')
            ->where('isopen','=',1)
     ...
     * @param $keywrod
     */
    public static function search_jump($keywrod,$platform='pc')
    {
        if($platform=='mobile')
        {

            $models = Model_Model::get_wap_search_model();

        }
        else
        {
            $models = Model_Model::get_search_model();
        }
        $models = $models[0];

        if($models['typeid']>200)
        {
            $pinyin = 'general/'.$models['pinyin'];

        }
        else
        {
            $pinyin = $models['pinyin'];
        }
        if($pinyin)
        {
            $url = Common::get_main_host().'/query/'.$pinyin.'?keyword='.$keywrod;
            Common::head301($url);
        }
