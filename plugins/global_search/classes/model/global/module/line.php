...
class Model_Global_Module_Line extends ORM
{
    ...
    public static function search_list($params,$p,$pagesize,$where,$model)
    {
        ...
        $sql = "select a.*,(LENGTH(a.title)-LENGTH(REPLACE(a.title,'{$params['keyword']}',''))) as order_by from sline_line as a LEFT JOIN `sline_allorderlist` b ON (a.id=b.aid and b.typeid=1) WHERE $where
