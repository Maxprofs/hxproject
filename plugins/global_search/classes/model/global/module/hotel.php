...
class Model_Global_Module_Hotel extends ORM
{
    ...
    public static function search_list($params,$p,$pagesize,$where,$model)
    {
        ...
        $sql = "select a.*,(LENGTH(a.title)-LENGTH(REPLACE(a.title,'{$params['keyword']}',''))) as order_by from sline_hotel as a  LEFT JOIN `sline_allorderlist` b ON (a.id=b.aid and b.typeid=2)  
 $extwhere WHERE $where ORDER BY order_by desc, IFNULL(b.displayorder,9999) ASC,a.modtime DESC,a.addtime DESC limit $offset,$pagesize";
