...
class Model_Global_Module_Notes extends ORM
{
    ...
    public static function search_list($params,$p,$pagesize,$where,$model)
    {
        ...
        $files = 'a.*';
        $sql = "select $files,(LENGTH(a.title)-LENGTH(REPLACE(a.title,'{$params['keyword']}',''))) as order_by from sline_notes as a  WHERE {$where} ORDER BY order_by desc,a.modtime DESC,a.addtime DESC limit {$offset},{$pagesize}";
