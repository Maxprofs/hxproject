...
class Model_Global_Module_News extends ORM
{
    ...
    public static function search_list($params,$p,$pagesize,$where,$model)
    {
        ...
        $sql = "select a.*,(LENGTH(a.title)-LENGTH(REPLACE(a.title,'{$params['keyword']}',''))) as order_by from sline_news as a LEFT JOIN `sline_allorderlist` b ON (a.id=b.aid and b.typeid={$params['typeid']}) WHERE $where
...
//            }
            ...
            if(ISMOBILE==1)
            {
                $v['url'] = Model_News::show_url($v['aid'],0);
            }
            else
            {
