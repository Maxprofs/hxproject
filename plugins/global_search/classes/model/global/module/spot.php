...
class Model_Global_Module_Spot extends ORM
{
    ...
    public static function search_list($params,$p,$pagesize,$where,$model)
    {
        ...
        $files = 'a.*';
        $sql = "select $files,(LENGTH(a.title)-LENGTH(REPLACE(a.title,'{$params['keyword']}',''))) as order_by from sline_spot as a LEFT JOIN `sline_allorderlist` b ON (a.id=b.aid and b.typeid=5) WHERE $where
