...
class Model_Global_Module_Tuan extends ORM
{
    ...
    public static function search_list($params,$p,$pagesize,$where,$model)
    {
        ...
        $files = 'a.*';
        $sql = "select $files,(LENGTH(a.title)-LENGTH(REPLACE(a.title,'{$params['keyword']}',''))) as order_by from sline_tuan as a LEFT JOIN `sline_allorderlist` b ON (a.id=b.aid and b.typeid=13) WHERE $where
