...
class Controller_Admin_Hotel_Order extends Stourweb_Controller
{
    ...
    public function action_index()
    {
        ...
        else if ($action == 'read')    //读取列表
        {
            ...
            $status=DB::select()->from('hotel_order_status')->where('is_show','=',1)->execute()->as_array();
    ...
    public function action_view()
    {
        ...
        $status = DB::select()->from('hotel_order_status')->where('is_show', '=', 1)->order_by('displayorder', 'asc')->execute()->as_array();
        foreach ($status as $v)
        {
        ...
        //计算几晚
        $diff = St_Time::time_span(strtotime($info['usedate']),strtotime($info['departdate']));
        $night = $diff['day'] ? $diff['day'].'晚' : '';
    ...
    public function action_ajax_save()
    {
        ...
        $product_model = 'Model_Hotel';
        $status = DB::select()->from('hotel_order_status')->where(DB::expr('status in (' . $model->status . ',' . $newstatus . ')'))->and_where('is_show', '=', 1)->order_by('displayorder', 'desc')->execute()->as_array();
