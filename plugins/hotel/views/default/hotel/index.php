              ...
              <div class="mdd-city-box" id="mdd-city-box">
                ...
                <div class="ss-mdd-box">
                	<div class="st-tabnav">
                  	...
                  	<span data-id="h,i,j,k">HIJK</span>
                  	<span data-id="l,m,n">LMN</span>
                  	<span data-id="o,p,q,r">OPQR</span>
                  	<span data-id="s,t,w">STW</span>
                  	<span data-id="x,y,z">XYZ</span>
                  </div>
          ...
          <div class="bd">
            <ul>
                {loop $hotelad['aditems'] $v}
                    <li><a href="{$v['adlink']}" target="_blank"><img src="{Product::get_lazy_img()}" original-src="{Common::img($v['adsrc'],768,324)}" alt="{$v['adname']}" /></a></li>
                {/loop}
            </ul>
          </div>
          <!--前/后按钮代码-->
        ...
        <!--酒店首页轮播图结束-->
          ...
          <div class="hotel-tj-box">
             ...
             {st:hotel action="query" flag="order" row="3"}
                {loop $data $row}
                 <li>
                    <div class="pic"><img src="{Product::get_lazy_img()}" st-src="{Common::img($row['litpic'],400,271)}" alt="{$row['title']}" title="{$row['title']}" /></div>
                    <div class="txt">
          	...
          	<ul>
                {st:hotel action="query" flag="order" row="8" offset="3"}
                 ...
                 {loop $data $row}
                    ...
                    <div class="pic"><a href="{$row['url']}" title="{$row['title']}"><img src="{Product::get_lazy_img()}" st-src="{Common::img($row['litpic'],277,188)}" alt="{$row['title']}" /></a></div>
                    <div class="txt">
                        <p class="bt"><a href="{$row['url']}" target="_blank" title="{$row['title']}">{$row['title']}</a><span>{if !empty($row['price'])}<i class="currency_sy">{Currency_Tool::symbol()}</i><b>{$row['price']}</b>起{else}电询{/if}</span></p>
