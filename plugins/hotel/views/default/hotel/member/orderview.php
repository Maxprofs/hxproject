    ...
    <div class="os-term">
        ...
        <div class="os-block">
            <div class="order-show-info">
                ...
                <table width="100%" class="order-show-table" border="0" bottom_float=Ykuokk >
                    <tr>
                        <th width="35%" height="40" scope="col"><span class="l-con">{__('产品名称')}</span></th>
                        <th width="15%" scope="col">{__('入住日期')}</th>
                        <th width="15%" scope="col">{__('离店日期')}</th>
                        <th width="10%" scope="col">{$night}{__('合计')}</th>
                        <th width="10%" scope="col">{__('房间数量')}</th>
...
<script>
    $(function () {
        ...
        $("#apply-refund-Click").on("click", function () {
            parent.layer.open({
                ...
                title: "申请退款",
                area: ['560px','570px'],
                content: '{$cmsurl}hotels/member/order_refund?ordersn={$info['ordersn']}',
                btn: ['确认', '取消'],
                btnAlign: 'C',
                closeBtn: 0,
    ...
    function refund_status(data) {
        $.post('{$GLOBALS["cfg_basehost"]}/hotels/member/ajax_order_refund', data, function (result) {
            parent.layer.open({
