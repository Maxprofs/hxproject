...
<head>
    ...
    {Common::css('base.css')}
    {Common::css_plugin('order.css','hotel')}
    ...
    <div class="order-pdt-info">
        <a class="pdt-info-tw" href="{$product['url']}">
            ...
            <div class="con">
                ...
                <p class="date">{$info['usedate']}住店</p>
                <p class="date">{$info['departdate']}离店</p>
            </div>
        </a>
        <ul class="pdt-info-list">
            {if !empty($info['price']) && !empty($info['dingnum'])}
            <li><span class="type">{$suit['roomname']}</span><span class="data">{Currency_Tool::symbol()}{$info['price']}×{$info['dingnum']}间</span></li>
            {/if}
        </ul>
        {if !empty($info['iscoupon']) || $info['usejifen']}
        <ul class="pdt-info-list">
            {if !empty($info['iscoupon'])}
            <li><span class="type">优惠券</span><span class="data">{Currency_Tool::symbol()}{$info['iscoupon']['cmoney']}（{$info['iscoupon']['name']}）</span></li>
            {/if}
            {if $info['usejifen']}
        ...
        <span class="price">{if $info['paytype']==2}定金支付{else}应付总额{/if}：<em class="num">{Currency_Tool::symbol()}{$info['payprice']}</em></span>
            ...
            <span class="btn-block">
             ...
             {if $info['status']==5 && $info['ispinlun']!=1}
              ...
              {/if}
                 {if $info['status']==2}
               <a href="{$cmsurl}hotels/member/order_refund?ordersn={$info['ordersn']}" data-ajax="false" class="btn btn-blue refund_btn">申请退款</a>
              {/if}
                 {if $info['status']==6}
    ...
    $(document).ready(function(){
        ...
        $('.refund_cancel_btn').click(function () {
            ...
            layer.open({
                ...
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(SITEURL+'hotels/member/ajax_order_refund_back', {ordersn:ordersn}, function (result) {
                        parent.layer.open({
