<div class="booking-info-block clearfix" id="discount_con">
    ...
    <div class="block-item">
        <ul>
            ...
            <li class="jifen-con">
                <div>
                    <p class="jf-use">使用<input type="number" id="pub_needjifen" class="jf-num">积分，抵现<span id="txt_jifentprice"></span>元</p>
            ...
            <li id="coupon_content">
                <a class="all" href="#useCoupon">
                    <span class="item-hd">优惠券</span>
                    {if count($couponlist)>0}
                        <span class="more-type">
                            <em class="org">{count($couponlist)}</em>张可用<i class="more-ico"></i>
                        </span>
                    {else}
                        <span class="more-type">无可用的优惠券<i class="more-ico"></i></span>
                    {/if}
                </a>
            </li>
            {/if}
            ...
            <li class="hide">
                ...
                <span class="more-type"><em class="org">1</em>张可用<i class="more-ico"></i></span>
            </li>
            {/if}
            {/if}
        </ul>
...
<script>
    ...
    function clear_coupon()
    {
        ...
        {if empty($couponlist)}
            $('.coupon_type').html('<strong class="no-style">优惠券</strong><em class="type no-style">暂无可用优惠券</em>');
            {else}
            $('.coupon_type').html('<strong class="no-style">优惠券</strong>');
            {/if}
            }
            //重设积分,即积分置0
