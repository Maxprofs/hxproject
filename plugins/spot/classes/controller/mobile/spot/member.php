...
class Controller_Mobile_Spot_Member extends Stourweb_Controller
{
    /*
     * 线路订单总控制器
     * */
    ...
    public function action_orderview()
    {
        ...
        //$info = DB::select()->from('spot')->where('id', '=', $info)->execute()->current();

        $product = St_Product::get_product_info($info['typeid'],$info['productautoid']);
        $suit = Model_Spot_Suit::suit_info($info['suitid']);
        $this->assign('product',$product);
        $this->assign('suit',$suit);
        $status=DB::select()->from('spot_order_status')->where('status','=',$info['status'])->and_where('is_show','=',1)->execute()->current();
        $info['statusname']=$status['status_name'];
        $info = Pay_Online_Refund::get_refund_info($info);
        //var_dump($info);exit;
        $this->assign('info', $info);
        $this->assign('issystem', $issystem);
        $this->display('../mobile/spot/member/orderview');
    }
    ...
    public function action_order_refund()
    {
        ...
        $this->assign('info',$info);
        $this->display('../mobile/spot/member/refund');
    }
    ...
    {
        $result = Pay_Online_Refund::apply_order_refund($_POST,$this->_mid,'Model_Spot');
    ...
    public function action_ajax_order_refund_back()
    {
        ...
        $result = Pay_Online_Refund::order_refund_back($ordersn,$this->_mid,'Model_Spot');
        echo json_encode($result);
