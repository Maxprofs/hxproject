 ...
 * 景点目的地控制器
 */
class Controller_Admin_Spot_Destination extends Stourweb_Controller
{
    ...
    public function action_destination()
    {
        ...
        if (empty($action))
        {
            ...
            $this->display('admin/spot/destination/destination');
        }
        else if ($action == 'read')
        {
            ...
            $sql = "select a.id,a.kindname,if(find_in_set($this->_typeid,a.opentypeids),1,0) as isopen,a.pinyin,a.iswebsite,{$bfields} from sline_destinations a left join $product_dest_table b on a.id=b.kindid where a.pid=$pid and a.isopen=1";
    ...
    */
    ...
    public function action_dialog_productinfo()
    {
        ...
        $this->display('admin/spot/destination/dialog_productinfo');
    }
    ...
    public function action_seo()
    {
        ...
        $this->assign('typeid', $typeid);
        $this->assign('info', $info);
        $this->display('admin/spot/destination/seo');
    ...
    public function action_ajax_save()
    {
        ...
        $flag = DB::update('spot_kindlist')->set($row)->where('kindid', '=', $id)->execute();
        if ($flag)
        {
    ...
    public function action_set_template()
    {
        ...
        $templatelist = array_reverse(Model_Templet::get_templet_list_by_page($webtype, 'spot_list', 0));

        $info = DB::select()->from('spot_kindlist')->where('kindid', '=', $id)->execute()->current();
        $this->assign('templatelist', $templatelist);
        $this->assign('info', $info);
        $this->assign('webtype', $webtype);
        $this->display('admin/spot/destination/set_template');
    }
    ...
    public function action_ajax_save_template()
    {
        ...
        DB::update('spot_kindlist')->set($data)->where('kindid','=',$kindid)->execute();
        $out = array();
        $out['id'] = $kindid;
