...
/**
 * Class Controller_Admin_Spot_Attrid
 * 景点属性管理
 */
class Controller_Admin_Spot_Attrid extends Stourweb_Controller
{
    private $_typeid = 5;
    ...
    public function action_list()
    {
        ...
        $this->assign('chooseid','attr_attr');
        ...
        if (empty($action))
        {
            ...
            $this->display('admin/spot/attrid/list');
        }
        else if ($action == 'read')
        {
            ...
            if ($node == 'root')//属性组根
            {
                ...
                $list[] = array(
                    'leaf' => true,
                    'id' => '0add',
                    'attrname' => '<button class="btn btn-primary radius size-S" onclick="addSub(0)">添加</button>',
                    'allowDrag' => false,
                    'allowDrop' => false,
                    'displayorder' => 'add'
                );
            ...
            else //子级
            {
                ...
                $list[] = array(
                    'leaf' => true,
                    'id' => $node . 'add',
                    'attrname' => '<button class="btn btn-primary radius size-S" onclick="addSub(\'' . $node . '\')">添加</button>',
                    'allowDrag' => false,
                    'allowDrop' => false,
                    'displayorder' => 'add'
                );
    ...
    public function action_config()
    {
        ...
        $this->assign('typeid', $typeid);
        $this->display('admin/spot/attrid/config');
    }
    ...
    public function action_extendlist()
    {
        ...
        if (empty($action))
        {
            ...
            $this->assign('typeid', $typeid);
            $this->assign('channelname', $channelname);
            $this->display('admin/spot/attrid/extendfield');
        }
        else if ($action == 'read')
        {
        ...
        {
            $this->display('admin/spot/attrid/addfield');
        }
        else if ($action == 'delete')
        {
            ...
            if (is_numeric($id))
            {
                ...
                //清理扩展字段
                DB::delete('spot_content')->where('columnname','=',$fieldname)->execute();
            }
        }
    ...
    public function action_content()
    {
        ...
        if (empty($action))
        {
            ...
            $this->display('admin/spot/attrid/content');
        }
        else if ($action == 'save')
        {
    ...
    public function action_dialog_delfield()
    {
        ...
        $this->assign('id', $id);
        $this->display('admin/spot/attrid/dialog_delfield');
    }
    ...
    public function action_attr_num_config()
    {
        $fields = array('cfg_spot_attr_show_num');
        $config = Model_Sysconfig::get_configs(0, $fields);
        $this->assign('config', $config);
        $this->assign('chooseid','attr_numconfig');//顶部选中
        $this->display('admin/spot/attrid/attr_num_config');
