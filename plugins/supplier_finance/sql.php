...
if(!$mysql->check_table("sline_supplier_finance_drawcash"))
{
    $mysql->query("CREATE TABLE `sline_supplier_finance_drawcash` (
...
{
    //更新字段
    if(!$mysql->check_column('sline_supplier_finance_drawcash','proceeds_type'))
    {
        $mysql->query("ALTER TABLE `sline_supplier_finance_drawcash` ADD COLUMN `proceeds_type` tinyint(1) DEFAULT '1' COMMENT '收款方类型:1银行 2支付宝' AFTER `withdrawamount`");
        $mysql->error();
    }
    if(!$mysql->check_column('sline_supplier_finance_drawcash','alipayaccount'))
    {
        $mysql->query("ALTER TABLE `sline_supplier_finance_drawcash` ADD COLUMN `alipayaccount` varchar(255) DEFAULT NULL COMMENT '支付宝账号' AFTER `bankaccountname`");
        $mysql->error();
    }
    if(!$mysql->check_column('sline_supplier_finance_drawcash','alipayaccountname'))
    {
        $mysql->query("ALTER TABLE `sline_supplier_finance_drawcash` ADD COLUMN `alipayaccountname`  varchar(255) DEFAULT NULL COMMENT '支付宝账户姓名' AFTER `alipayaccount`;");
        $mysql->error();
    }
}
