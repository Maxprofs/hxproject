...
class Model_Member_Order_Extend extends ORM
{
     ...
     * @param $pageno
     */
    public static function get_order_list($supplierid, $query_type=null, $query_id=null, $typeid=null, $starttime=null, $endtime=null, $settle_status=null, $status=null, $pagesize=999999, $pageno=1)
    {
        $sql = " SELECT * FROM sline_member_order a ";
        $where = "AND a.supplierlist ='{$supplierid}' ";
        //查询类型
        if($query_type==1)
        {
            //产品
     ...
     * 通过审核状态 获取体现的金额
     */
    public static function get_withdrawed_amount($supplierid, $status)
    {
        $sql = "SELECT SUM(withdrawamount) withdrawamount FROM sline_supplier_finance_drawcash WHERE supplierid='{$supplierid}' and status='{$status}'";
     ...
     **********************************************************************/
//获取筛选列表
    public static function get_query_list($supplierid, $category, $typeid=null, $keyword=null, $pagesize=10, $pageno=1 )
    {
        ...
        if($category==1)
        {
            ...
            $dbobj->where('supplierlist','=',$supplierid);
            if(!empty($keyword))
            {
                $num = intval($keyword);
     ...
     * 获取财务总览信息
     */
    public static function get_overview_info($supplierid)
    {
        $list_info = Model_Member_Order_Extend::get_order_list($supplierid);
        ...
        $withdraw = Model_Member_Order_Extend::get_withdrawed_amount($supplierid, 1); //已提现金额
        $withdrawing = Model_Member_Order_Extend::get_withdrawed_amount($supplierid, 0);//提现中金额
