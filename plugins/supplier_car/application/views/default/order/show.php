                ...
                <div class="order-msg">
                    ...
                    <li><strong>收货地址：</strong>{$info['receiver_address']}</li>
                  	...
                  	<li><strong>订单状态：</strong>
                        ...
                        {if !in_array($info['status'],Model_Member_Order::$changeableStatus)}
                            {if $info['status']==1}
                            ...
                            {/if}
                        {else}
                        <select onchange="change_status(this,'{$info['ordersn']}')">
                            {loop $statusarr $k=>$status}
                            <option value="{$k}" {if $k==$info['status']}selected="selected"{/if}  >{$status}</option>
                            {/loop}
                        </select>
                        {/if}
