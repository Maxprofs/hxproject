...
$(function(){
    ...
    $("#choose-btn").click(function(){
        ...
        {
            data = get_search_list(category, typeid, '',1);
            full_product_list('distributor', data, pagesize, pageno);
        }
        else if(category==3)
        {
            data = get_search_list(category, typeid, '',1);
    ...
    //供应商搜索条件展示列表
    $(".search-btn-distributor").click(function (e, param) {
        var category = $("#category").val();
        var typeid = '';
        var keyword = $(".search-text-distributor").val();
        var pagesize = 10;
        var pageno = typeof(param) == 'undefined' ? 1 : param;
        var data = get_search_list(category, typeid, keyword,pageno);
        full_product_list('distributor', data, pagesize, pageno);
    });
    $(document).on("click",".pm-btm-msg-distributor a",function(){
        var $this = $(this);
        var pageno = $this.attr('data-pageno')-0;
        if($this.hasClass('current'))
        {
        ...
        $(".search-btn-distributor").trigger("click",pageno);
    });
    //分销商搜索条件展示列表
    ...
    $(".sure-btn").click(function(){
        ...
        var distributor_name = $tr.find('td:eq(0)').text();
        ...
        if(link_name != '' && phone != '')
        {
            s_obj.title = distributor_name+' | '+link_name+' | '+phone;
        }else if(link_name == '' && phone != '')
        {
            s_obj.title = distributor_name+' | '+phone;
        }else if(link_name == '' && phone == ''){
            s_obj.title = distributor_name;
        }else{
            s_obj.title = distributor_name+' | '+link_name;
        }
    ...
    $("#export_excel").click(function(){
        ...
        var url = SITEURL+'pc/financeextend/ordercount_export_excel/?fields='+p+'&type='+type+'&typeid='+typeid+'&id='+id +
            "&starttime="+starttime+"&endtime="+endtime+"&order_status="+order_status+"&settle_status="+settle_status;
        console.info(url);
        window.open(url);
    })
    ...
    $('.module-select').change(function () {
        ...
        if($this.val()==-1)
        {
            $("#li_product_name").hide();
        }
        else
        {
            $("#li_product_name").show();
        }
...
//category, typeid, id, starttime, end
    ...
    var url = SITEURL+'pc/financeextend/ajax_ordercount_list';
    $.get(url,{
        category: category,
        typeid: typeid,
        id: id,
...
{
    var rtn=undefined;
    var url = SITEURL+"pc/financeextend/ajax_ordercount_query_list";
    $.ajax({
        type:"GET",
        url:url,
        data:{
