...
<div class="user-content-wrap fr" style="overflow:hidden">
    ...
    <div class="os-term">
        ...
        <div class="os-block">
            <div class="order-show-cheap">
                <ul>
                    <li><em>{__('支付方式')}：</em>{$info['paytype_name']} &nbsp;{if $info['paytype']==2}({$info['dingjin']}/{__('人')}){/if}</li>
        ...
        <div class="os-block">
            <div class="order-show-info">
                ...
                <table width="100%" border="0" class="order-show-table">
                    <tr>
                        <th width="39%" height="40" scope="col"><span class="l-con">{__('产品名称')}</span></th>
                        <th width="15%" scope="col">{__('出行日期')}</th>
                        <th width="11%" scope="col">{__('类型')}</th>
                        <th width="11%" scope="col">{__('单价')}</th>
                        <th width="11%" scope="col">{__('人数')}</th>
                        <th width="11%" scope="col">{__('总价')}</th>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>{$info['childnum']}</td>
                        <td><span class="jg"><i class="currency_sy">{Currency_Tool::symbol()}</i>{php}echo $info['childprice'] * $info['childnum'];{/php}</span></td>
                    ...
                    <tr>
                        ...
                        <td>{$info['oldprice']}</td>
                        <td>{$info['oldnum']}</td>
                        <td><span class="jg"><i class="currency_sy">{Currency_Tool::symbol()}</i>{php}echo $info['oldprice'] * $info['oldnum'];{/php}</span></td>
                    </tr>
                    {/if}
                    {if !empty($info['roombalancenum'])}
                    <tr>
                        <td height="40"><span class="l-con">{__('单房差')}({if $info['roombalance_paytype']==1}{__('预付')}{else}{__('到店付')}{/if})</span></td>
                        <td>--</td>
                        <td>{__('单房差')}</td>
                        <td>{$info['roombalance']}</td>
                        <td>{$info['roombalancenum']}</td>
                        <td><span class="jg"><i class="currency_sy">{Currency_Tool::symbol()}</i>{php} if($info['roombalance_paytype']!=1) echo 0;else{ echo $info['roombalancenum']*$info['roombalance'];}{/php}</span></td>
                    </tr>
                    {/if}

                    {if $new_insurs}
                    <tr>
                        <td height="40"><span class="l-con">{$new_insurs['title']}</span></td>
                        <td>{$new_insurs['usedate']}</td>
                        <td>保险</td>
                        <td>{$new_insurs['price']}</td>
                        <td>{$new_insurs['dingnum']}</td>
                        <td><span class="jg"><i class="currency_sy">{Currency_Tool::symbol()}</i>{php}echo $new_insurs['price'] * $new_insurs['dingnum'];{/php}</span></td>
                    </tr>
                    {/if}
                </table>
            </div>
...
<script>
    ...
    function check_refund_frm(frm_data)
    {
        ...
        var platform = $(frm_data).find('input[name=platform]').val();
        if(refund_auto!=1)
        {
            if(platform=='alipay')
            {
...
<script>
    ...
    function refund_status(data) {
        $.post('{$GLOBALS["cfg_basehost"]}/hotels/member/ajax_order_refund_back', data, function (result) {
            parent.layer.open({
                content: result.message,
                btn: ['{__("OK")}'],
                end:function(){
