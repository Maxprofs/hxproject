 ...
 */
class Taglib_Line {
	private static $basefiled = 'a.id,a.webid,a.aid,a.title,a.storeprice,a.litpic,a.startcity,a.kindlist,a.attrid,a.bookcount,a.storeprice,a.sellpoint,a.iconlist,a.satisfyscore,a.color,a.price_date,a.price';
	...
	public static function query($param) {
		...
		case 'relative':
			...
			break;
		case 'tagrelative':
			$list = self::get_line_bytagword($offset, $row, $tagword);
			break;
		case 'theme':
		...
		if ($changecity == 1) {
			foreach ($list as $key => $value) {
				if ($value['startcity'] != $GLOBALS['cityid']) {
					unset($list[$key]);
				}
			}
		}
		foreach ($list as &$v) {
			...
			$v['satisfyscore'] = St_Functions::get_satisfy(1, $v['id'], $v['satisfyscore']); //满意度
	...
	public static function day_list($params) {
		...
		$params = array_merge($default, $params);
		extract($params);
		$suit = ORM::factory('line_day')->order_by('word', 'asc')->get_all();
		$autoindex = 1;
		foreach ($suit as &$r) {
	...
	private static function get_line_bytagword($offset, $row, $tagword) {
		$offset = intval($offset);
		$row = intval($row);
