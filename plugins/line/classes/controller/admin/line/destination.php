...
class Controller_Admin_Line_Destination extends Stourweb_Controller
{
    ...
    {
        parent::before();
        $channel_name = DB::select('shortname')->from('nav')->where('typeid', '=', $this->_typeid)->execute()->get('shortname');
    ...
    public function action_destination()
    {
        ...
        if (empty($action)) {
            ...
            $templatelist = Model_Line::get_template_list('line_list', 0);
            $node = Arr::get($_GET, 'node');
            $node_arr = explode('_', $node);
            $pid = $node_arr[1] == 'root' ? 0 : $node_arr[1];
            $bfields = 'b.kindid,b.seotitle,b.keyword,b.description,b.tagword,b.jieshao,b.isfinishseo,b.isnav,b.ishot,b.displayorder,b.templetpath,b.wap_templetpath';
            $sql = "select a.id,a.kindname,if(find_in_set($this->_typeid,opentypeids),1,0) as isopen,a.pinyin,a.iswebsite,{$bfields} from sline_destinations a left join $product_dest_table b on a.id=b.kindid where a.pid=$pid and a.isopen=1";
            $list = DB::query(Database::SELECT, $sql)->execute()->as_array();
            foreach ($list as &$v) {
                if ($v['templetpath']) {
                    foreach ($templatelist['pclist'] as $temp) {
                        if ($temp['pagepath'] == $v['templetpath']) {
                            $v['templet_name'] = $temp['title'];
                        }
                    }
                }
                if ($v['wap_templetpath']) {
                    foreach ($templatelist['waplist'] as $temp) {
                        if ($temp['pagepath'] == $v['wap_templetpath']) {
                            $v['wap_templet_name'] = $temp['title'];
                        }
                    }
                }
                $subnum = DB::select(array(DB::expr("count(*)"), 'num'))->from('destinations')->where('pid', '=', $v['id'])->and_where('isopen', '=', 1)->execute()->get('num');
                if ($subnum <= 0)
                    $v['leaf'] = true;
            }
    ...
    */
    ...
    public function action_seo()
    {
        ...
        $this->assign('typeid', $typeid);
        $this->assign('info', $info);
        $this->display('admin/line/destination/seo');
    ...
    public function action_ajax_save()
    {
        ...
        $id = $data['id'];
        $row = array();
        if($data['seotitle'] && $data['keyword'] && $data['description'])
        {
            $row['isfinishseo'] = 1;
        }
        else
        ...
        $row['seotitle'] = $data['seotitle'];
        $row['tagword'] = $data['tagword'];
        $row['keyword'] = $data['keyword'];
        $row['description'] = $data['description'];
        $row['jieshao'] = $data['jieshao'];

        $flag = DB::update('line_kindlist')->set($row)->where('kindid', '=', $id)->execute();
        if ($flag)
        {
    ...
    public function action_set_template()
    {
        ...
        $templatelist = array_reverse(Model_Templet::get_templet_list_by_page($webtype, 'line_list', 0));
        $info = DB::select()->from('line_kindlist')->where('kindid', '=', $id)->execute()->current();
        $this->assign('templatelist', $templatelist);
        $this->assign('info', $info);
        $this->assign('webtype', $webtype);
        $this->display('admin/line/destination/set_template');
    }
    ...
    {
        $kindid = $_POST['kindid'];
        $templetpath = $_POST['templetpath'];
        $wap_templetpath = $_POST['wap_templetpath'];
        $data = array(
            'templetpath'=>$templetpath,
            'wap_templetpath'=>$wap_templetpath,
        );
        DB::update('line_kindlist')->set($data)->where('kindid','=',$kindid)->execute();
        $out = array();
        $out['id'] = $kindid;
        $out['templet_name'] = $_POST['templet_name'];
        $out['wap_templet_name'] = $_POST['wap_templet_name'];
