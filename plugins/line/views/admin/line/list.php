...
<head>
<meta charset="utf-8">
<title>产品列表-笛卡CMS{$coreVersion}</title>
				   ...
				   renderer : function(value, metadata,record) {
					  //  return  "<input type='checkbox' class='product_check' value='"+value+"'/>"; 
                                     ...
                                     var name=record.get('title');
                                      ...
                                      {
                                          return '<a href="javascript:;" class="btn-link" onclick="ST.Util.addTab(\'添加套餐\',\'line/admin/line/addsuit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1/lineid/'+id+'\')">添加套餐</a>';
                                      }
					},				 
				   listeners:{
					    afterrender:function(obj,eopts)
