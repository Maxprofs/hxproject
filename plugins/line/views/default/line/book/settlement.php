<div class="booking-r-container">
    <div class="booking-side-total">
        ...
        <div class="side-total-block">
            ...
            <ul class="side-total-list">
                <li>
                    ...
                    <span class="bd">{if $suitInfo['paytype']==1}全额预订{else}定金预订{/if}</span>
                </li>
                <li>
                    <span class="hd">预订时间</span>
                    <span class="bd">{date('Y-m-d')}</span>
                </li>
                <li>
                    <span class="hd">出发日期</span>
                    <span class="bd usedate">{$info['usedate']}</span>
                </li>
                <li>
                    <span class="hd">预定套餐</span>
                    <span class="bd">{$suitInfo['title']}</span>
                </li>

                <li class="settlement_adult"  {if !Common::check_instr($suitPrice['propgroup'],2) || $adultnum==0}style="display:none"{/if}>
                    <span class="hd">成人</span>
                    <span class="bd">{Currency_Tool::symbol()}<em class="txt_adultprice">{$suitPrice['adultprice']}</em>×<em class="txt_adultnum">{$adultnum}</em></span>
                </li>
                <li class="settlement_child" {if !Common::check_instr($suitPrice['propgroup'],1) || $childnum==0}style="display:none"{/if}>
                    <span class="hd">儿童</span>
                    <span class="bd">{Currency_Tool::symbol()}<em class="txt_childprice">{$suitPrice['childprice']}</em>×<em class="txt_childnum">{$childnum}</em></span>
                </li>
                <li class="settlement_old" {if !Common::check_instr($suitPrice['propgroup'],3) || $oldnum==0}style="display:none"{/if}>
                    <span class="hd">老人</span>
                    <span class="bd">{Currency_Tool::symbol()}<em class="txt_oldprice">{$suitPrice['oldprice']}</em>×<em class="txt_oldnum">{$oldnum}</em></span>
                </li>
                <li class="settlement_roombalance" {if (!$suitPrice['roombalance'])}style="display:none"{/if}>
                    <span class="hd">单房差</span>
                    <span class="bd">{Currency_Tool::symbol()}<em class="txt_roombalanceprice">{$suitPrice['roombalance']}</em>×<em class="txt_roombalancenum">{$roombalance}</em></span>
                </li>
            </ul>
        </div>
        <div class="side-total-block">
            <h4 class="side-total-tit">附加产品</h4>
            <ul class="side-total-list" id="additional">
        ...
        <div class="side-total-block">
            ...
            <ul class="side-total-list">
                <li>
                    <span class="hd">定金</span>
                    <span class="bd settle-dg-detail"></span>
                </li>
            </ul>
        </div>
        {/if}
        <div class="side-total-block side-amount-bar">
            <ul class="side-total-list">
                {if $suitInfo['paytype']==2}
                    <li>
                        <span class="hd">预定总额</span>
                        <span class="bd c-f60 order-total-price"></span>
                    </li>
                    <li class="discount-total">
                        <span class="hd">优惠总额</span>
                        <span class="bd c-f60 discount-total-price"></span>
                    </li>
                    <li class="settel-dg">
                        <span class="hd">应付总额</span>
                        <span class="bd c-f60 pay-total-price"></span>
                    </li>
                    <li class="settel-dg">
                        <span class="hd">待支付尾款</span>
                        <span class="bd c-f60 left-total-price"></span>
                    </li>
                    <li class="all-li">
                        <span class="hd all-item">应付定金</span>
                        <span class="bd all-pri dingjin-total-price"></span>
                    </li>
                {else}
                    <li>
                        <span class="hd">预定总额</span>
                    ...
                    <li class="all-li">
                        <span class="hd all-item">应付总额</span>
                        <span class="bd all-pri pay-total-price"></span>
                    </li>
                {/if}
            </ul>
        </div>
