...
<html>
	...
	<head>
        ...
        {include "pub/varname"}
        {Common::css_plugin('booking.css','line')}
		...
		<script>
            //预订送积分
            var jifenbook = {
                rewardtype:"{$jifenbook_info['rewardway']}",
                value:"{$jifenbook_info['value']}"
                    ...
                    <div class="booking-l-container">
                        ...
                        <div class="booking-info-block">
                            <div class="bib-hd-bar">
                                <h1 class="hb-title">{$info['title']}</h1>
                                <div class="hb-number">产品编号：{$info['series']}</div>
                            </div>
                            <div class="bib-bd-wrap">
                                <ul class="booking-item-block">
                                    <li>
                                        <span class="item-hd">预订套餐：</span>
                                        <div class="item-bd">
                                            <div class="b-item-text">{$suitInfo['title']}</div>
                                        </div>
                                    </li>
                                    <li>
                                        <span class="item-hd">预订方式：</span>
                                        <div class="item-bd">
                                            <div class="b-item-text">{if $suitInfo['paytype']==1}全额预订{else}定金预订{/if}</div>
                                        </div>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        ...
                                        <div class="item-bd">
                                            <table class="booking-price-table people_info">
                                                ...
                                                <tbody>
                                                {if Common::check_instr($suitPrice['propgroup'],2) && $suitPrice['adultprice']}
                                                    <tr>
                                                        <td>成人</td>
                                                        <td>
                                                            <i class="currency_sy">{Currency_Tool::symbol()}</i>
                                                            <span class="txt_adultprice">{$suitPrice['adultprice']}</span>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <span class="o-price adult_total_price">
                                                                <i class="currency_sy">{Currency_Tool::symbol()}</i>
                                                            </span>
                                                        </td>
                                                    </tr>
                                                 {/if}
                                                {if Common::check_instr($suitPrice['propgroup'],1) && $suitPrice['childprice']}
                                                    <tr>
                                                        <td>儿童</td>
                                                        <td>
                                                            <i class="currency_sy">{Currency_Tool::symbol()}</i>
                                                            <span class="txt_childprice">{$suitPrice['childprice']}</span>
                                                        </td>
                                                        <td>
                                                            <div class="booking-amount-control">
                                                                <span class="sub-btn  sub is_order_number">-</span>
                                                                <input type="text" id="child_num" name="child_num" class="number-text" readonly value="{$childnum}"/>
                                                                <span class="add-btn add is_order_number">+</span>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <span class="o-price child_total_price">
                                                                <i class="currency_sy">{Currency_Tool::symbol()}</i>
                                                            </span>
                                                        </td>
                                                    </tr>
                                                {/if}
                                                {if Common::check_instr($suitPrice['propgroup'],3) && $suitPrice['oldprice']}
                                                    <tr>
                                                        <td>老人</td>
                                                        <td>
                                                            <i class="currency_sy">{Currency_Tool::symbol()}</i>
                                                            <span class="txt_oldprice">{$suitPrice['oldprice']}</span>
                                                        </td>
                                                        <td>
                                                            <div class="booking-amount-control">
                                                                <span class="sub-btn sub is_order_number">-</span>
                                                                <input type="text" id="old_num" name="old_num" class="number-text" readonly value="{$oldnum}"/>
                                                                <span class="add-btn add is_order_number">+</span>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <span class="o-price old_total_price">
                                                                <i class="currency_sy">{Currency_Tool::symbol()}</i>{$suitPrice['oldprice']}
                                                            </span>
                                                        </td>
                                                    </tr>
                                                  {/if}
                                                {if ($suitPrice['roombalance']>0)}
                                                <tr>
                                                    <td>单房差</td>
                                                    <td>
                                                        <i class="currency_sy">{Currency_Tool::symbol()}</i>
                                                        <span class="txt_roombalancerice">{$suitPrice['roombalance']}</span>
                                                    </td>
                                                    <td>
                                                        <div class="booking-amount-control">
                                                            <span class="sub-btn sub">-</span>
                                                            <input type="text" id="roombalance_num" name="roombalance_num" class="number-text" readonly value="{$roomnum}"/>
                                                            <span class="add-btn add">+</span>
                                                        </div>
                                                    </td>
                                                    <td>
                                                            <span class="o-price roombalance_total_price">
                                                                <i class="currency_sy">{Currency_Tool::symbol()}</i>{$suitPrice['roombalance']}
                                                            </span>
                                                    </td>
                                                </tr>
                                                {/if}
                            ...
                            <div class="bib-bd-wrap">
                                <ul class="booking-item-block">
                                    <li>
                                        ...
                                        <div class="item-bd">
                                            <input type="text" class="input-text w230" name="linkman" value="{$userInfo['truename']}" placeholder="" />
                                    ...
                                    <li>
                                        ...
                                        <div class="item-bd">
                                            <input type="text" class="input-text w230" name="linktel" value="{$userInfo['mobile']}" placeholder="" />
                                    ...
                                    <li>
                                        ...
                                        <div class="item-bd">
                                            <input type="text" class="input-text w230" name="linkemail" value="{$userInfo['email']}" placeholder="" />
                                    ...
                                    <li>
                                        ...
                                        <div class="item-bd">
                                            <textarea name="remark" class="text-area"></textarea>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        ...
                        <!-- 联系人信息 -->
                        {include "line/book/tourer"}
                        <!--定金支付不享受优惠-->
                        {if $suitInfo['paytype']!=2}
                          {include "line/book/discount"}
                        {/if}
                        <div class="booking-info-block">
                            <div class="pay-info-bar">
                                {if $suitInfo['paytype'] == 2}
                                 <span class="item">应付定金：<span class="pri dingjin-total-price"></span></span>
                                {else}
                                 <span class="item">应付总额：<span class="pri pay-total-price"></span></span>
                                {/if}
                                <span class="c-999 ml10 jifenbook"></span>
                                <div class="mt10 c-999" id="left-dg-info"></div>
                            </div>
                        ...
                        <div class="booking-info-block clearfix">
                            {include "line/book/agreement"}
                            <div class="fr booking-submit-block">
                                <span class="yzm-txt">验证码：</span>
                                <img class="yzm-label" src="{$cmsurl}captcha" onClick="this.src=this.src+'?math='+ Math.random()" width="80" height="30"/>
                                <input type="text" name="checkcode" id="checkcode" maxlength="4" class="input-text w100 ml10" />

                                <a class="booking-submit-btn ml10" href="javascript:;">提交订单</a>
                            </div>
                        </div>
                        <!-- 协议、合同 -->
    ...
    <script>
            //游客信息
            var is_tourer_open = "{$GLOBALS['cfg_write_tourer']}";
			$(function() {
