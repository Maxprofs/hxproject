      ...
      <div class="st-photoshow-box">
            ...
            <div class="picshowlist">
              <div class="picshowlist_mid">
                ...
                <div class="picmidmid">
                  <ul>
                    {loop $info['piclist'] $p}
                     <li><a href="javascript:void(0);"><img src="{Common::img($p['litpic'],103,70)}" alt="" bigimg="{Common::img($p['bigpic'],890,0,true)}" /></a></li>
                    {/loop}
        ...
        <div class="photo-side-con">
          ...
          <div class="photo-comment-lsit">
          	<ul>
                {st:photo action="comment" articleid="$info['id']"}
                {loop $data $c}
                    <li>
                        <p class="tit">{$c['member']['nickname']}<a class="hf-fix cursor" data-replyid="{$c['id']}"  data-nickname="{$c['member']['nickname']}">回复</a></p>
                        {if !empty($c['dockid'])}
                        <p class="quote"><em>@{$c['reply']['nickname']}</em>{$c['reply']['content']}</p>
                        {/if}
                        <p class="txt">{$c['content']}</p>
                        <p class="date">{$c['addtime']}</p>
                    </li>
                {/loop}
            </ul>
          </div><!-- 评论列表 -->
...
<script>
    $(function(){
        $('.hf-fix').click(function(){
            $('#msg').attr('placeholder','回复 '+$(this).attr('data-nickname'));
        ...
        $(".btn_pl").click(function(){
            ...
            $.ajax({
                type:'POST',
                url:SITEURL+'photo/ajax_add_comment',
                data:{
                    productid:productid,
                    content:msg,
                    checkcode:checkcode,
