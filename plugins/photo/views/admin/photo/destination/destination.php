...
<script>
   ...
   // var otherModule={php echo json_encode($allmodule);};
        ...
        function () {
            ...
            window.dest_store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    api: {
                        read: SITEURL+'photo/admin/destination/destination/action/read',  //读取数据的URL
                        update: SITEURL+'photo/admin/destination/destination/action/save',
                        destroy: SITEURL+'photo/admin/destination/destination/action/delete'
                    },
                    reader: 'json'
                },
                autoLoad: true,
                        ...
                        tpl: new Ext.XTemplate(
                            '{[this.realName(values.id)]}',
                            {
                                realName: function (id) {
                                    ...
                                    return '<a href="javascript:;" class="btn-link" title="优化设置" onclick="destProductSet(\'' + id + '\')">设置</a>'+viewHtml;
                                }
                            }
                        ),
                        listeners: {
    ...
    function togStatus(obj, record, field) {
        ...
        Ext.Ajax.request({
            url: SITEURL+"photo/admin/destination/destination/action/update",
            method: "POST",
            datatype: "JSON",
    ...
    function destProductSet(dest_id) {
        ...
        ST.Util.showBox('设置',SITEURL+'photo/admin/destination/dialog_productinfo?id='+id+"&typeid="+display_mode,600,'',null,null,document,{loadCallback:setProductInfo,loadWindow:window});
    ...
    function searchDest() {
        ...
        if (!s_str)
            return;
        Ext.Ajax.request({
            url: SITEURL+'photo/admin/destination/destination/action/search',
