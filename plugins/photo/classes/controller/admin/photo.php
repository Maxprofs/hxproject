...
class Controller_Admin_Photo  extends Stourweb_Controller{
...
a.addtime,a.modtime,a.ishidden,b.isjian,ifnull(b.displayorder,999999) as displayorder,b.isding from sline_photo as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=6) where $w $order limit $start,$limit";
			...
			foreach($list as $k=>$v)
			{
			   ...
			   // $v['attrname']=Model_Photo_Attr::getAttrnameList($v['attrid']);
                $v['modtime']=Common::myDate('Y-m-d',$v['modtime']);
                $v['url'] = Common::getBaseUrl($v['webid']) . '/photos/show_' . $v['aid'] . '.html';
				$new_list[]=$v;
			}
		...
		else if($action=='update')//更新某个字段
		{
			...
			else  //如果不是排序字段
			{
                ...
                if ($field == 'kindlist')
                {
                    $value_arr[$field] = implode(',', Model_Destinations::getParentsStr($val));
                }
                else
                {
                    $value_arr[$field] = $val;
                }
                $isupdated = DB::update('photo')->set($value_arr)->where('id','=',$id)->execute();
                if($isupdated)
                    echo 'ok';
                else
                    echo 'no';
   ...
   */
    public function action_edit()
    {
        ...
        $info['piclist_arr'] = json_encode(Model_Photo_Picture::get_pictures_bypid($info['id']));//图片数组
