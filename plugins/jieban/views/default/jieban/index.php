...
<meta charset="utf-8">
    ...
    {/if}
    {include "pub/varname"}
    {Common::css_plugin('jieban.css','jieban')}
...
<body>
  ...
  <div class="st-jieban-home-top">
      ...
      <div class="top-position-img">
          {st:ad action="getad" name="JiebanTopAd" pc="1" row="1" return="ad"}
          {if !empty($ad)}
            <a class="ad-img" {if !empty($ad['adlink'])}href="{$ad['adlink']}"{else}href="javascript:;"{/if} target="_blank"><img src="{$ad['adsrc']}" alt="{$ad['adname']}" width="1920" height="380"></a>
          {else}
            <a class="ad-img" href="javascript:;" target="_blank"><img src="{$GLOBALS['cfg_public_url']}images/jieban-top-img.jpg" width="1920" height="380"></a>
          {/if}
      </div>
          	...
          	<ul>
                {loop $list $row}
            	    <li {if $n%4 == 0}class="mr_0"{/if}>
                        <div class="pic"><a href="{$row['url']}" target="_blank"><img src="{Product::get_lazy_img()}" st-src="{Common::img($row['litpic'],250,169)}" /></a></div>
                        <div class="con">
                            <a href="{$row['url']}" target="_blank">{$row['title']}</a>
                          <p>{$row['description']}</p>
                        </div>
                        <div class="user">
                          <img src="{Product::get_lazy_img()}" st-src="{Common::img($row['memberpic'],45,45)}"/>
