...
class  Controller_Pc_Brokerage extends Stourweb_Controller
{
    ...
    function before()
    {
        ...
        //登陆状态判断
        $this->_id = Cookie::get('st_distributor_id');
        if (empty($this->_id)) {
            $this->request->redirect('pc/login');
        } else {
            $this->_user_info = Model_Supplier::get_distributor_byid($this->_id);
    ...
    public function action_index()
    {
        ...
        $where = "a.id>0 and a.distributorlist=" . $this->_id;
        if ($status) {
        ...
        $sql = "select a.*,b.productname,b.status as order_status,c.distributorname,b.paytype,b.dingjin from sline_distributor_brokerage as a LEFT JOIN sline_member_order as b  on a.ordersn=b.ordersn  LEFT JOIN sline_distributor as c  on  a.distributorlist=c.id WHERE $where order by a.addtime desc limit $start,$pagesize";
    ...
    public function action_approval()
    {
        ...
        $page = $_GET['page'] ? $_GET['page'] : 1;
        $status = $_GET['status'];
        $where = " a.id>0 and b.id>0 and a.distributorid=" . $this->_id;
        switch ($status) {
            case 1://未审核
        ...
        $sql = "select a.*,b.distributorname from sline_distributor_finance_drawcash as a LEFT JOIN sline_distributor as b on a.distributorid=b.id WHERE $where ORDER by a.id desc limit $start,$pagesize";
        $list = DB::query(1, $sql)->execute()->as_array();
        foreach ($list as &$l) {
            $l['addtime'] = date('Y-m-d H:i:s', $l['addtime']);
    ...
    public function action_approval_apply()
    {
        ...
        $price_arr = Model_Supplier_Brokerage::get_distributor_price_info($this->_id);
    ...
    public function action_ajax_save_approval()
    {
        ...
        $rsn = DB::insert('distributor_finance_drawcash',array_keys($data))
    ...
    public function action_stat()
    {
        $price_arr = Model_Supplier_Brokerage::get_distributor_price_info($this->_id);
        //总结算金额
        $total_brokerage_price = DB::select(DB::expr('sum(brokerage) as total'))->from('distributor_brokerage')
            ->where('distributorlist','=',$this->_id)->execute()->get('total');
        //未结算金额
