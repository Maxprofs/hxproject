...
class Model_Supplier_Brokerage extends ORM
{
    ...
    public static function add_brokerage($ordersn,$order=null)
    {
        ...
        if(!$order['status']||!$order['distributorlist'])
        {
            $order = DB::select('id','status','distributorlist')->from('member_order')
                ->where('ordersn','=',$ordersn)->execute()->current();
        }
        //必须是已经付款的订单,且有供应商
        if(!in_array($order['status'],array(2,5))||!$order['distributorlist'])
        {
            return false;
        }
        ...
        $data = array(
            'distributorlist'=>$order['distributorlist'],
            'ordersn'=>$ordersn,
            'addtime'=>time(),
        ...
        try
        {
            DB::insert('distributor_brokerage',array_keys($data))
    ...
    public static function auto_change_status()
    {
        ...
        //待消费开始自动结算
        if($config['cfg_distributor_brokerage_type']==1)
        {
            $days = $config['cfg_distributor_brokerage_start_days'];
            empty($days) ?  $time= time() : $time = strtotime("-$days days");
            $sql ="select a.id from sline_distributor_brokerage as a LEFT JOIN sline_member_order as b on a.ordersn=b.ordersn WHERE a.status=1 and b.status in (2,5) and (b.paytime<=$time OR  b.`paytime` IS NULL)";

        }
        //订单完成自动结算
        else
        {
            $days = $config['cfg_distributor_brokerage_finish_days'];
            empty($days) ?  $time= time() : $time = strtotime("-$days days");
            $sql ="select a.id from sline_distributor_brokerage as a LEFT JOIN sline_member_order as b on a.ordersn=b.ordersn WHERE a.status=1 and b.status=5 and (b.finishtime<=$time OR  b.`finishtime` IS NULL)";
        }
        ...
        if($ids)
        {
            ...
            DB::update('distributor_brokerage')->set($data)->where("id in ($ids)")->execute();
        }
        return true;
    }
     ...
     * @function 获取供应商的金额情况
     */
    public static function get_distributor_price_info($distributorid)
    {
        if(!$distributorid)
        {
            return array();
        }
        $withdraw_price_finish = self::get_distributor_withdraw_price($distributorid,1);//已提现的金额
        $withdraw_price_approval = self::get_distributor_withdraw_price($distributorid,0);//申请中的金额
        $brokerage_price = self::get_distributor_brokerage_price($distributorid);
        return array(
     ...
     * @param $status 提现状态
     */
    public static function get_distributor_withdraw_price($distributorid,$status)
    {
        return DB::select(DB::expr('sum(withdrawamount) as price'))
            ->from('distributor_finance_drawcash')
     ...
     * @param $distributorid
     */
    public static function get_distributor_brokerage_price($distributorid)
    {
        return DB::select(DB::expr('sum(finish_brokerage) as price'))
            ->from('distributor_brokerage')
            ->where('distributorlist','=',$distributorid)
            ->execute()->get('price');
    ...
    public static function get_total_price_info()
    {
        ...
        $total_price = DB::select(DB::expr('sum(finish_brokerage) as total'))
            ->from('distributor_brokerage')->execute()->get('total');
        //已经完成提现的
     ...
     * @param $endtime
     */
    public static function get_distributor_time_price($distributorid,$starttime,$endtime)
    {
        if($endtime)
        {
        ...
        //佣金总
        $w = "distributorlist=$distributorid";
        if($starttime)
        {
        ...
        $total_price = DB::select(DB::expr('sum(finish_brokerage) as total'))->from('distributor_brokerage')
            ->where($w)->execute()->get('total');
        //提现记录信息
        $w = "distributorid=$distributorid";
        if($starttime)
        {
            $w .=" and addtime>=$starttime";
        }
        ...
        //提现中金额
        $withdraw_price_approval = DB::select(DB::expr('sum(withdrawamount) as total'))->from('distributor_finance_drawcash')
            ->where($w)->and_where('status','=',0)->execute()->get('total');
        //已提现金额
        $withdraw_price_finish = DB::select(DB::expr('sum(withdrawamount) as total'))->from('distributor_finance_drawcash')
