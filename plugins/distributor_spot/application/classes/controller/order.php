...
class Controller_Order extends Stourweb_Controller
{
    ...
    {
        $distributorid = Cookie::get('st_distributor_id');
        $ordersn = Arr::get($_POST, 'ordersn');
        $status = Arr::get($_POST, 'status');
        $close_reason = Arr::get($_POST, 'content') ? Arr::get($_POST, 'content') : 0;
        $order_model = ORM::factory('member_order')->where('ordersn','=',$ordersn)->and_where('distributorlist','=',$distributorid)->find();
        if($order_model->loaded()&& in_array($status,Model_Member_Order::$changeableStatus) && in_array($order_model->status,Model_Member_Order::$changeableStatus))
        {
            ...
            {
                $this->send_order_msgnotice($ordersn, $distributorid);
    ...
    public function send_order_msgnotice($ordersn, $distributorid)
    {
        $order = DB::select()->from('member_order')->where('ordersn', '=', $ordersn)->and_where('distributorlist', '=', $distributorid)->execute()->current();
        if (empty($order))
        {
            return;
        }
