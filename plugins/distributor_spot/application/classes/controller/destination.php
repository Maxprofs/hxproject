...
class Controller_Destination extends Stourweb_Controller
{
    ...
    */
    public function action_ajax_getDestsetList()
    {
        ...
        if ($keyword)
            $sql = "select id,kindname,pinyin from sline_destinations where kindname like '%{$keyword}%' and isopen=1 and find_in_set(5,opentypeids)  order by pinyin asc";
        else
            $sql = "select id,kindname,pinyin from sline_destinations where pid=$pid and isopen=1 and find_in_set(5,opentypeids) order by pinyin asc";
        $destlist = DB::query(Database::SELECT, $sql)->execute()->as_array();
        foreach ($destlist as $key => $row)
        {
            $sql = "select count(*) as num from sline_destinations where pid='{$row['id']}' and isopen=1 and find_in_set(5,opentypeids)";
