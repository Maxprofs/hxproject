...
Product={
    ...
    setStartPlace:function (dom,selector) {
        var id=$(selector).find("input:hidden[name='startcity']")?$(selector).find("input:hidden[name='startcity']").val():'';
        CHOOSE.setSome("设置出发地",{maxHeight:500,loadWindow:window,
        ...
        },SITEURL+'index/dialog_setstartplace/id/'+id+"?selector="+encodeURI(selector),1);
    },
    ...
    getSupplier:function(dom,selector)
    {
        ...
        distributor=distributor?distributor.slice(0,-1):'';
        CHOOSE.setSome("设置供应商",{loadWindow:window,loadCallback:Product.listSupplier,maxHeight:500},SITEURL+'distributor/dialog_set?&distributors='+distributor+'&selector='+encodeURI(selector),1);
    ...
    listSupplier:function(result,bool)
    {
        ...
        for(var i in result.data)
        {
            html+="<span><s onclick=\"$(this).parent('span').remove()\"></s>"+ result.data[i].distributorname+"<input type='hidden' name='distributorlist[]' value='"+ result.data[i].id+"'/></span>";
            $(result.selector).html(html);
        }
    },
