...
class Controller_Index extends Stourweb_Controller {
	...
	public function action_list() {
		...
		$w = "a.id is not null and a.distributorlist='$distributorid'";
		...
		if (!empty($keyword)) {
			...
			$w .= preg_match('`^\d+$`', $keyword) && preg_match('`^[A-Za-z]\d+`', $serial) ? " and a.id={$keyword}" : " and (a.title like '%{$serial}%' or a.distributorlist in(select id from sline_distributor where distributorname like '%{$serial}%'))";
		}

		$sql = "select a.refuse_msg,a.status,a.id,a.aid,a.title,a.distributorlist,a.iconlist,a.finaldestid,a.price,a.startcity,a.attrid,a.webid,a.kindlist,a.ishidden,IFNULL(b.displayorder,9999) as displayorder from sline_line as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=1)   where $w $order  limit $start,$pagesize";
		...
		foreach ($list as $k => &$v) {
			...
			//供应商信息
			$distributor = ORM::factory('distributor')->where("id='{$v['distributorlist']}'")->find()->as_array();
			$v['distributorname'] = $distributor['distributorname'];
			$v['linkman'] = $distributor['linkman'];
			$v['mobile'] = $distributor['mobile'];
			$v['address'] = $distributor['address'];
			$v['qq'] = $distributor['qq'];
	...
	public function action_suit() {
		...
		if (!empty($keyword)) {
			$searchParams['keyword'] = $keyword;
			$w .= preg_match('`^\d+$`', $keyword) && preg_match('`^[A-Za-z]\d+`', $keyword) ? " and a.id={$keyword}" : " and (a.title like '%{$keyword}%' or a.distributorlist in(select id from sline_distributor where distributorname like '%{$keyword}%'))";
		}
		$w .= $webid == '-1' ? '' : " and a.webid=$webid";

		$sql = "select a.id,a.aid,a.title,a.distributorlist,a.iconlist,a.finaldestid,a.price,a.startcity,a.attrid,a.webid,a.kindlist,a.ishidden,a.piclist,IFNULL(b.displayorder,9999) as displayorder from sline_line as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=1)   where $w $order  limit $start,$pagesize";
	...
	public function action_edit() {
		...
		if (!$model->loaded() || $model->distributorlist != $distributorid) {
			exit("没有权限操作此线路");
		}
	...
	public function action_ajax_linesave() {
		...
		$data_arr['distributorlist'] = $distributorid;
		$data_arr['status'] = 0;
		$data_arr['ishidden'] = 1;
		...
		if ($lineid == 0) {
			...
			if ($model->distributorlist != $distributorid) {
				exit('权限不足');
			}
