...
class Product
{
    ...
    * */
    ...
    public static function add_order($arr)
    {
        ...
        if (is_array($arr))
        {
            ...
            $distributorlist = Model_Model::get_product_bymodel($arr['typeid'], $arr['productautoid'], 'distributorlist');
            if ($distributorlist != null)
            {
                $arr['distributorlist'] = $distributorlist[0]['distributorlist'];
            }
            ...
            if ($flag)
            {
                ...
                $cfg_distributor_msg_open = $GLOBALS['cfg_distributor_msg_open'];
                $content = $GLOBALS['cfg_distributor_msg'];

                if ($cfg_distributor_msg_open == 1)
                {
                    ...
                    if (!empty($cfg_distributor_msg_open))
                    {
                        $distributorphone = self::get_distributor_tel($arr['productautoid'], $arr['typeid']);

                        if (!empty($distributorphone))
                        {
                            self::send_msg($distributorphone, $prefix, $content);//发送短信.
                        }
    ...
    * */
    ...
    public static function calculate_price($orderinfo)
    {
        ...
        $result['payPrice'] = Model_Member_Order::cur_order_total_pay_price($orderinfo['ordersn']);
        return $result;
    }
     ...
     * 获取供应商手机号码
     */
    public static function get_distributor_tel($productid, $typeid)
    {
        ...
        $sql = "SELECT distributorlist FROM {$table} where id='$productid'";
        $row = DB::query(1, $sql)->execute()->as_array();
        $distributorid = $row[0]['distributorlist'];
        $sql = "SELECT mobile FROM `sline_distributor` WHERE id='$distributorid'";
        $row = DB::query(1, $sql)->execute()->as_array();
