...
class Controller_Index extends Stourweb_Controller
{
    ...
    public function action_ajax_save()
    {
        ...
        if($action == 'add' && empty($productid))
        {
            ...
            $model->supplierlist = Cookie::get('st_supplier_id');
            $model->addtime = time();
            $model->ishidden = 1;
        }
        else
    ...
    {
        $supplier_id = $this->_user_info['id'];
        ...
        if($model->loaded() && $model->supplierlist!=$supplier_id)
        {
    ...
    * */
    ...
    public function action_ajax_verify_product()
    {
        ...
        if(!$ticket->loaded()||$ticket->supplierlist!=$supplierid)
        {
    ...
    {
        $cfg_spot_supplier_check_auto = DB::select('value')
            ->from('sysconfig')
            ->where('varname','=','cfg_spot_supplier_check_auto')
            ->execute()
            ->get('value');
        $data = array(
            'status'=>1
        ...
        if($cfg_spot_supplier_check_auto==0)
        {
