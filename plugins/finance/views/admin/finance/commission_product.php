...
<head>
    <meta charset="utf-8">
    <title>返佣比例和金额-笛卡CMS{$coreVersion}</title>
...
<table class="content-tab" div_head=ROACXC >
    <tr>
            ...
            <div class="cfg-search-bar" id="search_bar">
                <span class="select-box w150 mt-3 ml-10 fl">
                    <select class="select" id="fx_products">
                        ...
                        <option value="{$product['id']}" {if $typeid==$product['id']}selected="selected"{/if}>{$product['modulename']}</option>
                        {/loop}
                    </select>
                </span>
    ...
    function()
    {
        ...
        window.product_store=Ext.create('Ext.data.Store',{
            ...
            proxy:{
                ...
                api: {
                    read: SITEURL+'finance/admin/financeextend/config_commission_product/action/read/typeid/{$typeid}',  //读取数据的URL
                    update:SITEURL+'finance/admin/financeextend/config_commission_product/action/save/typeid/{$typeid}',
                    destroy:SITEURL+'finance/admin/financeextend/config_commission_product/action/delete/typeid/{$typeid}'
                },
                reader:{
                    type: 'json',   //获取数据的格式
                    root: 'lists',
...
$("#fx_products").change(function(){
    ...
    var url=SITEURL+'finance/admin/financeextend/config_commission_product/typeid/'+tid+'/parentkey/finance/itemid/5{if $menuid}/menuid/{$menuid}{/if}';
        ...
        onshow:function(){
            $("#ratio_set_btn").click(function(){
                ...
                $(".product_check:checked").each(function(){
                    var id=$(this).val();
                    updateFields(id,{'commission_type':commission_type,'commission_ratio':commission_ratio,'commission_cash':commission_cash})
                })
                dlg.remove();
            });
        }
