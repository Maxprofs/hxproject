...
class Controller_Admin_Financeextend extends Stourweb_Controller
{
    ...
    public function action_config_commission_product()
    {
        ...
        if(empty($action))
        {
            $supplier_list = DB::query(Database::SELECT, "SELECT id,suppliername FROM sline_supplier WHERE find_in_set({$typeid},authorization)")->execute()->as_array();
            $productlist = Model_Member_Order_Extend::get_commission_product();
        ...
        elseif($action == 'read')
        {
            ...
            $keyword = Arr::get($_GET, 'keyword');
            $supplier = Arr::get($_GET, 'supplier');
            ...
            $w .= !empty($supplier)? " and c.id=$supplier":'';
