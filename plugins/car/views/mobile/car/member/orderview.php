...
<head>
    ...
    {Common::css('base.css')}
    {Common::css_plugin('order.css','car')}
        ...
        <div class="order-pdt-info">
            <a class="pdt-info-tw" href="{$product['url']}">
                ...
                <div class="con">
                    ...
                    <p class="date">{$info['departdate']}还车</p>
                </div>
            </a>
        ...
        <div class="order-fix-total-bar">
            ...
            <span class="btn-block">
             ...
             {if $info['status']==5 && $info['ispinlun']!=1}
              ...
              {/if}
                 {if $info['status']==2}
               <a href="{$cmsurl}cars/member/order_refund?ordersn={$info['ordersn']}" data-ajax="false" class="btn btn-blue refund_btn">申请退款</a>
              {/if}
                 {if $info['status']==6}
    ...
    <script>
        ...
        $('.refund_cancel_btn').click(function () {
            ...
            layer.open({
                ...
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    $.post(SITEURL+'cars/member/ajax_order_refund_back', {ordersn:ordersn}, function (result) {
                        parent.layer.open({
