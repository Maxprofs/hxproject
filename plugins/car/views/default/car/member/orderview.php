...
<div class="user-content-wrap fr" style="overflow:hidden">
    ...
    <div class="os-term">
        ...
        <div class="os-block">
            <div class="order-show-cheap">
                <ul>
                    <li><em>{__('支付方式')}：</em>{$info['paytype_name']} &nbsp;{if $info['paytype']==2}({$info['dingjin']}/{__('辆')}){/if}</li>
    ...
    <div class="os-term">
        ...
        <div class="os-block">
            <div class="order-show-info">
                ...
                <table width="100%"  class="order-show-table" border="0" padding_strong=u2QzDt >
                    <tr>
                        <th width="35%" height="40" scope="col"><span class="l-con">{__('产品名称')}</span></th>
                        <th width="15%" scope="col">{__('使用日期')}</th>
                        <th width="15%" scope="col">{__('还车日期')}</th>
                        <th width="10%" scope="col">{__('数量')}</th>
                        <th width="12%" scope="col">{__('单价')}</th>
                        <th width="13%" scope="col">{__('总价')}</th>
                    </tr>
                    <tr>
...
<script>
    $(function () {
        ...
        $("#apply-refund-Click").on("click", function () {
            parent.layer.open({
                ...
                title: "申请退款",
                area: ['560px','570px'],
                content: '{$cmsurl}cars/member/order_refund?ordersn={$info['ordersn']}',
                btn: ['确认', '取消'],
                btnAlign: 'C',
                closeBtn: 0,
    ...
    function refund_status(data) {
        $.post('{$GLOBALS["cfg_basehost"]}/cars/member/ajax_order_refund', data, function (result) {
            parent.layer.open({
...
<script>
    ...
    function refund_status(data) {
        $.post('{$GLOBALS["cfg_basehost"]}/cars/member/ajax_order_refund_back', data, function (result) {
            parent.layer.open({
                content: result.message,
                btn: ['{__("OK")}'],
                end:function(){
