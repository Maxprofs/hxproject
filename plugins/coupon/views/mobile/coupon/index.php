...
<head padding_strong=C2QzDt >
    <meta charset="UTF-8">
    <title>领取优惠券-{$GLOBALS['cfg_webname']}</title>
    {if $seoinfo['keyword']}
    ...
    {Common::js('lib-flexible.js,jquery.min.js,common.js,layer/layer.m.js,template.js')}
</head>
<body>
<header>
    ...
    <li class="clearfix {{if value.status==2}}after{{else if value.status==1||value.status==4}}un{{/if}}">
        ...
        <div class="item-r fr">
            <div class="con">
                ...
                <i class="ico"><img src="{$img_url}/plugins/coupon/public/mobile/images/coupon-ico-02.png" alt="" title="" /></i>
                {{else if value.status==3}}
                <i class="ico"><img src="{$img_url}/plugins/coupon/public/mobile/images/coupon-ico-01.png" alt="" title="" /></i>
                <a class="btn" href="{$cmsurl}coupon/search-{{value.id}}" >立即使用</a>
                {{/if}}
            </div>
        </div>
    </li>
...
<script type="text/javascript">
    ...
    function get_coupon(couponid)
    {
        ...
        $.ajax({
            type: 'POST',
            url:  '{$cmsurl}coupon/ajxa_get_coupon',
    ...
    function get_morecoupon()
    {
        ...
        $("#page").val(page+1);
        var url = '{$cmsurl}coupon/ajax_get_list';
        $.ajax({
            type:'post',
            dataType:'json',
