                ...
                <div class="save-value-div mt-2 ml-10 insurance-sel">
                    {loop $memberlist  $member}
                                     <span style="display: inline-block;float: none" onclick="$(this).parent('span').remove()" >
                                         ...
                                         {$member[nickname]}({$member['truename']})
    ...
    Ext.onReady(
        function () {
            ...
            window.product_store = Ext.create('Ext.data.Store', {
                ...
                proxy: {
                    type: 'ajax',
                    extraParams: {typeid: '{$typeid}',productid:'{$productid}',planids:'{$planids}'},
                    api: {
                        read: SITEURL+'coupon/admin/coupon/member_give?action=dialog_getmember&method=read&memberid={$memberid}',  //读取数据的URL
                    },
                    reader: {
                        type: 'json',   //获取数据的格式
                        root: 'lists',
