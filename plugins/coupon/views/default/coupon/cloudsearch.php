...
<head>
<meta charset="utf-8">
<title>{$keyword}{__('搜索结果')}-{$webname}</title>
    {include "pub/varname"}
...
{request "pub/header"}
  <div class="big">
  	<div class="wm-1200">
    	...
    	<div class="st-guide">
            <a href="{$cmsurl}">{$GLOBALS['cfg_indexname']}</a>&nbsp;&nbsp;&gt;&nbsp;&nbsp;{$couponinfo['name']}
        </div><!--面包屑-->
    	<div class="st-main-page">

        <div class="cloud-search-side">
           {loop $leftnav $nav}
        	<a {if $typeid==$nav['typeid']}class="on"{/if} href="{$cmsurl}coupon/search-{$cid}?typeid={$nav['typeid']}"><strong>{$nav['channelname']}</strong>{$nav['num']}</a>
           {/loop}
        </div><!-- 搜索导航 -->
        <div class="cloud-search-box">
        	<div class="cs-tit">{__('共有')}<strong>{$total}</strong>个内容满足&nbsp;&nbsp;<strong >{$couponinfo['name']}</strong>&nbsp;&nbsp;使用条件</div>
