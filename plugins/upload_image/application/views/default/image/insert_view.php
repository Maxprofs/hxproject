...
<head>
    <meta charset="utf-8">
    <title>图片上传-笛卡CMS</title>
    {Common::js('jquery.min.js,image/plupload.full.min.js,image/zh_CN.js,common.js')}
    {include "pub/public_js"}
    {Common::css('admin_base.css,admin_base2.css,admin_style.css,image.css,base_new.css')}
</head>
<body>
        ...
        <div class="photo_net">
            <table width="100%" border="0">
                <tr>
                    ...
                    <td width="20%"><a href="javascript:;" class="add_net_btn btn btn-primary radius size-S">添加</a></td>
                </tr>
            </table>
        </div>
    </div>
...
function getImages(p,search_type){
    ...
    $.post(SITEURL + 'image/image_manage', {action: 'find', pid:pid ,'page':p, 'keyword':keyword,'search_type':search_type,limit_ext:limit_ext}, function (data) {
...
//    ST.Util.closeBox();
    window.parent.window.ST.Util.closeBox();
});
...
$('.add_net_btn').live('click',function(){
    ...
    var html='<tr><th width="20%" height="40" align="right" scope="row">图片地址：</th><td width="60%"><input type="text" class="ads_text"/></td>'
        +'<td width="20%"><a href="javascript:;"  class="add_net_btn btn btn-primary radius size-S">添加</a></td></tr>';
