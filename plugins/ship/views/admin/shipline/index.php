...
<head>
<meta charset="utf-8">
<title>邮轮线路列表-笛卡CMS{$coreVersion}</title>
...
<script>
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            ST.Util.addTab('添加邮轮航线','{$cmsurl}ship/admin/shipline/add/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1',0);
        });
					  ...
					  //  return  "<input type='checkbox' class='product_check' value='"+value+"'/>"; 
                                     ...
                                     var name=record.get('title');
			                          if(id.indexOf('suit')!=-1)
                                      {
									     ...
									     return "<a href='javascript:;' class='btn-link' title='修改套餐'  onclick=\"ST.Util.addTab(\'修改套餐\',\'ship/admin/shipline/editsuit/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/1/suitid/"+suitid+"\')\">修改套餐</a>";
                                      }
                                      else
                                      {
  ...
  function goModify(lineid)
  {
      ...
      parent.window.addTab('修改-'+title,SITEURL+'ship/admin/shipline/edit/lineid/'+lineid+'/{if isset($_GET['menuid'])}menuid/{$_GET['menuid']}/{/if}parentkey/product/itemid/{$itemid}',1);
  }
