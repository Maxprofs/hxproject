...
<div class="user-content-wrap fr" style="overflow:hidden">
    ...
    <div class="os-term">
        ...
        <div class="os-block">
            <div class="order-show-cheap">
                <ul>
                    <li><em>{__('支付方式')}：</em>{$info['paytype_name']} &nbsp;{if $info['paytype']==2}({$info['dingjin']}/{__('间')}){/if}</li>
    ...
    <div class="os-term">
        ...
        <div class="os-block">
            <div class="order-show-info">
                ...
                <table width="100%" border="0" class="order-show-table" html_div=BmACXC >
                    <tr>
                        <th width="26%" height="40" scope="col"><span class="l-con">{__('房型名称')}</span></th>
                        <th width="17%" scope="col">{__('出行日期')}</th>
                        <th width="17%" scope="col">{__('结束日期')}</th>
                        <th width="10%" scope="col">{__('单价/间')}</th>
                        <th width="10%" scope="col">{__('房间数')}</th>
                        <th width="10%" scope="col">{__('人数')}</th>
                        <th width="10%" scope="col">{__('总价')}</th>
                    </tr>
                    {loop $sublist $sub}
                    <tr>
                        <td height="40"><span class="l-con">{$sub['title']}</span></td>
                        <td>{$info['usedate']}</td>
                        <td>{$info['departdate']}</td>
                        <td>{$sub['price']}</td>
                        <td>{$sub['dingnum']}</td>
                        <td>{$sub['peoplenum']}</td>
                        <td><span class="jg"><i class="currency_sy">{Currency_Tool::symbol()}</i>{php}echo intval($sub['price']) * intval($sub['dingnum']);{/php}</span></td>
                    </tr>
                    {/loop}
                </table>
            </div>
...
<script>
    $(function () {
        ...
        $("#apply-refund-Click").on("click", function () {
            parent.layer.open({
                ...
                title: "申请退款",
                area: ['560px','570px'],
                content: '{$cmsurl}ship/member/order_refund?ordersn={$info['ordersn']}',
                btn: ['确认', '取消'],
                btnAlign: 'C',
                closeBtn: 0,
    ...
    function refund_status(data) {
        $.post('{$GLOBALS["cfg_basehost"]}/ship/member/ajax_order_refund', data, function (result) {
            parent.layer.open({
...
<script>
    ...
    function refund_status(data) {
        $.post('{$GLOBALS["cfg_basehost"]}/ship/member/ajax_order_refund_back', data, function (result) {
            parent.layer.open({
                content: result.message,
                btn: ['{__("OK")}'],
                end:function(){
