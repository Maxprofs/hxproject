...
(function($) {
	...
	var _gridRows = 0;
	var _totalBoxes = 0;
	var _dateRange = { startDate: new Date(), endDate: null };
	...
	var defaults = {
            ...
            onMoreBtnClick:function(ele,event){return true;},
            onTdOver:function(ele,event){return true;},
            onTdOut:function(ele,event){return true;},
             onTdClick:function(ele,event){return true;},
            locale: {
	...
	var _initHeaders = function() {
        ...
        monthNavHead.append(jQuery('<div class="MonthName"></div>').append(_workingDate.getFullYear()+"年"+defaults.locale.months[_workingDate.getMonth()] + "月"));
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		for (var i = 0; i < _totalBoxes; i++) {
			...
			//DateBox Events
			var dateLink = jQuery('<div class="DateLabel"><a href="javascript:;">' + currentDate.getDate() + '</a></div>').click(function(e) {
			...
			jQuery.each(calendarEvents, function(){
				...
				for (var i = istart; i <= iend; i++) {
					...
					if (toManyEvents) {
                        ...
                        tdContent.append(block);
                        var dataDescription = jQuery('<div class="Event">'+ev.Description+'</div>')


                        var morebtn = jQuery('<span class="more">'+ '更多' +'</span>');
                        morebtn.click(function(e) { defaults.onMoreBtnClick(morebtn,ev); e.stopPropagation();	});
                        var prices = jQuery('<span class="price">' + '<em>' + CURRENCY_SYMBOL + '<b>' + ev.Price + '</b>' + '</em>' + '起/人' + '</span>');

                        var dataPrice = jQuery('<div class="Event"></div>');
                        dataPrice.append(morebtn);
                        dataPrice.append(prices);
                        tdContent.append(dataDescription);
                        tdContent.append(dataPrice);


						_eventObj[block.attr("id")] = tdContent;

                        b.cell.append(tdContent);
                        b.cell.mouseover(function(e) { defaults.onTdOver(b.cell,ev); e.stopPropagation();	});
                        b.cell.mouseout(function(e) { defaults.onTdOut(b.cell,ev); e.stopPropagation();	});
                        b.cell.click(function(e) { defaults.onTdClick(b.cell,ev); e.stopPropagation();	});
	...
	var _buildEventBlock = function(event, weekNumber) {
        ...
        var dataDescription = jQuery('<div class="Event" id="Event_' + event.EventID + '"></div>')
        var prices = jQuery('<span class="more">'+ '更多' +'</span><span class="price">' + '<em>' + '&yen;' + '<b>' + event.Price + '</b>' + '</em>' + '起' + '</span>');
        var dataPrice = jQuery('<div class="Event" id="Event_' + event.EventID + '"></div>').append(prices);
        content.append(dataPrice);*/
