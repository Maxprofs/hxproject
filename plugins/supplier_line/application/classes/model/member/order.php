...
class Model_Member_Order extends ORM {
	...
	public function get_my_order_list($typeid, $pagesize) {
		...
		$st_supplier_id = intval(Cookie::get('st_supplier_id'));
		$whereStr = " FIND_IN_SET('{$st_supplier_id}',supplierlist) AND typeid=$typeid and dconfirm=1";
		$searchKey = trim($_GET['searchKey']);
		if ($searchKey != '') {
			$whereStr .= " AND (ordersn='{$searchKey}' OR eticketno='{$searchKey}')";
		}
		//查询出待消费,已完成,待确认的订单
		$whereStr .= " AND (status=3 OR status=5 OR status=0 OR status=1 OR status=2 OR status=6)";
		//2.路由字符串
		$url_array = array(
			'controller' => 'order',
			'action' => 'all',
	...
	public function get_my_all_order_list($typeid, $status, $where = false) {
		...
		$st_supplier_id = intval(Cookie::get('st_supplier_id'));
		$whereStr = " FIND_IN_SET('{$st_supplier_id}',supplierlist) AND typeid=$typeid";
		$searchKey = trim($_GET['searchKey']);
		if ($searchKey != '') {
			$whereStr .= " AND (ordersn='{$searchKey}' OR eticketno='{$searchKey}')";
		}
		if ($status !== null && $status !== '') {
			...
			$whereStr .= " AND (status=2 OR status=5 OR status=0)";
		}
		if ($where) {
			$whereStr .= $where;
		}
		...
		if ($whereStr) {
			$page_object = ORM::factory($db_name)->where($whereStr);
		} else {
	...
	public function get_order_info($id) {
		...
		$data = ORM::factory('member_order')
			->where($whereStr)
			->get_all();
		$data = self::_get_data_format($data[0]);
	...
	private function _get_data_format($data) {
		if (empty($data)) {
			...
			//产品地址
			$product = self::_get_product_url($data['productautoid'], $data['typeid']);
			$data['url'] = $product['url'];
			$data['productname'] = $product['title'] ? $product['title'] : $data['productname'];
			//价格类型——套餐名称
			$data['pricetype'] = self::_get_product_suit_name($data['suitid'], $data['typeid']);
	...
	private function _get_product_suit_name($suitid, $typeid) {
		...
		if ($suitid > 0) {
			switch ($typeid) {
			case 1: //线路
				$suit = self::_get_field_by_where(array('suitname'), "id='{$suitid}'", 'line_suit');
				$suit_name = $suit['suitname'];
				break;
			case 2: //酒店
				$suit = self::_get_field_by_where(array('roomname'), "id='{$suitid}'", 'hotel_room');
				$suit_name = $suit['roomname'];
				break;
			case 3: //租车
				$suit = self::_get_field_by_where(array('suitname'), "id='{$suitid}'", 'car_suit');
				$suit_name = $suit['suitname'];
				break;
			case 5: //门票
				$suit = self::_get_field_by_where(array('title'), "id='{$suitid}'", 'spot_ticket');
	...
	private function _get_product_url($id, $typeid) {
		...
		if (!empty($row)) {
			$weburl = self::_get_webURL_by_webid($row['webid']);
	...
	private function _get_webURL_by_webid($webid) {
		$globals = self::_get_field_by_where(array('*'), "webid='{$webid}'", 'sysconfig');
		$root = '';
		$prefix = $globals['sys_webprefix'];
		$domain = self::_get_base_url($globals); //顶级域名
		if ($webid != 0) {
			$destinfo = self::_get_field_by_where(array('*'), "id='{$webid}'", 'destinations');
			$prefix = $destinfo['webprefix'];
		} else {
			$weblist = self::_get_field_by_where(array('webprefix'), "webid='0'", 'weblist');
	...
	*/
	...
	public static function order_info($ordersn) {
		...
		if ($rs['roombalancenum'] && $rs['roombalance']) {
			...
			$rs['total_price'] += intval($rs['roombalance']);
		}
	...
	*/
	 ...
	 * @function 获取计算好的收益
	 */
	public static function get_compute_info($order_id) {
