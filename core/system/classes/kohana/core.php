...
class Kohana_Core {
	...
	public static function modules(array $modules = NULL) {
		...
		$paths = array(""); //remain first position for developer module path
		$paths[] = APPPATH;
		$paths[] = TOOLS_PATH;
		...
		foreach ($modules as $name => $path) {
			if (is_dir($path)) {
				...
				if ($name == "developer") //if developer module then set to first path
				{
					$paths[0] = $modules[$name] = realpath($path) . DIRECTORY_SEPARATOR;
				} else {
